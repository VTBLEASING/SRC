create or replace package dm.PKG_CC_SE is
  /******************************************************************************
     NAME:       DWH.PKG_CC
...
create or replace package body dm.PKG_CC_SE is
  /******************************************************************************
     NAME:       DWH.PKG_CC
 ...
 */
 ...
 BEGIN
   open cur for 'select '
   ||getColList(pTable=>'DM.DM_DETAILS_DAILY',pMask=>' ,"[COL_NAME]"',pExclude=>'SNAPSHOT_DT')||
   '  FROM DM.DM_DETAILS_DAILY where snapshot_dt=:B1' using trunc(p_snapshot_dt);
   execute immediate Q'~alter session set nls_numeric_characters='. '~';
