                                ...
                                -- and cn1.close_dt >= trunc (p_REPORT_DT, 'mm')
                                 /*and ( ( cn1.IS_CLOSED_CONTRACT <> 0 
                                      ...
                                      or (cn1.IS_CLOSED_CONTRACT is null)                                --
                                           and cn1.branch_key in (select branch_key 
                                                              from dwh.cgp_group cgp_group 
                                                              where cgp_group.cgp_group_key not in (1, 4))  -- Выбираем действующие незакрытые договора
                                      ) */ 
                                  ...
                                  union all
                                    ...
                                    select   f.L_KEY, 
                                            ...
                                            where min_dt is null
                                              ...
                                              group by f.l_key, f.branch_key, pay_dt -- склейка
                ...
                then
                 ...
                 -- XIRR := xirr_calc_new(rec.contract_id)/100;
                  ...
                  ka_prev := ka;
                          ...
                          elsif ka_prev = 0 then ka := round(diff_prev2 * ka_prev2 / diff_prev, 2); -- падало на нуле
                  ...
                  values(rec.L_KEY,rec.pay_dt,(case when rec.summ < 0 then rec.summ else 0 end), (case when rec.summ > 0 then rec.summ else 0 end), rec.leasing_pay, rec.supply_pay, pers,nill,diff,p_REPORT_DT,round(ka, 2), nil_ka, rec.summ, rec.branch_key, 'Тест');
                  ...
                  insert into dm_nil (contract_id,contract_date, fact_amt, plan_amt, leasing_amt, supply_amt, nil_pers,nil,nil_diff,ODTTM, ka, nil_ka, PAY_AMT, branch_key, snapshot_cd)
                  values(rec.L_KEY,rec.pay_dt,(case when rec.summ < 0 then rec.summ else 0 end), (case when rec.summ > 0 then rec.summ else 0 end), rec.leasing_pay, rec.supply_pay, pers,nill,diff,p_REPORT_DT,round(ka, 2), nil_ka, rec.PAY_AMT, rec.branch_key, 'Тест');
                  commit;
              end if;
            end loop;
           -- end if;
...
a.supply_amt,
a.PAY_AMT, 
round (a.NIL_KA_KB * (100 / (100 + v.vat_rate*100)), 2),                                        -- учет НДС
a.NIL_PERS, 
a.NIL, 
a.NIL_DIFF, 
a.KA, 
