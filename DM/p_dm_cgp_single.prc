CREATE OR REPLACE PROCEDURE DM.p_DM_CGP_SINGLE (
...
BEGIN
  ...
  */
    dm.u_log(p_proc => 'DM.p_DM_CGP_SINGLE',
           p_step => 'INPUT PARAMS',
 ...
 and valid_to_dttm = to_date('01.01.2400','dd.mm.yyyy');
  --execute immediate 'CREATE INDEX I#DM_XIRR_FLOW_ORIGIN ON  DM.DM_XIRR_FLOW_ORIG (L_KEY, snapshot_dt, snapshot_c)';   -- создание индекса 
 
  dm.u_log(p_proc => 'DM.p_DM_CGP_SINGLE',
           p_step => 'delete from DM_CGP_HIST ',
           p_info => SQL%ROWCOUNT|| ' row(s) deleted');  

select nvl(exclude_cgp,0) into v_exclude_cgp from dwh.contracts where contract_key = p_contract_key and valid_to_dttm=to_date('01.01.2400','dd.mm.yyyy');
if v_exclude_cgp<>1 then
    insert ALL
    into DM_CGP
    (      snapshot_cd,
           snapshot_dt,
    ...
    */
    WITH contr_cess AS
          ...
          FROM
            ...
            AND contract_id_cd in (select contract_id_cd from dwh.contracts where contract_key = p_contract_key and valid_to_dttm = to_date('01.01.2400', 'dd.mm.yyyy'))
            )
          ),
    assets_transfer_flg as (
          SELECT c.cgp_contract_key AS CONTRACT_KEY, 'V' as asset_flg
                ...
                AND a.contract_key = p_contract_key
    ...
    START_DT as
        ...
        from DM.DM_XIRR_FLOW_ORIG 
          where 
            ...
            and snapshot_dt = p_REPORT_DT and SNAPSHOT_CD = p_snapshot_cd and PAY_DT<=p_REPORT_DT and L_KEY = p_contract_key
          group by L_KEY),             
     ...
     */
     Term_amt as (
           ...
           left join 
            ...
            and xirr.contract_id = p_contract_key
            ),
avg_term_amt as
     (   
...
/*                                inner join dwh.vat vat
                              ...
                              inner join dwh.contracts vat on vat.contract_key=rs.contract_key
                                ...
                                and rs.contract_key = p_contract_key
                                ...
                                from Term_amt 
                                where contract_key = p_contract_key
 ...
 /* comment Zan          ,case when sup.l_key is null then 
           (nvl(case when trunc(contr.force_close_date) <= p_REPORT_DT and contr.force_close_date >= trunc(p_REPORT_DT, 'MM') then 0 else (case 
  ...
  /*         (case
           ...
           )  avg_overdue_amt, end comment*/
   /*Add comment        case when sup.l_key is null then 
           (nvl(case when trunc(contr.force_close_date) <= p_REPORT_DT and contr.force_close_date >= trunc(p_REPORT_DT, 'MM') then 0 else (case
...
---Add By Zanozin---
           ...
           case when sup.l_key is null then 
           (nvl(case when trunc(contr.force_close_date) <= p_REPORT_DT and contr.force_close_date >= trunc(p_REPORT_DT, 'MM') then 0 else (case 
             when nvl(round(overdue.vat_overdue_amt, 2), 0) <= 0 
                then 0
             when round(overdue.overdue_vat_free_amt,2) > 0 and nvl(assets_transfer_flg.asset_flg,0) = 'V' and nvl(term.term_amt,0) = 0 
                then 0 
...
from
   ...
   */
      ...
      contr.contract_key = p_contract_key
  ...
  dm.u_log(p_proc => 'DM.p_DM_CGP_SINGLE',
           p_step => 'insert into DM_CGP_SINGLE',
