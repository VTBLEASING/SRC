create or replace package dm.PKG_OL is

  -- Author  : VLADIMIR.ZANOZIN
  -- Created : 26-ноя-17 26-ноя-17 
  -- Purpose : working with Oper Liasing

  -- Public type declarations
  -- delete form  SubDataMart and  refill SubDataMart
  procedure FILL_SUB_OL(p_REPORT_DT IN date,
                        --  p_group_key    in number,
                        p_contract_key in number default null,
                        p_snapshot_cd  varchar2 default 'Основной КИС');
  -- delete from  datamart DM_OL_FLOW_ORIG_ORIG  and execute procedure FILL_SUB_OL
  -- execute from procedure PROCESS_DM_QUEUE
  procedure DEL_OL(p_REPORT_DT    date,
                   p_exec_type    varchar2,
                   p_group_key    number,
                   p_contract_key number,
                   p_snapshot_cd  varchar2 default 'Основной КИС');

  -- fill datamart DM_OL_FLOW_ORIG_ORIG  and execute procedure FILL_SUB_OL
  PROCEDURE p_DM_OL_FLOW_KIS(p_REPORT_DT    IN date,
                             p_group_key    in number,
                             p_contract_key in number default null,
                             p_snapshot_cd  varchar2 default 'Основной КИС');
  PROCEDURE P_DM_OL_FLOW_KIS_SINGLE(p_contract_key in number,
                                    p_REPORT_DT    IN date);

end PKG_OL;
/

create or replace package body dm.PKG_OL is
  ...
  --RETURN NUMBER 
   is
  begin
    dm.u_log(p_proc => 'PKG_OL.DEL_OL',
             p_step => 'INPUT PARAMS',
             p_info => 'p_group_key:' || p_group_key || 'p_REPORT_DT:' ||
                       p_REPORT_DT || 'p_exec_type:' || p_exec_type ||
                       'p_snapshot_cd:' || p_snapshot_cd);
    DBMS_LOCK.SLEEP(10);
    IF p_exec_type = 'branch' THEN
      delete from DM.DM_OL_FLOW_ORIG olf
       where olf.snapshot_dt = p_REPORT_DT
         and branch_key in (select branch_key
                              from dwh.cgp_group
                             where cgp_group_key = p_group_key
                               and begin_dt <= p_REPORT_DT
                               and end_dt > p_REPORT_DT)
         and snapshot_cd = 'Основной КИС';
      dm.u_log(p_proc => 'PKG_OL.DEL_OL',
               p_step => 'delete from DM.DM_OL_FLOW_ORIG',
               p_info => SQL%ROWCOUNT || ' row(s) deleted');
    
    ELSIF p_exec_type = 'contract' THEN
      delete from DM.DM_OL_FLOW_ORIG OLF
       where olf.SNAPSHOT_DT = p_REPORT_DT
         and olf.l_key = p_contract_key
         and snapshot_cd = 'Основной КИС';
      dm.u_log(p_proc => 'PKG_OL.DEL_OL',
               p_step => 'delete from DM.DM_OL_FLOW_ORIG',
               p_info => SQL%ROWCOUNT || ' row(s) deleted');
    ...
    fill_sub_ol(p_report_dt => p_report_dt,
                --  p_group_key => p_group_key,
                p_contract_key => p_contract_key,
                p_snapshot_cd  => p_snapshot_cd);
  ...
  procedure FILL_SUB_OL(p_REPORT_DT IN date,
                        --  p_group_key    in number,
                        p_contract_key in number default null,
                        p_snapshot_cd  varchar2 default 'Основной КИС') is
  begin
    ...
    dm.u_log(p_proc => 'PKG_OL.FILL_OL',
  ...
  BEGIN
    dm.u_log(p_proc => 'p_DM_OL_FLOW_KIS',
             p_step => 'INPUT PARAMS',
             p_info => 'p_group_key:' || p_group_key || 'p_REPORT_DT:' ||
                       p_REPORT_DT || 'p_snapshot_cd:' || p_snapshot_cd ||
                       'p_contract_key:' || p_contract_key);
    ...
    delete from dm_ol_flow_orig
     where SNAPSHOT_dt = p_report_dt
       ...
       and (branch_key in
           (select branch_key
               from dwh.cgp_group
              where cgp_group_key = v_group_key
    ...
    insert into dm.dm_ol_flow_orig
      ...
      (select *
         from (with
              ...
              */ Contr_prev as (SELECT cn1.contract_key L_key,
                                       ...
                                       count(cn1.contract_key) over(partition by cn1.contract_key) countt
                                  from dwh.contracts cn1
                                  left join dwh.contracts cn2
                                    ON cn1.contract_key =
                                       cn2.contract_leasing_key
                                   and cn2.valid_to_dttm =
                                 ...
                                 where cn1.valid_to_dttm =
                                       to_date('01.01.2400', 'dd.mm.yyyy')
                                   and cl.valid_to_dttm =
                                       to_date('01.01.2400', 'dd.mm.yyyy')
              ...
              */ countt as (select l_key, count(L_KEY) countt
                              from contr_prev
                             group by l_KEY),
              
              Contr as (select contr.l_key l_key,
                               s_key,
                               branch_key,
                               l_cur,
                               s_cur,
              ...
              */ Contr_ces as (SELECT cn1.L_key                 L_key,
                                      cn1.S_key                 S_key,
                               ...
                               and cn1.branch_key = cn2.branch_key*/
                                 left join dwh.contracts cn3
                                  ...
                                  and cn3.valid_to_dttm =
                                      to_date('01.01.2400', 'dd.mm.yyyy')
                                  and cn3.branch_key = cn1.branch_key --cn2.branch_key
                                 Left join dwh.contracts cn4
                                   ON cn3.contract_key =
                                      cn4.contract_leasing_key
                                  and cn4.valid_to_dttm =
                                      to_date('01.01.2400', 'dd.mm.yyyy')), Contr_ces_uniq7 as (select distinct L_KEY,
                                                                                                                S_KEY_DOP,
                                                                                                                branch_key,
                                                                                                                L_CUR,
                                                                                                                S_CUR,
              ...
              */ Flow_S as (SELECT Contr.L_KEY,
                             ...
                             where fact_rp1.CBC_DESC in
                                   (select CBC_DESC
                                     ...
                                     where TYPE_CALC = 'Supply' -- выбор фактических платежей по договорам поставок с типом КБК 'Supply'
                                       and p_REPORT_DT BETWEEN BEGIN_DT and
                                           END_DT)
                               and fact_rp1.valid_to_dttm =
                                   to_date('01.01.2400', 'dd.mm.yyyy')
                             ...
                             where fact_pp.CBC_DESC in
                                   (select CBC_DESC
                                     ...
                                     where TYPE_CALC = 'Supply'
                                       and p_REPORT_DT BETWEEN BEGIN_DT and
                                           END_DT)
                               and fact_pp.valid_to_dttm =
                                   to_date('01.01.2400', 'dd.mm.yyyy')
                               and p_REPORT_DT between fact_pp.BEGIN_DT and
                                   fact_pp.END_DT
                            ...
                            SELECT /*+ NO_INDEX(fact_pp) */ Contr.L_key,
                                   ...
                                   Contr.S_KEY_DOP S_KEY,
                                   Null ex_rate
                              from (select * from Contr_ces_uniq7) Contr
                             inner join dwh.fact_plan_payments fact_pp
              ...
              Flow_S_CUR as (select L_KEY,
                                    ...
                                    S_KEY,
                                    (case
                                      when CUR1 <> s.base_currency and
                                           CUR2 <> s.base_currency and
                                           PAY_DT <= p_REPORT_DT --Если Д_ВАЛ <> Б_ВАЛ и ФП_ВАЛ <> Б_ВАЛ и ДАТА <= О_ДАТА, 
                                       then
                                       PAY_AMT * rt1.EXCHANGE_RATE /
                                       rt2.EXCHANGE_RATE --то    СУММ = СУММ * ФП_КУРС /  Д_КУРС  
                                    
                                      when CUR1 <> s.base_currency and
                                           CUR2 <> s.base_currency and
                                           PAY_DT > p_REPORT_DT --Если Д_ВАЛ <> Б_ВАЛ и ФП_ВАЛ <> Б_ВАЛ и ДАТА > О_ДАТА, 
                                       then
                                       PAY_AMT * rt_rp1.EXCHANGE_RATE /
                                       rt_rp2.EXCHANGE_RATE --то    СУММ = СУММ * ФП_КУРС_О_ДАТА /  Д_КУРС_О_ДАТА 
                                    
                                      when CUR1 = s.base_currency and
                                           CUR2 <> s.base_currency and
                                           PAY_DT <= p_REPORT_DT --Если Д_ВАЛ = Б_ВАЛ и ФП_ВАЛ <> Б_ВАЛ и ДАТА <= О_ДАТА,
                                       then
                                       PAY_AMT * rt1.EXCHANGE_RATE --то    СУММ = СУММ * ФП_КУРС
                                    
                                      when CUR1 = s.base_currency and
                                           CUR2 <> s.base_currency and
                                           PAY_DT > p_REPORT_DT --Если Д_ВАЛ = Б_ВАЛ и ФП_ВАЛ <> Б_ВАЛ и ДАТА > О_ДАТА, 
                                       then
                                       PAY_AMT * rt_rp1.EXCHANGE_RATE --то    СУММ = СУММ * ФП_КУРС_О_ДАТА
                                    
                                      when CUR2 = s.base_currency and
                                           CUR1 <> s.base_currency and
                                           PAY_DT <= p_REPORT_DT --Если Д_ВАЛ <> Б_ВАЛ и ФП_ВАЛ = Б_ВАЛ и ДАТА <= О_ДАТА,
                                      --then PAY_AMT*round(1/rt2.EXCHANGE_RATE,8)                                                   --то    СУММ = СУММ / Д_КУРС
                                       then
                                       PAY_AMT / rt2.EXCHANGE_RATE
                                      ...
                                      --then PAY_AMT*round(1/rt_rp2.EXCHANGE_RATE,8)                                                --то    СУММ = СУММ / Д_КУРС_О_ДАТА
                                       then
                                       PAY_AMT / rt_rp2.EXCHANGE_RATE --то    СУММ = СУММ / Д_КУРС_О_ДАТА
                                    
                                      else
                                       PAY_AMT --Иначе, СУММ = СУММ.
                               ...
                               left join dwh.EXCHANGE_RATES rt_rp1 -- привязка к курсам валют по равенству даты курса отчетной дате и по валюте факта/плана
                                ...
                                and rt_rp1.BASE_CURRENCY_KEY =
                                    s.base_currency
                                and rt_rp1.valid_to_dttm =
                                    to_date('01.01.2400', 'dd.mm.yyyy')
                               ...
                               left join dwh.EXCHANGE_RATES rt_rp2 -- привязка к курсам валют по равенству даты курса отчетной дате и по валюте договора
                                ...
                                and rt_rp2.BASE_CURRENCY_KEY =
                                    s.base_currency
                                and rt_rp2.valid_to_dttm =
                                    to_date('01.01.2400', 'dd.mm.yyyy')
                             
                             ),
              
              FLOW_S_UNDERPAY_1 as (select f1.L_Key,
                                           f1.branch_key,
                                           f1.CBC_DESC,
                                           (case
                                             when f1.pay_amt2 >=
                                                  nvl(f2.pay_amt_sum, 0) and
                                                  pay_dt > p_REPORT_DT then
                                              p_REPORT_DT
                                             else
                                              pay_dt
                                           end) pay_dt,
                                      ...
                                      left join (select L_Key,
                                        ...
                                        on f1.L_Key = f2.L_Key
                                       and f1.CBC_DESC = f2.CBC_DESC
              ...
              Flow_L as (SELECT /*+ NO_INDEX(fact_pp) */ Contr.L_KEY, -- Исключение задваивания в случае, если одному контракту лизинга соответствует несколько договоров поставки (flag = 1)
                                contr.branch_key,
                                fact_pp.CBC_DESC,
                                PAY_DT,
                                PAY_AMT,
                          ...
                          where (fact_pp.CBC_DESC in
                                (select CBC_DESC
                                   ...
                                   where TYPE_CALC = 'Leasing'
                                     and p_REPORT_DT BETWEEN BEGIN_DT and
                                         END_DT) or
                                (fact_pp.contract_key in
                                (select contract_key
                                     from dwh.leasing_contracts
                                    where valid_to_dttm =
                                          to_date('01.01.2400', 'dd.mm.yyyy')
                                      and SUBSIDIZATION_FLG = 1) and
                                fact_pp.CBC_DESC in
                                (select CBC_DESC
                                     from DWH.CLS_CBC_TYPE_CALC
                                    where TYPE_CALC = 'Subsidization'
                                      and p_REPORT_DT BETWEEN BEGIN_DT and
                                          END_DT))) -- выбор плановых платежей по договорам лизинга с типом классификации КБК 'Leasing'
                            and fact_pp.valid_to_dttm =
                                to_date('01.01.2400', 'dd.mm.yyyy')
                            and p_REPORT_DT between fact_pp.BEGIN_DT and
                                fact_pp.END_DT
                         ...
                         SELECT /*+ NO_INDEX(fact_pp) */ Contr.L_KEY, -- Исключение задваивания в случае, если одному контракту лизинга соответствует несколько договоров поставки (flag = 1)
                          ...
                          INNER join (select distinct L_KEY,
                                                     L_KEY_DOP,
                                                     branch_key,
                                                     L_CUR,
                                                     base_currency,
              ...
              */ Flow_Cur as (select fl.*, 0 PAY_AMT_cur_supply, Null ex_rate
                                from Flow_L fl
                              UNION ALL
                              select *
                                from Flow_S_cur
              ...
              */ Flow_L_CUR as (select L_KEY,
                                       ...
                                       */
                                       (case
                                         ...
                                         WHEN ex_rate is not Null and
                                              ex_rate = 0 and cur1 != cur2 then
                                          0
                                         WHEN ex_rate is not Null and
                                              ex_rate <> 0 and cur1 != cur2 then
                                          PAY_AMT / ex_rate /
                ...
                select L_KEY,
                       ...
                       S_KEY,
                       Case
                         when BRANCH_key <> 1 then
                          sysdate
                         else
                          (Select min(ACT_DT)
                             from dwh.LEASING_SUBJECT_TRANSMIT
                            Where valid_to_dttm =
                                  to_date('01.01.2400', 'dd.mm.yyyy')
                              And contract_app_key in
                                  (select contract_app_key
                                     from dwh.leasing_contracts_appls
                                    where valid_to_dttm =
                                          to_date('01.01.2400', 'dd.mm.yyyy')
                                      and contract_key = L_KEY))
                       end ACT_DT
                  from flow_L_CUR l)
       );
    dm.u_log(p_proc => 'p_DM_OL_FLOW_KIS',
  ...
  PROCEDURE P_DM_OL_FLOW_KIS_SINGLE(p_contract_key in number,
    ...
    */
  BEGIN
    dm.u_log(p_proc => 'p_DM_OL_FLOW_KIS_SINGLE',
             p_step => 'INPUT PARAMS',
             p_info => /*'p_group_key:'||p_group_key||*/ 'p_REPORT_DT:' ||
                                                         p_REPORT_DT /*||'p_snapshot_cd:'||p_snapshot_cd*/
                                                         ||
      ...
      (select *
         from (with
              ...
              */ Contr_prev as (SELECT cn1.contract_key     L_key,
                                 ...
                                 where cn1.valid_to_dttm =
                                   ...
                                   and LC.valid_to_dttm =
                                       to_date('01.01.2400', 'dd.mm.yyyy')
                                   and ((LC.contract_fin_kind_desc =
                                       'ОперационнаяАренда' and
                                       cn1.branch_key in
                                       ( -- В случае Головного офиса выбирается 'ФинансовыйЛизинг'
                                          select branch_key
                                            from dwh.cgp_group cgp_group
                                           where cgp_group.cgp_group_key = 2
                                             and cgp_group.begin_dt <=
                                                 p_REPORT_DT
                                             and cgp_group.end_dt > p_REPORT_DT)) or
                                       (LC.contract_fin_kind_desc is Null -- В случае дочерних организаций выбирается Null
                                       and cn1.branch_key in
                                       (select branch_key
                                                from dwh.cgp_group cgp_group
                                               where cgp_group.cgp_group_key not in
                                                     (1, 2)
                                                 and cgp_group.begin_dt <=
                                                     p_REPORT_DT
                                                 and cgp_group.end_dt >
                                                     p_REPORT_DT)))
                                      -- and cl.cgp_flg = '1'                                                -- Договор КГП
              ...
              */ Flow_S as (SELECT Contr.L_KEY,
              ...
              Flow_S_CUR as (select L_KEY,
                                    ...
                                    (case
                                      ...
                                      when CUR2 = s.base_currency and
                                           ...
                                           PAY_DT <= p_REPORT_DT --Если Д_ВАЛ <> Б_ВАЛ и ФП_ВАЛ = Б_ВАЛ и ДАТА <= О_ДАТА,
                                       then
                                       PAY_AMT / rt2.EXCHANGE_RATE --то    СУММ = СУММ / Д_КУРС
              ...
              Flow_L as (SELECT Contr.L_KEY, -- Исключение задваивания в случае, если одному контракту лизинга соответствует несколько договоров поставки (flag = 1)
                                contr.branch_key,
                                fact_pp.CBC_DESC,
                                PAY_DT,
                                PAY_AMT,
