CREATE OR REPLACE PROCEDURE DM.P_DM_EXP_REG_LIQ_TO_XLS (
    p_REPORT_DT date
)
is
j number;
...
BEGIN
  ...
  j := 0;
    dm.u_log(p_proc => 'DM.P_DM_EXP_REG_LIQ_TO_XLS',
           p_step => 'INPUT PARAMS',
  ...
  LOOP
      ...
      into v_flag_prev
      FROM user_objects
      where object_name = 'JOB_EXCEL_REG_LIQ_TO_XLS';
  ...
  job_name=>'JOB_EXCEL_REG_LIQ_TO_XLS',
  job_type=>'EXECUTABLE',
  number_of_arguments=>3,
  job_action=>'C:\Windows\System32\cmd.exe',
  enabled=>FALSE);
  
  DBMS_SCHEDULER.set_job_argument_value('JOB_EXCEL_REG_LIQ_TO_XLS', 1, '/c');
  DBMS_SCHEDULER.set_job_argument_value('JOB_EXCEL_REG_LIQ_TO_XLS', 2, 'D:\VTBL\scripts\COPY_TABLE_DM_REG_LIQ.vbs');
  DBMS_SCHEDULER.set_job_argument_value('JOB_EXCEL_REG_LIQ_TO_XLS', 3, TO_CHAR(p_REPORT_DT, 'DD.MM.YYYY'));
  DBMS_SCHEDULER.enable('JOB_EXCEL_REG_LIQ_TO_XLS');
  ...
  loop
      ...
      where JOB_NAME = 'JOB_EXCEL_REG_LIQ_TO_XLS'; 
   ...
   END LOOP;
    dm.u_log(p_proc => 'DM.P_DM_EXP_REG_LIQ_TO_XLS',
           p_step => 'JOB FINISH: JOB_EXCEL_REG_LIQ_TO_XLS',
