CREATE OR REPLACE PROCEDURE DM.p_DM_NIL_CALC_KIS_PRD (
      p_REPORT_DT in date,
...
BEGIN
   dm.u_log(p_proc => 'DM.p_DM_NIL_CALC_KIS',
           p_step => 'INPUT PARAMS',
   ...
   dm.u_log(p_proc => 'DM.p_DM_NIL_CALC_KIS',
    ...
    and snapshot_cd = 'Основной КИС';
   dm.u_log(p_proc => 'DM.p_DM_NIL_CALC_KIS',
           p_step => 'insert into dm.DM_XIRR_TMP select * from dm.DM_XIRRwhere ODTTM= p_REPORT_DT and snapshot_cd = ''Основной КИС''',
   ...
   --if (v_count = 0)then
            ...
            (
                with
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил  подзапрос xirr]
--------------------------------------------------------------------------------------
                xirr as
                (
                    select /*+ MATERIALIZE */
                    contract_id
                    from dm.dm_xirr x
                    where x.snapshot_cd = 'Основной КИС'
                ...
                Contr_prev as (
                          SELECT cn1.contract_key L_key,
                                 ...
                                 cn1.valid_to_dttm,
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил  поле SUBSIDIZATION_FLG]
--------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил проверку на exists(перенес из финального скрипта)
--------------------------------------------------------------------------------------
                                  and   exists
                                  (
                                    select 1
                ...
                */
                Contr_ces as (
                        SELECT
                            ...
                            cn1.flag,
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил  поле L_key_dop_SUBSIDIZATION_FLG]
--------------------------------------------------------------------------------------
               ...
               Flow_L as
                          (
                           SELECT Contr.L_KEY,                      -- Исключение задваивания в случае, если одному контракту лизинга соответствует несколько договоров поставки (flag = 1)
                                  ...
                                  Contr.flag,
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил поля  SUBSIDIZATION_FLG, TYPE_CALC]
--------------------------------------------------------------------------------------
                                  SUBSIDIZATION_FLG,
                                  TYPE_CALC
                           from (select distinct
...
--                                                        and p_REPORT_DT BETWEEN BEGIN_DT and END_DT)))                -- выбор плановых платежей по договорам лизинга с типом классификации КБК 'Leasing'
          ...
          */
                 Flow_Cur as(
--------------------------------------------------------------------------------------
--[Update by Kolobkov 15/04/2016: Добавил изменил логику выбоки из Flow_L]
--------------------------------------------------------------------------------------
                           SELECT L_KEY,                      -- Исключение задваивания в случае, если одному контракту лизинга соответствует несколько договоров поставки (flag = 1)
                                  branch_key,
                                  CBC_DESC,
                                  PAY_DT,
                                  PAY_AMT,
                                  ...
                                  (case
                                   ...
                                   WHEN ex_rate is not Null and ex_rate <> 0 and cur1 != cur2
                                      then PAY_AMT/ex_rate/rt2.EXCHANGE_RATE
                                         ...
                                         when CUR2 = s.base_currency and CUR1 <> s.base_currency and PAY_DT > p_REPORT_DT     --Если Д_ВАЛ <> Б_ВАЛ и ФП_ВАЛ = Б_ВАЛ и ДАТА > О_ДАТА,
                                            --then PAY_AMT*round(1/rt_rp2.EXCHANGE_RATE,8)                                                --то    СУММ = СУММ / Д_КУРС_О_ДАТА
                                            then PAY_AMT/rt_rp2.EXCHANGE_RATE                                                --то    СУММ = СУММ / Д_КУРС_О_ДАТА                                          --то    СУММ = СУММ / Д_КУРС_О_ДАТА
...
*/
                     ...
                     UNDERPAY_L as (
                                  select L_fact.L_key CONTRACT_KEY,
                                        ...
                                        from
                                              (select L_key, sum(pay_amt_cur) summ_p from dm.dm_xirr_flow_orig_tmp
                                                where tp = 'LEASING'
                                                  and snapshot_cd = 'Основной КИС'
                                                  and snapshot_dt = p_REPORT_DT
                                                  and pay_dt <=p_REPORT_DT
                                              ...
                                              (select L_key, sum(pay_amt_cur) summ_f from dm.dm_xirr_flow_orig_tmp
...
--                                                  on f.l_key = x.contract_id
  ...
  dm.u_log(p_proc => 'p_DM_NIL_CALC_KIS',
           p_step => 'insert into dm_flow',
           p_info => SQL%ROWCOUNT|| ' row(s) inserted');
...
--                            )
    ...
    dm.u_log(p_proc => 'p_DM_NIL_CALC_KIS',
           p_step => 'insert into DM_NIL_TMP',
           p_info => ' row(s) inserted');
...
commit;
    ...
    dm.u_log(p_proc => 'p_DM_NIL_CALC_KIS',
           p_step => 'NIL_JOB_(p_dm_nill_chunk)',
           p_info => 'jobs compleated');
...
with SUB_NIL_AMT as (
              ...
              from dm.dm_xirr_flow_orig_tmp p
              ...
              left join
                   (select l_key as contract_key,
                     ...
                     from dm.dm_xirr_flow_orig_tmp where snapshot_dt = p_REPORT_DT
...
--[apolyakov 05.09.2016]: добавление алисасов для обратного КГП
...
where a.snapshot_cd = 'Основной КИС'
  ...
  dm.u_log(p_proc => 'p_DM_NIL_CALC_KIS',
           p_step => 'insert into dm_repayment_schedule',
           p_info => SQL%ROWCOUNT|| ' row(s) inserted');
