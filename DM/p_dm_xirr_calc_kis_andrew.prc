CREATE OR REPLACE PROCEDURE DM.p_DM_XIRR_CALC_KIS_ANDREW(
      p_REPORT_DT IN date,
...
BEGIN
   ...
   execute immediate 'drop index IX_DM_xirr_flow_orig_tmp';     -- Удаление индекса таблицы DM_XIRR_FLOW_orig Shishlyanikov
   execute immediate 'truncate table dm.dm_xirr_flow_orig_tmp'; --Shishlyanikov
   ...
   insert all into dm_xirr_flow_orig
          values(
                  l_key              ,
                  branch_key         ,
                  cbc_desc           ,
          ...
          into dm_xirr_flow_orig_tmp --Shishlyanikov
          values(
                  l_key              ,
                  branch_key         ,
                  cbc_desc           ,
...
-- Добавление потока по цессии из старых договоров
 ...
 -- Конец доп. потока
   ...
   execute immediate 'CREATE INDEX IX_DM_xirr_flow_orig_tmp ON DM.dm_xirr_flow_orig_tmp (L_KEY)';   -- создание индекса
                  ...
                  */
                      ...
                      --КИС
                                            ...
                                            (select l_key,
                                                  ...
                                                  --from dm_xirr_flow_orig Shishlyanikov
                          from dm_xirr_flow_orig_tmp
                            ...
                            Flow_Underpay as
                                            ...
                                            join ( select orig.l_key,
                                                  ...
                                                  from dm_xirr_flow_orig_tmp orig
                  ...
                  Flow_Underpay_FLG as                                                                              -- Проверка необходимости добавлять переплату (Исключения курсовой разницы)
                              ...
                              Supply_eq as (select l_key as unpaid_l_key from dm_xirr_flow_orig_tmp   ----- 26102015 Составление списка контрактов с оплаченной поставкой
                                           ...
                                           SELECT
                                            ...
                                            and FUF.FLG_UNDERPAY in ('Y')
                                             ...
                                             SELECT ----- 26102015 Выбор только тех будущих плановых платежей по поставке, которые еще не оплачены
                                                    ...
                                                    SUM(nvl(PAY_AMT_CUR, 0)) summ,
                                                    SUM(PAY_AMT) PAY_AMT
                                             FROM dm_xirr_flow_orig_tmp a, Supply_eq b
                                             WHERE snapshot_cd = 'Основной КИС'
                                             and snapshot_dt = p_REPORT_DT
                                             and branch_key in (select branch_key from dwh.cgp_group where cgp_group_key = p_group_key and cgp_group.begin_dt <= p_REPORT_DT and cgp_group.end_dt > p_REPORT_DT)
