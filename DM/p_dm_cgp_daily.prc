CREATE OR REPLACE PROCEDURE DM."P_DM_CGP_DAILY" (
    p_REPORT_DT in date
)
 ...
 with cgp as
            (
              select
                          ...
                          decode(min(CFD.ACCEPT),0,'Нет',1,'Да','Undef') as PTS_FLG,
                          min(CFD.VTBLDATE) as PTS_DT,
                          min(CFD."comment") as PTS_COMM,
                          decode(min(CFD.noneed),0,'Нет',1,'Да','Undef') as NONEED,
                          ...
                          case
                            when LC.end_dt >=  add_months(p_REPORT_DT, -12) and LC.rehiring_flg = 'Нет' --by ovilkova 01.11.2017 rehiring_flg will be deleted after Crosys testing
                              then 1
                            else 0
                          end as cr_ch_flg,
                          --lc.doc_from,
