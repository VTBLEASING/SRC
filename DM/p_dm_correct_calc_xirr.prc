CREATE OR REPLACE PROCEDURE DM.p_DM_CORRECT_CALC_XIRR(
...
--------------------------------------------------------------------------------------------------------------------------------------------
--  [asavgurov 02.11.2015] Этот кусок кода полностью взят из процедуры p_dm_xirr_calc_kis_single.
--------------------------------------------------------------------------------------------------------------------------------------------

   delete from DM_XIRR_FLOW_ORIG
   where rowid in 
   (   select a.rowid  
       from DM_XIRR_FLOW_ORIG a
            inner join DM_CORRECT_DNIL dcd
               on  l_key = dcd.contract_key
               and dcd.CORRECTION_NUMBER = p_CORRECTION_NUMBER 
        where SNAPSHOT_dt = p_report_dt               
        and snapshot_cd = 'Основной КИС'      -- Очистка таблицы DM_XIRR_FLOW_ORIG за данный отчетный период
    );        
   ...
   */
                  Contr_prev as (
                          ...
                          inner join DM_CORRECT_DNIL dcd
                              ON cn1.contract_key = dcd.contract_key
                              and dcd.CORRECTION_NUMBER = p_CORRECTION_NUMBER                                
...
-- Добавление потока по цессии из старых договоров        
 ...
 -- Конец доп. потока                             
   ...
   -- [aapolyakov 12.01.2016]: добавление трейсинга для отслеживания выпадения договоров
...
insert into TEMP_CORR_Flow_Underpay_FLG                                            
         with        Flow_Underpay_FLG as                                                                              -- Проверка необходимости добавлять переплату (Исключения курсовой разницы)
                                    ( 
                                      Select  
                                            SUM_FL.L_KEY,  
...
insert into TEMP_CORR_Supply_eq                                         
              with                Supply_eq as (select l_key as unpaid_l_key 
                                              from dm_xirr_flow_orig   ----- 26102015 Составление списка контрактов с оплаченной поставкой
                                                      inner join DM_CORRECT_DNIL dcd
                                                           on  l_key = dcd.contract_key
                                                           and dcd.CORRECTION_NUMBER = p_CORRECTION_NUMBER 
                       ...
                       */
                                            ...
                                            and FUF.FLG_UNDERPAY='Y'
                                             ...
                                             SELECT ----- 26102015 Выбор только тех будущих плановых платежей по поставке, которые еще не оплачены
                                                    ...
                                                    SUM(nvl(PAY_AMT_CUR, 0)) summ, 
                                                    SUM(PAY_AMT) PAY_AMT
                                             FROM dm_xirr_flow_orig a, TEMP_CORR_Supply_eq b,DM_CORRECT_DNIL dcd
                                             WHERE snapshot_cd = 'Основной КИС'
                                             and dcd.CORRECTION_NUMBER = p_CORRECTION_NUMBER 
                                             and l_key = dcd.contract_key
                                             and snapshot_dt = p_REPORT_DT
...
insert into TEMP_CORR_min_dt
               with              min_dt as                                                                -- дата, когда отрицательный платеж по модулю превзошел положительный баланс
                                        (
                                            select 
                                                  L_KEY,
...
--[asavgurov 03.11.2015]: Изменил логику excptn_zero_div. Сама логика определена выше, здесь просто расчет поля flag. Убрал supply_fact_sum, т.к она тут не нужна
...
insert into TEMP_CORR_fff            
                    with                       fff as   -----начало-----------изменили(только архитектуру, а не логику) для того, чтобы избежать падения процедуры при пересчете Европы    
                                        (                    
SELECT f.l_key,
  branch_key,
  PAY_DT,
...
ON f.l_key = d.l_key
--WHERE (NVL(summ, 0) != 0
--OR NVL(sum_prev, 0) != 0) -- на случай, если ни плановых ни фактических платежей нет, эти договоры мы не выключаем в поток.
  --  and d.div > 1
                                        ) 
select * from fff;                                        
...
insert into TEMP_CORR_FINALL
        ...
        SELECT l_key
              ...
              , pay_dt
              , summ
              , case when pay_dt = dcl.CORRECT_DT and summ = 0 then -dcl.dnil else 0 end as sum_prev
              , excptn_zero_div
              , report_dt
              , flag
              , 2 as NUM_UNION
              , ZERO_DIV_FLG 
  from  TEMP_CORR_fff b 
    inner join DM_CORRECT_DNIL dcl
        on  b.l_key = dcl.contract_key
        and b.pay_dt >= dcl.CORRECT_DT
        and dcl.CORRECTION_NUMBER = p_CORRECTION_NUMBER
  union all
  select dcl.contract_key as l_key
       , cont.branch_key
       , dcl.CORRECT_DT as pay_dt
       , -dnil as summ
       , 0 as sum_prev
       ...
       , 1 as NUM_UNION
       , -11 as ZERO_DIV_FLG
       from DM_CORRECT_DNIL dcl
            inner join dwh.contracts cont
                on cont.contract_key = dcl.contract_key
                and cont.valid_to_dttm = to_date('01.01.2400', 'dd.mm.yyyy')
        where dcl.CORRECTION_NUMBER = p_CORRECTION_NUMBER                 
                                         )
select * from FINALL;
...
--[asavgurov 03.11.2015]: Изменил логику excptn_zero_div. Внизу по умолчанию ставится 1.
       , case when sum(summ) over (partition by l_key, branch_key, report_dt, flag) > 0 then 1 else 0 end as excptn_zero_div
       , report_dt
       , flag
       , NUM_UNION
       , ZERO_DIV_FLG
from TEMP_CORR_FINALL;   --------------------------------------конец-----------изменили для того, чтобы избежать падения процедуры при пересчете Европы
...
-- расчет xIRR
   ...
   and CONTRACT_ID in (select CONTRACT_KEY from DM_CORRECT_DNIL where CORRECTION_NUMBER = p_CORRECTION_NUMBER ) 
