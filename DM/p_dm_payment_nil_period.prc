CREATE OR REPLACE PROCEDURE DM.P_DM_PAYMENT_NIL_PERIOD
(
P_REPORT_ID number
)
IS
...
(
    select *
    from dm.dm_cgp cgp
    where cgp.snapshot_cd = 'Основной КИС'
...
,dates_add as
(
    select add_months(to_date ('30.04.2016', 'dd.mm.yyyy'), level - 1) as REP_DT from dual connect by level < 350
)
...
(
    ...
    where snapshot_dt = v_snapshot_dt
      and pay_dt >= v_snapshot_dt
      and pay_dt >= nvl (v_pay_start_dt, to_date('01.01.2000', 'dd.mm.yyyy'))
      and pay_dt <= nvl (v_pay_end_dt, to_date ('31.12.3999', 'dd.mm.yyyy'))
    group by contract_key
)

,tt as
(
select REP_DT + 1 pay_dt
,t3.BRANCH_NAM
,cgp.contract_id_cd
,cgp.snapshot_dt
,0   val
...
from  cgp_client cgp
inner join m_dt max_dt on cgp.contract_key = max_dt.contract_key
inner join dates_add dadd on dadd.REP_DT <= max_dt.max_dt
left join dwh.leasing_contracts lc on cgp.contract_key = lc.contract_key and lc.valid_to_dttm = to_date ('01.01.2400', 'dd.mm.yyyy')
left join dwh.currencies cur on cgp.currency_key = cur.currency_key and cur. valid_to_dttm = to_date ('01.01.2400', 'dd.mm.yyyy') and cur.begin_dt <= cgp.snapshot_dt and cur.end_dt > cgp.snapshot_dt
--left join dwh.contracts t2 on t2.valid_to_dttm>sysdate+100 and t2.contract_key=t.contract_key
left join dwh.org_structure t3 on t3.branch_key = cgp.BRANCH_KEY and t3.VALID_TO_DTTM > sysdate+100
where 1=1
and cgp.snapshot_dt = v_snapshot_dt
and cgp.term_amt != 0
--------------------------------------------------------------------------------------
...
,round(
        case when v_snapshot_cd in ('PAYMENT_RUB_MONTH', 'PAYMENT_RUB_QUARTER') then to_number(sum(t.leasing_pay_amt*t4.EXCHANGE_RATE))
            ...
            -- [apolyakov 17:10:2016] -- чтоб отбор был как в КГП с учетом субсидирования
            when v_snapshot_cd in ('NIL_RUB_MONTH', 'NIL_RUB_QUARTER')
                    then to_number(sum(
                                        case
                                            when t.nil_amt > 0
            ...
            when v_snapshot_cd in ('NIL_ORIG_MONTH', 'NIL_ORIG_QUARTER')
                    then to_number(sum(
                                        case
                                            when t.nil_amt > 0
                                             and lc.SUBSIDIZATION_FLG = 1
...
left join dm.dm_xirr xirr on t.contract_key = xirr.contract_id and xirr.snapshot_cd = 'Основной КИС' and xirr.odttm = t.snapshot_dt
left JOIN dm.DM_MAX_DT max_dt on t.contract_key = max_dt.contract_id and max_dt.oddtm = xirr.odttm
left join cgp_client cgp on t.contract_key = cgp.contract_key and t.snapshot_dt = cgp.snapshot_dt and cgp.snapshot_cd = 'Основной КИС'
...
and cur.currency_key = nvl (v_currency_key, cur.currency_key)
and case when nvl (lc.auto_flg, 0) != 0 then 'AUTOLEASING' else 'CORPORATIVE' end = nvl (v_auto_flg, case when nvl (lc.auto_flg, 0) != 0 then 'AUTOLEASING' else 'CORPORATIVE' end)
and add_months (trunc(t.pay_dt, 'MONTH'), 1) >= nvl (v_pay_start_dt, to_date('01.01.2000', 'dd.mm.yyyy'))
and add_months (trunc(t.pay_dt, 'MONTH'), 1) <= nvl (v_pay_end_dt, to_date ('31.12.3999', 'dd.mm.yyyy'))
--------------------------------------------------------------------------------------
group by
...
(
    ...
    ,contract_id_cd
    ,snapshot_dt,
          case when v_snapshot_cd in ('PAYMENT_RUB_MONTH', 'PAYMENT_RUB_QUARTER', 'PAYMENT_ORIG_MONTH', 'PAYMENT_ORIG_QUARTER')
                    then val
               when v_snapshot_cd in ('NIL_RUB_MONTH', 'NIL_RUB_QUARTER', 'NIL_ORIG_MONTH', 'NIL_ORIG_QUARTER')
                    then sum (val) over (partition by contract_id_cd order by pay_dt asc rows between 1 following and unbounded following)
    end as val
    ,CLIENT_NAM
    ,auto_flg
    ,currency_letter_cd
...
case when to_char(f1,'SS') = 2 then 'Квартал '||to_char(f1,'Q')
     when to_char(f1,'SS') = 3 then 'Год '||to_char(f1,'YYYY')
     else to_char(f1,'DD.MM.YYYY')
        end f1,
case when to_char(f2,'SS') = 2 then 'Квартал '||to_char(f2,'Q')
     when to_char(f2,'SS') = 3 then 'Год '||to_char(f2,'YYYY')
     else to_char(f2,'DD.MM.YYYY')
        end f2,
case when to_char(f3,'SS') = 2 then 'Квартал '||to_char(f3,'Q')
     when to_char(f3,'SS') = 3 then 'Год '||to_char(f3,'YYYY')
     else to_char(f3,'DD.MM.YYYY')
        end f3,
case when to_char(f4,'SS') = 2 then 'Квартал '||to_char(f4,'Q')
     when to_char(f4,'SS') = 3 then 'Год '||to_char(f4,'YYYY')
     else to_char(f4,'DD.MM.YYYY')
        end f4,
case when to_char(f5,'SS') = 2 then 'Квартал '||to_char(f5,'Q')
     when to_char(f5,'SS') = 3 then 'Год '||to_char(f5,'YYYY')
     else to_char(f5,'DD.MM.YYYY')
        end f5,
case when to_char(f6,'SS') = 2 then 'Квартал '||to_char(f6,'Q')
     when to_char(f6,'SS') = 3 then 'Год '||to_char(f6,'YYYY')
     else to_char(f6,'DD.MM.YYYY')
        end f6,
case when to_char(f7,'SS') = 2 then 'Квартал '||to_char(f7,'Q')
     when to_char(f7,'SS') = 3 then 'Год '||to_char(f7,'YYYY')
     else to_char(f7,'DD.MM.YYYY')
        end f7,
case when to_char(f8,'SS') = 2 then 'Квартал '||to_char(f8,'Q')
     when to_char(f8,'SS') = 3 then 'Год '||to_char(f8,'YYYY')
     else to_char(f8,'DD.MM.YYYY')
        end f8,
case when to_char(f9,'SS') = 2 then 'Квартал '||to_char(f9,'Q')
     when to_char(f9,'SS') = 3 then 'Год '||to_char(f9,'YYYY')
     else to_char(f9,'DD.MM.YYYY')
        end f9,
case when to_char(f10,'SS') = 2 then 'Квартал '||to_char(f10,'Q')
     when to_char(f10,'SS') = 3 then 'Год '||to_char(f10,'YYYY')
     else to_char(f10,'DD.MM.YYYY')
        end f10,
case when to_char(f11,'SS') = 2 then 'Квартал '||to_char(f11,'Q')
     when to_char(f11,'SS') = 3 then 'Год '||to_char(f11,'YYYY')
     else to_char(f11,'DD.MM.YYYY')
        end f11,
case when to_char(f12,'SS') = 2 then 'Квартал '||to_char(f12,'Q')
     when to_char(f12,'SS') = 3 then 'Год '||to_char(f12,'YYYY')
     else to_char(f12,'DD.MM.YYYY')
        end f12,
case when to_char(f13,'SS') = 2 then 'Квартал '||to_char(f13,'Q')
     when to_char(f13,'SS') = 3 then 'Год '||to_char(f13,'YYYY')
     else to_char(f13,'DD.MM.YYYY')
        end f13,
case when to_char(f14,'SS') = 2 then 'Квартал '||to_char(f14,'Q')
     when to_char(f14,'SS') = 3 then 'Год '||to_char(f14,'YYYY')
     else to_char(f14,'DD.MM.YYYY')
        end f14,
case when to_char(f15,'SS') = 2 then 'Квартал '||to_char(f15,'Q')
     when to_char(f15,'SS') = 3 then 'Год '||to_char(f15,'YYYY')
     else to_char(f15,'DD.MM.YYYY')
        end f15,
case when to_char(f16,'SS') = 2 then 'Квартал '||to_char(f16,'Q')
     when to_char(f16,'SS') = 3 then 'Год '||to_char(f16,'YYYY')
     else to_char(f16,'DD.MM.YYYY')
        end f16,
case when to_char(f17,'SS') = 2 then 'Квартал '||to_char(f17,'Q')
     when to_char(f17,'SS') = 3 then 'Год '||to_char(f17,'YYYY')
     else to_char(f17,'DD.MM.YYYY')
        end f17,
case when to_char(f18,'SS') = 2 then 'Квартал '||to_char(f18,'Q')
     when to_char(f18,'SS') = 3 then 'Год '||to_char(f18,'YYYY')
     else to_char(f18,'DD.MM.YYYY')
        end f18,
case when to_char(f19,'SS') = 2 then 'Квартал '||to_char(f19,'Q')
     when to_char(f19,'SS') = 3 then 'Год '||to_char(f19,'YYYY')
     else to_char(f19,'DD.MM.YYYY')
        end f19,
case when to_char(f20,'SS') = 2 then 'Квартал '||to_char(f20,'Q')
     when to_char(f20,'SS') = 3 then 'Год '||to_char(f20,'YYYY')
     else to_char(f20,'DD.MM.YYYY')
        end f20,
case when to_char(f21,'SS') = 2 then 'Квартал '||to_char(f21,'Q')
     when to_char(f21,'SS') = 3 then 'Год '||to_char(f21,'YYYY')
     else to_char(f21,'DD.MM.YYYY')
        end f21,
case when to_char(f22,'SS') = 2 then 'Квартал '||to_char(f22,'Q')
     when to_char(f22,'SS') = 3 then 'Год '||to_char(f22,'YYYY')
     else to_char(f22,'DD.MM.YYYY')
        end f22,
case when to_char(f23,'SS') = 2 then 'Квартал '||to_char(f23,'Q')
     when to_char(f23,'SS') = 3 then 'Год '||to_char(f23,'YYYY')
     else to_char(f23,'DD.MM.YYYY')
        end f23,
case when to_char(f24,'SS') = 2 then 'Квартал '||to_char(f24,'Q')
     when to_char(f24,'SS') = 3 then 'Год '||to_char(f24,'YYYY')
     else to_char(f24,'DD.MM.YYYY')
        end f24,
case when to_char(f25,'SS') = 2 then 'Квартал '||to_char(f25,'Q')
     when to_char(f25,'SS') = 3 then 'Год '||to_char(f25,'YYYY')
     else to_char(f25,'DD.MM.YYYY')
        end f25,
case when to_char(f26,'SS') = 2 then 'Квартал '||to_char(f26,'Q')
     when to_char(f26,'SS') = 3 then 'Год '||to_char(f26,'YYYY')
     else to_char(f26,'DD.MM.YYYY')
        end f26,
case when to_char(f27,'SS') = 2 then 'Квартал '||to_char(f27,'Q')
     when to_char(f27,'SS') = 3 then 'Год '||to_char(f27,'YYYY')
     else to_char(f27,'DD.MM.YYYY')
        end f27,
case when to_char(f28,'SS') = 2 then 'Квартал '||to_char(f28,'Q')
     when to_char(f28,'SS') = 3 then 'Год '||to_char(f28,'YYYY')
     else to_char(f28,'DD.MM.YYYY')
        end f28,
case when to_char(f29,'SS') = 2 then 'Квартал '||to_char(f29,'Q')
     when to_char(f29,'SS') = 3 then 'Год '||to_char(f29,'YYYY')
     else to_char(f29,'DD.MM.YYYY')
        end f29,
case when to_char(f30,'SS') = 2 then 'Квартал '||to_char(f30,'Q')
     when to_char(f30,'SS') = 3 then 'Год '||to_char(f30,'YYYY')
     else to_char(f30,'DD.MM.YYYY')
        end f30,
case when to_char(f31,'SS') = 2 then 'Квартал '||to_char(f31,'Q')
     when to_char(f31,'SS') = 3 then 'Год '||to_char(f31,'YYYY')
     else to_char(f31,'DD.MM.YYYY')
        end f31,
case when to_char(f32,'SS') = 2 then 'Квартал '||to_char(f32,'Q')
     when to_char(f32,'SS') = 3 then 'Год '||to_char(f32,'YYYY')
     else to_char(f32,'DD.MM.YYYY')
        end f32,
case when to_char(f33,'SS') = 2 then 'Квартал '||to_char(f33,'Q')
     when to_char(f33,'SS') = 3 then 'Год '||to_char(f33,'YYYY')
     else to_char(f33,'DD.MM.YYYY')
        end f33,
case when to_char(f34,'SS') = 2 then 'Квартал '||to_char(f34,'Q')
     when to_char(f34,'SS') = 3 then 'Год '||to_char(f34,'YYYY')
     else to_char(f34,'DD.MM.YYYY')
        end f34,
case when to_char(f35,'SS') = 2 then 'Квартал '||to_char(f35,'Q')
     when to_char(f35,'SS') = 3 then 'Год '||to_char(f35,'YYYY')
     else to_char(f35,'DD.MM.YYYY')
        end f35,
case when to_char(f36,'SS') = 2 then 'Квартал '||to_char(f36,'Q')
     when to_char(f36,'SS') = 3 then 'Год '||to_char(f36,'YYYY')
     else to_char(f36,'DD.MM.YYYY')
        end f36,
case when to_char(f37,'SS') = 2 then 'Квартал '||to_char(f37,'Q')
     when to_char(f37,'SS') = 3 then 'Год '||to_char(f37,'YYYY')
     else to_char(f37,'DD.MM.YYYY')
        end f37,
case when to_char(f38,'SS') = 2 then 'Квартал '||to_char(f38,'Q')
     when to_char(f38,'SS') = 3 then 'Год '||to_char(f38,'YYYY')
     else to_char(f38,'DD.MM.YYYY')
        end f38,
case when to_char(f39,'SS') = 2 then 'Квартал '||to_char(f39,'Q')
     when to_char(f39,'SS') = 3 then 'Год '||to_char(f39,'YYYY')
     else to_char(f39,'DD.MM.YYYY')
        end f39,
case when to_char(f40,'SS') = 2 then 'Квартал '||to_char(f40,'Q')
     when to_char(f40,'SS') = 3 then 'Год '||to_char(f40,'YYYY')
     else to_char(f40,'DD.MM.YYYY')
        end f40,
case when to_char(f41,'SS') = 2 then 'Квартал '||to_char(f41,'Q')
     when to_char(f41,'SS') = 3 then 'Год '||to_char(f41,'YYYY')
     else to_char(f41,'DD.MM.YYYY')
        end f41,
case when to_char(f42,'SS') = 2 then 'Квартал '||to_char(f42,'Q')
     when to_char(f42,'SS') = 3 then 'Год '||to_char(f42,'YYYY')
     else to_char(f42,'DD.MM.YYYY')
        end f42,
case when to_char(f43,'SS') = 2 then 'Квартал '||to_char(f43,'Q')
     when to_char(f43,'SS') = 3 then 'Год '||to_char(f43,'YYYY')
     else to_char(f43,'DD.MM.YYYY')
        end f43,
case when to_char(f44,'SS') = 2 then 'Квартал '||to_char(f44,'Q')
     when to_char(f44,'SS') = 3 then 'Год '||to_char(f44,'YYYY')
     else to_char(f44,'DD.MM.YYYY')
        end f44,
case when to_char(f45,'SS') = 2 then 'Квартал '||to_char(f45,'Q')
     when to_char(f45,'SS') = 3 then 'Год '||to_char(f45,'YYYY')
     else to_char(f45,'DD.MM.YYYY')
        end f45,
case when to_char(f46,'SS') = 2 then 'Квартал '||to_char(f46,'Q')
     when to_char(f46,'SS') = 3 then 'Год '||to_char(f46,'YYYY')
     else to_char(f46,'DD.MM.YYYY')
        end f46,
case when to_char(f47,'SS') = 2 then 'Квартал '||to_char(f47,'Q')
     when to_char(f47,'SS') = 3 then 'Год '||to_char(f47,'YYYY')
     else to_char(f47,'DD.MM.YYYY')
        end f47,
case when to_char(f48,'SS') = 2 then 'Квартал '||to_char(f48,'Q')
     when to_char(f48,'SS') = 3 then 'Год '||to_char(f48,'YYYY')
     else to_char(f48,'DD.MM.YYYY')
        end f48,
case when to_char(f49,'SS') = 2 then 'Квартал '||to_char(f49,'Q')
     when to_char(f49,'SS') = 3 then 'Год '||to_char(f49,'YYYY')
     else to_char(f49,'DD.MM.YYYY')
        end f49,
case when to_char(f50,'SS') = 2 then 'Квартал '||to_char(f50,'Q')
     when to_char(f50,'SS') = 3 then 'Год '||to_char(f50,'YYYY')
     else to_char(f50,'DD.MM.YYYY')
        end f50,
case when to_char(f51,'SS') = 2 then 'Квартал '||to_char(f51,'Q')
     when to_char(f51,'SS') = 3 then 'Год '||to_char(f51,'YYYY')
     else to_char(f51,'DD.MM.YYYY')
        end f51,
case when to_char(f52,'SS') = 2 then 'Квартал '||to_char(f52,'Q')
     when to_char(f52,'SS') = 3 then 'Год '||to_char(f52,'YYYY')
     else to_char(f52,'DD.MM.YYYY')
        end f52,
case when to_char(f53,'SS') = 2 then 'Квартал '||to_char(f53,'Q')
     when to_char(f53,'SS') = 3 then 'Год '||to_char(f53,'YYYY')
     else to_char(f53,'DD.MM.YYYY')
        end f53,
case when to_char(f54,'SS') = 2 then 'Квартал '||to_char(f54,'Q')
     when to_char(f54,'SS') = 3 then 'Год '||to_char(f54,'YYYY')
     else to_char(f54,'DD.MM.YYYY')
        end f54,
case when to_char(f55,'SS') = 2 then 'Квартал '||to_char(f55,'Q')
     when to_char(f55,'SS') = 3 then 'Год '||to_char(f55,'YYYY')
     else to_char(f55,'DD.MM.YYYY')
        end f55,
case when to_char(f56,'SS') = 2 then 'Квартал '||to_char(f56,'Q')
     when to_char(f56,'SS') = 3 then 'Год '||to_char(f56,'YYYY')
     else to_char(f56,'DD.MM.YYYY')
        end f56,
case when to_char(f57,'SS') = 2 then 'Квартал '||to_char(f57,'Q')
     when to_char(f57,'SS') = 3 then 'Год '||to_char(f57,'YYYY')
     else to_char(f57,'DD.MM.YYYY')
        end f57,
case when to_char(f58,'SS') = 2 then 'Квартал '||to_char(f58,'Q')
     when to_char(f58,'SS') = 3 then 'Год '||to_char(f58,'YYYY')
     else to_char(f58,'DD.MM.YYYY')
        end f58,
case when to_char(f59,'SS') = 2 then 'Квартал '||to_char(f59,'Q')
     when to_char(f59,'SS') = 3 then 'Год '||to_char(f59,'YYYY')
     else to_char(f59,'DD.MM.YYYY')
        end f59,
case when to_char(f60,'SS') = 2 then 'Квартал '||to_char(f60,'Q')
     when to_char(f60,'SS') = 3 then 'Год '||to_char(f60,'YYYY')
     else to_char(f60,'DD.MM.YYYY')
        end f60,
case when to_char(f61,'SS') = 2 then 'Квартал '||to_char(f61,'Q')
     when to_char(f61,'SS') = 3 then 'Год '||to_char(f61,'YYYY')
     else to_char(f61,'DD.MM.YYYY')
        end f61,
case when to_char(f62,'SS') = 2 then 'Квартал '||to_char(f62,'Q')
     when to_char(f62,'SS') = 3 then 'Год '||to_char(f62,'YYYY')
     else to_char(f62,'DD.MM.YYYY')
        end f62,
case when to_char(f63,'SS') = 2 then 'Квартал '||to_char(f63,'Q')
     when to_char(f63,'SS') = 3 then 'Год '||to_char(f63,'YYYY')
     else to_char(f63,'DD.MM.YYYY')
        end f63,
case when to_char(f64,'SS') = 2 then 'Квартал '||to_char(f64,'Q')
     when to_char(f64,'SS') = 3 then 'Год '||to_char(f64,'YYYY')
     else to_char(f64,'DD.MM.YYYY')
        end f64,
case when to_char(f65,'SS') = 2 then 'Квартал '||to_char(f65,'Q')
     when to_char(f65,'SS') = 3 then 'Год '||to_char(f65,'YYYY')
     else to_char(f65,'DD.MM.YYYY')
        end f65,
case when to_char(f66,'SS') = 2 then 'Квартал '||to_char(f66,'Q')
     when to_char(f66,'SS') = 3 then 'Год '||to_char(f66,'YYYY')
     else to_char(f66,'DD.MM.YYYY')
        end f66,
case when to_char(f67,'SS') = 2 then 'Квартал '||to_char(f67,'Q')
     when to_char(f67,'SS') = 3 then 'Год '||to_char(f67,'YYYY')
     else to_char(f67,'DD.MM.YYYY')
        end f67,
case when to_char(f68,'SS') = 2 then 'Квартал '||to_char(f68,'Q')
     when to_char(f68,'SS') = 3 then 'Год '||to_char(f68,'YYYY')
     else to_char(f68,'DD.MM.YYYY')
        end f68,
case when to_char(f69,'SS') = 2 then 'Квартал '||to_char(f69,'Q')
     when to_char(f69,'SS') = 3 then 'Год '||to_char(f69,'YYYY')
     else to_char(f69,'DD.MM.YYYY')
        end f69,
case when to_char(f70,'SS') = 2 then 'Квартал '||to_char(f70,'Q')
     when to_char(f70,'SS') = 3 then 'Год '||to_char(f70,'YYYY')
     else to_char(f70,'DD.MM.YYYY')
        end f70,
case when to_char(f71,'SS') = 2 then 'Квартал '||to_char(f71,'Q')
     when to_char(f71,'SS') = 3 then 'Год '||to_char(f71,'YYYY')
     else to_char(f71,'DD.MM.YYYY')
        end f71,
case when to_char(f72,'SS') = 2 then 'Квартал '||to_char(f72,'Q')
     when to_char(f72,'SS') = 3 then 'Год '||to_char(f72,'YYYY')
     else to_char(f72,'DD.MM.YYYY')
        end f72,
case when to_char(f73,'SS') = 2 then 'Квартал '||to_char(f73,'Q')
     when to_char(f73,'SS') = 3 then 'Год '||to_char(f73,'YYYY')
     else to_char(f73,'DD.MM.YYYY')
        end f73,
case when to_char(f74,'SS') = 2 then 'Квартал '||to_char(f74,'Q')
     when to_char(f74,'SS') = 3 then 'Год '||to_char(f74,'YYYY')
     else to_char(f74,'DD.MM.YYYY')
        end f74,
case when to_char(f75,'SS') = 2 then 'Квартал '||to_char(f75,'Q')
     when to_char(f75,'SS') = 3 then 'Год '||to_char(f75,'YYYY')
     else to_char(f75,'DD.MM.YYYY')
        end f75,
case when to_char(f76,'SS') = 2 then 'Квартал '||to_char(f76,'Q')
     when to_char(f76,'SS') = 3 then 'Год '||to_char(f76,'YYYY')
     else to_char(f76,'DD.MM.YYYY')
        end f76,
case when to_char(f77,'SS') = 2 then 'Квартал '||to_char(f77,'Q')
     when to_char(f77,'SS') = 3 then 'Год '||to_char(f77,'YYYY')
     else to_char(f77,'DD.MM.YYYY')
        end f77,
case when to_char(f78,'SS') = 2 then 'Квартал '||to_char(f78,'Q')
     when to_char(f78,'SS') = 3 then 'Год '||to_char(f78,'YYYY')
     else to_char(f78,'DD.MM.YYYY')
        end f78,
case when to_char(f79,'SS') = 2 then 'Квартал '||to_char(f79,'Q')
     when to_char(f79,'SS') = 3 then 'Год '||to_char(f79,'YYYY')
     else to_char(f79,'DD.MM.YYYY')
        end f79,
case when to_char(f80,'SS') = 2 then 'Квартал '||to_char(f80,'Q')
     when to_char(f80,'SS') = 3 then 'Год '||to_char(f80,'YYYY')
     else to_char(f80,'DD.MM.YYYY')
        end f80,
case when to_char(f81,'SS') = 2 then 'Квартал '||to_char(f81,'Q')
     when to_char(f81,'SS') = 3 then 'Год '||to_char(f81,'YYYY')
     else to_char(f81,'DD.MM.YYYY')
        end f81,
case when to_char(f82,'SS') = 2 then 'Квартал '||to_char(f82,'Q')
     when to_char(f82,'SS') = 3 then 'Год '||to_char(f82,'YYYY')
     else to_char(f82,'DD.MM.YYYY')
        end f82,
case when to_char(f83,'SS') = 2 then 'Квартал '||to_char(f83,'Q')
     when to_char(f83,'SS') = 3 then 'Год '||to_char(f83,'YYYY')
     else to_char(f83,'DD.MM.YYYY')
        end f83,
case when to_char(f84,'SS') = 2 then 'Квартал '||to_char(f84,'Q')
     when to_char(f84,'SS') = 3 then 'Год '||to_char(f84,'YYYY')
     else to_char(f84,'DD.MM.YYYY')
        end f84,
case when to_char(f85,'SS') = 2 then 'Квартал '||to_char(f85,'Q')
     when to_char(f85,'SS') = 3 then 'Год '||to_char(f85,'YYYY')
     else to_char(f85,'DD.MM.YYYY')
        end f85,
case when to_char(f86,'SS') = 2 then 'Квартал '||to_char(f86,'Q')
     when to_char(f86,'SS') = 3 then 'Год '||to_char(f86,'YYYY')
     else to_char(f86,'DD.MM.YYYY')
        end f86,
case when to_char(f87,'SS') = 2 then 'Квартал '||to_char(f87,'Q')
     when to_char(f87,'SS') = 3 then 'Год '||to_char(f87,'YYYY')
     else to_char(f87,'DD.MM.YYYY')
        end f87,
case when to_char(f88,'SS') = 2 then 'Квартал '||to_char(f88,'Q')
     when to_char(f88,'SS') = 3 then 'Год '||to_char(f88,'YYYY')
     else to_char(f88,'DD.MM.YYYY')
        end f88,
case when to_char(f89,'SS') = 2 then 'Квартал '||to_char(f89,'Q')
     when to_char(f89,'SS') = 3 then 'Год '||to_char(f89,'YYYY')
     else to_char(f89,'DD.MM.YYYY')
        end f89,
case when to_char(f90,'SS') = 2 then 'Квартал '||to_char(f90,'Q')
     when to_char(f90,'SS') = 3 then 'Год '||to_char(f90,'YYYY')
     else to_char(f90,'DD.MM.YYYY')
        end f90,
case when to_char(f91,'SS') = 2 then 'Квартал '||to_char(f91,'Q')
     when to_char(f91,'SS') = 3 then 'Год '||to_char(f91,'YYYY')
     else to_char(f91,'DD.MM.YYYY')
        end f91,
case when to_char(f92,'SS') = 2 then 'Квартал '||to_char(f92,'Q')
     when to_char(f92,'SS') = 3 then 'Год '||to_char(f92,'YYYY')
     else to_char(f92,'DD.MM.YYYY')
        end f92,
case when to_char(f93,'SS') = 2 then 'Квартал '||to_char(f93,'Q')
     when to_char(f93,'SS') = 3 then 'Год '||to_char(f93,'YYYY')
     else to_char(f93,'DD.MM.YYYY')
        end f93,
case when to_char(f94,'SS') = 2 then 'Квартал '||to_char(f94,'Q')
     when to_char(f94,'SS') = 3 then 'Год '||to_char(f94,'YYYY')
     else to_char(f94,'DD.MM.YYYY')
        end f94,
case when to_char(f95,'SS') = 2 then 'Квартал '||to_char(f95,'Q')
     when to_char(f95,'SS') = 3 then 'Год '||to_char(f95,'YYYY')
     else to_char(f95,'DD.MM.YYYY')
        end f95,
case when to_char(f96,'SS') = 2 then 'Квартал '||to_char(f96,'Q')
     when to_char(f96,'SS') = 3 then 'Год '||to_char(f96,'YYYY')
     else to_char(f96,'DD.MM.YYYY')
        end f96,
case when to_char(f97,'SS') = 2 then 'Квартал '||to_char(f97,'Q')
     when to_char(f97,'SS') = 3 then 'Год '||to_char(f97,'YYYY')
     else to_char(f97,'DD.MM.YYYY')
        end f97,
case when to_char(f98,'SS') = 2 then 'Квартал '||to_char(f98,'Q')
     when to_char(f98,'SS') = 3 then 'Год '||to_char(f98,'YYYY')
     else to_char(f98,'DD.MM.YYYY')
        end f98,
case when to_char(f99,'SS') = 2 then 'Квартал '||to_char(f99,'Q')
     when to_char(f99,'SS') = 3 then 'Год '||to_char(f99,'YYYY')
     else to_char(f99,'DD.MM.YYYY')
        end f99,
case when to_char(f100,'SS') = 2 then 'Квартал '||to_char(f100,'Q')
     when to_char(f100,'SS') = 3 then 'Год '||to_char(f100,'YYYY')
     else to_char(f100,'DD.MM.YYYY')
        end f100,
case when to_char(f101,'SS') = 2 then 'Квартал '||to_char(f101,'Q')
     when to_char(f101,'SS') = 3 then 'Год '||to_char(f101,'YYYY')
     else to_char(f101,'DD.MM.YYYY')
        end f101,
case when to_char(f102,'SS') = 2 then 'Квартал '||to_char(f102,'Q')
     when to_char(f102,'SS') = 3 then 'Год '||to_char(f102,'YYYY')
     else to_char(f102,'DD.MM.YYYY')
        end f102,
case when to_char(f103,'SS') = 2 then 'Квартал '||to_char(f103,'Q')
     when to_char(f103,'SS') = 3 then 'Год '||to_char(f103,'YYYY')
     else to_char(f103,'DD.MM.YYYY')
        end f103,
case when to_char(f104,'SS') = 2 then 'Квартал '||to_char(f104,'Q')
     when to_char(f104,'SS') = 3 then 'Год '||to_char(f104,'YYYY')
     else to_char(f104,'DD.MM.YYYY')
        end f104,
case when to_char(f105,'SS') = 2 then 'Квартал '||to_char(f105,'Q')
     when to_char(f105,'SS') = 3 then 'Год '||to_char(f105,'YYYY')
     else to_char(f105,'DD.MM.YYYY')
        end f105,
case when to_char(f106,'SS') = 2 then 'Квартал '||to_char(f106,'Q')
     when to_char(f106,'SS') = 3 then 'Год '||to_char(f106,'YYYY')
     else to_char(f106,'DD.MM.YYYY')
        end f106,
case when to_char(f107,'SS') = 2 then 'Квартал '||to_char(f107,'Q')
     when to_char(f107,'SS') = 3 then 'Год '||to_char(f107,'YYYY')
     else to_char(f107,'DD.MM.YYYY')
        end f107,
case when to_char(f108,'SS') = 2 then 'Квартал '||to_char(f108,'Q')
     when to_char(f108,'SS') = 3 then 'Год '||to_char(f108,'YYYY')
     else to_char(f108,'DD.MM.YYYY')
        end f108,
case when to_char(f109,'SS') = 2 then 'Квартал '||to_char(f109,'Q')
     when to_char(f109,'SS') = 3 then 'Год '||to_char(f109,'YYYY')
     else to_char(f109,'DD.MM.YYYY')
        end f109,
case when to_char(f110,'SS') = 2 then 'Квартал '||to_char(f110,'Q')
     when to_char(f110,'SS') = 3 then 'Год '||to_char(f110,'YYYY')
     else to_char(f110,'DD.MM.YYYY')
        end f110,
case when to_char(f111,'SS') = 2 then 'Квартал '||to_char(f111,'Q')
     when to_char(f111,'SS') = 3 then 'Год '||to_char(f111,'YYYY')
     else to_char(f111,'DD.MM.YYYY')
        end f111,
case when to_char(f112,'SS') = 2 then 'Квартал '||to_char(f112,'Q')
     when to_char(f112,'SS') = 3 then 'Год '||to_char(f112,'YYYY')
     else to_char(f112,'DD.MM.YYYY')
        end f112,
case when to_char(f113,'SS') = 2 then 'Квартал '||to_char(f113,'Q')
     when to_char(f113,'SS') = 3 then 'Год '||to_char(f113,'YYYY')
     else to_char(f113,'DD.MM.YYYY')
        end f113,
case when to_char(f114,'SS') = 2 then 'Квартал '||to_char(f114,'Q')
     when to_char(f114,'SS') = 3 then 'Год '||to_char(f114,'YYYY')
     else to_char(f114,'DD.MM.YYYY')
        end f114,
case when to_char(f115,'SS') = 2 then 'Квартал '||to_char(f115,'Q')
     when to_char(f115,'SS') = 3 then 'Год '||to_char(f115,'YYYY')
     else to_char(f115,'DD.MM.YYYY')
        end f115,
case when to_char(f116,'SS') = 2 then 'Квартал '||to_char(f116,'Q')
     when to_char(f116,'SS') = 3 then 'Год '||to_char(f116,'YYYY')
     else to_char(f116,'DD.MM.YYYY')
        end f116,
case when to_char(f117,'SS') = 2 then 'Квартал '||to_char(f117,'Q')
     when to_char(f117,'SS') = 3 then 'Год '||to_char(f117,'YYYY')
     else to_char(f117,'DD.MM.YYYY')
        end f117,
case when to_char(f118,'SS') = 2 then 'Квартал '||to_char(f118,'Q')
     when to_char(f118,'SS') = 3 then 'Год '||to_char(f118,'YYYY')
     else to_char(f118,'DD.MM.YYYY')
        end f118,
case when to_char(f119,'SS') = 2 then 'Квартал '||to_char(f119,'Q')
     when to_char(f119,'SS') = 3 then 'Год '||to_char(f119,'YYYY')
     else to_char(f119,'DD.MM.YYYY')
        end f119,
case when to_char(f120,'SS') = 2 then 'Квартал '||to_char(f120,'Q')
     when to_char(f120,'SS') = 3 then 'Год '||to_char(f120,'YYYY')
     else to_char(f120,'DD.MM.YYYY')
        end f120,
case when to_char(f121,'SS') = 2 then 'Квартал '||to_char(f121,'Q')
     when to_char(f121,'SS') = 3 then 'Год '||to_char(f121,'YYYY')
     else to_char(f121,'DD.MM.YYYY')
        end f121,
case when to_char(f122,'SS') = 2 then 'Квартал '||to_char(f122,'Q')
     when to_char(f122,'SS') = 3 then 'Год '||to_char(f122,'YYYY')
     else to_char(f122,'DD.MM.YYYY')
        end f122,
case when to_char(f123,'SS') = 2 then 'Квартал '||to_char(f123,'Q')
     when to_char(f123,'SS') = 3 then 'Год '||to_char(f123,'YYYY')
     else to_char(f123,'DD.MM.YYYY')
        end f123,
case when to_char(f124,'SS') = 2 then 'Квартал '||to_char(f124,'Q')
     when to_char(f124,'SS') = 3 then 'Год '||to_char(f124,'YYYY')
     else to_char(f124,'DD.MM.YYYY')
        end f124,
case when to_char(f125,'SS') = 2 then 'Квартал '||to_char(f125,'Q')
     when to_char(f125,'SS') = 3 then 'Год '||to_char(f125,'YYYY')
     else to_char(f125,'DD.MM.YYYY')
        end f125,
case when to_char(f126,'SS') = 2 then 'Квартал '||to_char(f126,'Q')
     when to_char(f126,'SS') = 3 then 'Год '||to_char(f126,'YYYY')
     else to_char(f126,'DD.MM.YYYY')
        end f126,
case when to_char(f127,'SS') = 2 then 'Квартал '||to_char(f127,'Q')
     when to_char(f127,'SS') = 3 then 'Год '||to_char(f127,'YYYY')
     else to_char(f127,'DD.MM.YYYY')
        end f127,
case when to_char(f128,'SS') = 2 then 'Квартал '||to_char(f128,'Q')
     when to_char(f128,'SS') = 3 then 'Год '||to_char(f128,'YYYY')
     else to_char(f128,'DD.MM.YYYY')
        end f128,
case when to_char(f129,'SS') = 2 then 'Квартал '||to_char(f129,'Q')
     when to_char(f129,'SS') = 3 then 'Год '||to_char(f129,'YYYY')
     else to_char(f129,'DD.MM.YYYY')
        end f129,
case when to_char(f130,'SS') = 2 then 'Квартал '||to_char(f130,'Q')
     when to_char(f130,'SS') = 3 then 'Год '||to_char(f130,'YYYY')
     else to_char(f130,'DD.MM.YYYY')
        end f130,
case when to_char(f131,'SS') = 2 then 'Квартал '||to_char(f131,'Q')
     when to_char(f131,'SS') = 3 then 'Год '||to_char(f131,'YYYY')
     else to_char(f131,'DD.MM.YYYY')
        end f131,
case when to_char(f132,'SS') = 2 then 'Квартал '||to_char(f132,'Q')
     when to_char(f132,'SS') = 3 then 'Год '||to_char(f132,'YYYY')
     else to_char(f132,'DD.MM.YYYY')
        end f132,
case when to_char(f133,'SS') = 2 then 'Квартал '||to_char(f133,'Q')
     when to_char(f133,'SS') = 3 then 'Год '||to_char(f133,'YYYY')
     else to_char(f133,'DD.MM.YYYY')
        end f133,
case when to_char(f134,'SS') = 2 then 'Квартал '||to_char(f134,'Q')
     when to_char(f134,'SS') = 3 then 'Год '||to_char(f134,'YYYY')
     else to_char(f134,'DD.MM.YYYY')
        end f134,
case when to_char(f135,'SS') = 2 then 'Квартал '||to_char(f135,'Q')
     when to_char(f135,'SS') = 3 then 'Год '||to_char(f135,'YYYY')
     else to_char(f135,'DD.MM.YYYY')
        end f135,
case when to_char(f136,'SS') = 2 then 'Квартал '||to_char(f136,'Q')
     when to_char(f136,'SS') = 3 then 'Год '||to_char(f136,'YYYY')
     else to_char(f136,'DD.MM.YYYY')
        end f136,
case when to_char(f137,'SS') = 2 then 'Квартал '||to_char(f137,'Q')
     when to_char(f137,'SS') = 3 then 'Год '||to_char(f137,'YYYY')
     else to_char(f137,'DD.MM.YYYY')
        end f137,
case when to_char(f138,'SS') = 2 then 'Квартал '||to_char(f138,'Q')
     when to_char(f138,'SS') = 3 then 'Год '||to_char(f138,'YYYY')
     else to_char(f138,'DD.MM.YYYY')
        end f138,
case when to_char(f139,'SS') = 2 then 'Квартал '||to_char(f139,'Q')
     when to_char(f139,'SS') = 3 then 'Год '||to_char(f139,'YYYY')
     else to_char(f139,'DD.MM.YYYY')
        end f139,
case when to_char(f140,'SS') = 2 then 'Квартал '||to_char(f140,'Q')
     when to_char(f140,'SS') = 3 then 'Год '||to_char(f140,'YYYY')
     else to_char(f140,'DD.MM.YYYY')
        end f140,
case when to_char(f141,'SS') = 2 then 'Квартал '||to_char(f141,'Q')
     when to_char(f141,'SS') = 3 then 'Год '||to_char(f141,'YYYY')
     else to_char(f141,'DD.MM.YYYY')
        end f141,
case when to_char(f142,'SS') = 2 then 'Квартал '||to_char(f142,'Q')
     when to_char(f142,'SS') = 3 then 'Год '||to_char(f142,'YYYY')
     else to_char(f142,'DD.MM.YYYY')
        end f142,
case when to_char(f143,'SS') = 2 then 'Квартал '||to_char(f143,'Q')
     when to_char(f143,'SS') = 3 then 'Год '||to_char(f143,'YYYY')
     else to_char(f143,'DD.MM.YYYY')
        end f143,
case when to_char(f144,'SS') = 2 then 'Квартал '||to_char(f144,'Q')
     when to_char(f144,'SS') = 3 then 'Год '||to_char(f144,'YYYY')
     else to_char(f144,'DD.MM.YYYY')
        end f144,
case when to_char(f145,'SS') = 2 then 'Квартал '||to_char(f145,'Q')
     when to_char(f145,'SS') = 3 then 'Год '||to_char(f145,'YYYY')
     else to_char(f145,'DD.MM.YYYY')
        end f145,
case when to_char(f146,'SS') = 2 then 'Квартал '||to_char(f146,'Q')
     when to_char(f146,'SS') = 3 then 'Год '||to_char(f146,'YYYY')
     else to_char(f146,'DD.MM.YYYY')
        end f146,
case when to_char(f147,'SS') = 2 then 'Квартал '||to_char(f147,'Q')
     when to_char(f147,'SS') = 3 then 'Год '||to_char(f147,'YYYY')
     else to_char(f147,'DD.MM.YYYY')
        end f147,
case when to_char(f148,'SS') = 2 then 'Квартал '||to_char(f148,'Q')
     when to_char(f148,'SS') = 3 then 'Год '||to_char(f148,'YYYY')
     else to_char(f148,'DD.MM.YYYY')
        end f148,
case when to_char(f149,'SS') = 2 then 'Квартал '||to_char(f149,'Q')
     when to_char(f149,'SS') = 3 then 'Год '||to_char(f149,'YYYY')
     else to_char(f149,'DD.MM.YYYY')
        end f149,
case when to_char(f150,'SS') = 2 then 'Квартал '||to_char(f150,'Q')
     when to_char(f150,'SS') = 3 then 'Год '||to_char(f150,'YYYY')
     else to_char(f150,'DD.MM.YYYY')
        end f150,
case when to_char(f151,'SS') = 2 then 'Квартал '||to_char(f151,'Q')
     when to_char(f151,'SS') = 3 then 'Год '||to_char(f151,'YYYY')
     else to_char(f151,'DD.MM.YYYY')
        end f151,
case when to_char(f152,'SS') = 2 then 'Квартал '||to_char(f152,'Q')
     when to_char(f152,'SS') = 3 then 'Год '||to_char(f152,'YYYY')
     else to_char(f152,'DD.MM.YYYY')
        end f152,
case when to_char(f153,'SS') = 2 then 'Квартал '||to_char(f153,'Q')
     when to_char(f153,'SS') = 3 then 'Год '||to_char(f153,'YYYY')
     else to_char(f153,'DD.MM.YYYY')
        end f153,
case when to_char(f154,'SS') = 2 then 'Квартал '||to_char(f154,'Q')
     when to_char(f154,'SS') = 3 then 'Год '||to_char(f154,'YYYY')
     else to_char(f154,'DD.MM.YYYY')
        end f154,
case when to_char(f155,'SS') = 2 then 'Квартал '||to_char(f155,'Q')
     when to_char(f155,'SS') = 3 then 'Год '||to_char(f155,'YYYY')
     else to_char(f155,'DD.MM.YYYY')
        end f155,
case when to_char(f156,'SS') = 2 then 'Квартал '||to_char(f156,'Q')
     when to_char(f156,'SS') = 3 then 'Год '||to_char(f156,'YYYY')
     else to_char(f156,'DD.MM.YYYY')
        end f156,
case when to_char(f157,'SS') = 2 then 'Квартал '||to_char(f157,'Q')
     when to_char(f157,'SS') = 3 then 'Год '||to_char(f157,'YYYY')
     else to_char(f157,'DD.MM.YYYY')
        end f157,
case when to_char(f158,'SS') = 2 then 'Квартал '||to_char(f158,'Q')
     when to_char(f158,'SS') = 3 then 'Год '||to_char(f158,'YYYY')
     else to_char(f158,'DD.MM.YYYY')
        end f158,
case when to_char(f159,'SS') = 2 then 'Квартал '||to_char(f159,'Q')
     when to_char(f159,'SS') = 3 then 'Год '||to_char(f159,'YYYY')
     else to_char(f159,'DD.MM.YYYY')
        end f159,
case when to_char(f160,'SS') = 2 then 'Квартал '||to_char(f160,'Q')
     when to_char(f160,'SS') = 3 then 'Год '||to_char(f160,'YYYY')
     else to_char(f160,'DD.MM.YYYY')
        end f160,
case when to_char(f161,'SS') = 2 then 'Квартал '||to_char(f161,'Q')
     when to_char(f161,'SS') = 3 then 'Год '||to_char(f161,'YYYY')
     else to_char(f161,'DD.MM.YYYY')
        end f161,
case when to_char(f162,'SS') = 2 then 'Квартал '||to_char(f162,'Q')
     when to_char(f162,'SS') = 3 then 'Год '||to_char(f162,'YYYY')
     else to_char(f162,'DD.MM.YYYY')
        end f162,
case when to_char(f163,'SS') = 2 then 'Квартал '||to_char(f163,'Q')
     when to_char(f163,'SS') = 3 then 'Год '||to_char(f163,'YYYY')
     else to_char(f163,'DD.MM.YYYY')
        end f163,
case when to_char(f164,'SS') = 2 then 'Квартал '||to_char(f164,'Q')
     when to_char(f164,'SS') = 3 then 'Год '||to_char(f164,'YYYY')
     else to_char(f164,'DD.MM.YYYY')
        end f164,
case when to_char(f165,'SS') = 2 then 'Квартал '||to_char(f165,'Q')
     when to_char(f165,'SS') = 3 then 'Год '||to_char(f165,'YYYY')
     else to_char(f165,'DD.MM.YYYY')
        end f165,
case when to_char(f166,'SS') = 2 then 'Квартал '||to_char(f166,'Q')
     when to_char(f166,'SS') = 3 then 'Год '||to_char(f166,'YYYY')
     else to_char(f166,'DD.MM.YYYY')
        end f166,
case when to_char(f167,'SS') = 2 then 'Квартал '||to_char(f167,'Q')
     when to_char(f167,'SS') = 3 then 'Год '||to_char(f167,'YYYY')
     else to_char(f167,'DD.MM.YYYY')
        end f167,
case when to_char(f168,'SS') = 2 then 'Квартал '||to_char(f168,'Q')
     when to_char(f168,'SS') = 3 then 'Год '||to_char(f168,'YYYY')
     else to_char(f168,'DD.MM.YYYY')
        end f168,
case when to_char(f169,'SS') = 2 then 'Квартал '||to_char(f169,'Q')
     when to_char(f169,'SS') = 3 then 'Год '||to_char(f169,'YYYY')
     else to_char(f169,'DD.MM.YYYY')
        end f169,
case when to_char(f170,'SS') = 2 then 'Квартал '||to_char(f170,'Q')
     when to_char(f170,'SS') = 3 then 'Год '||to_char(f170,'YYYY')
     else to_char(f170,'DD.MM.YYYY')
        end f170,
case when to_char(f171,'SS') = 2 then 'Квартал '||to_char(f171,'Q')
     when to_char(f171,'SS') = 3 then 'Год '||to_char(f171,'YYYY')
     else to_char(f171,'DD.MM.YYYY')
        end f171,
case when to_char(f172,'SS') = 2 then 'Квартал '||to_char(f172,'Q')
     when to_char(f172,'SS') = 3 then 'Год '||to_char(f172,'YYYY')
     else to_char(f172,'DD.MM.YYYY')
        end f172,
case when to_char(f173,'SS') = 2 then 'Квартал '||to_char(f173,'Q')
     when to_char(f173,'SS') = 3 then 'Год '||to_char(f173,'YYYY')
     else to_char(f173,'DD.MM.YYYY')
        end f173,
case when to_char(f174,'SS') = 2 then 'Квартал '||to_char(f174,'Q')
     when to_char(f174,'SS') = 3 then 'Год '||to_char(f174,'YYYY')
     else to_char(f174,'DD.MM.YYYY')
        end f174,
case when to_char(f175,'SS') = 2 then 'Квартал '||to_char(f175,'Q')
     when to_char(f175,'SS') = 3 then 'Год '||to_char(f175,'YYYY')
     else to_char(f175,'DD.MM.YYYY')
        end f175,
case when to_char(f176,'SS') = 2 then 'Квартал '||to_char(f176,'Q')
     when to_char(f176,'SS') = 3 then 'Год '||to_char(f176,'YYYY')
     else to_char(f176,'DD.MM.YYYY')
        end f176,
case when to_char(f177,'SS') = 2 then 'Квартал '||to_char(f177,'Q')
     when to_char(f177,'SS') = 3 then 'Год '||to_char(f177,'YYYY')
     else to_char(f177,'DD.MM.YYYY')
        end f177,
case when to_char(f178,'SS') = 2 then 'Квартал '||to_char(f178,'Q')
     when to_char(f178,'SS') = 3 then 'Год '||to_char(f178,'YYYY')
     else to_char(f178,'DD.MM.YYYY')
        end f178,
case when to_char(f179,'SS') = 2 then 'Квартал '||to_char(f179,'Q')
     when to_char(f179,'SS') = 3 then 'Год '||to_char(f179,'YYYY')
     else to_char(f179,'DD.MM.YYYY')
        end f179,
case when to_char(f180,'SS') = 2 then 'Квартал '||to_char(f180,'Q')
     when to_char(f180,'SS') = 3 then 'Год '||to_char(f180,'YYYY')
     else to_char(f180,'DD.MM.YYYY')
        end f180,
case when to_char(f181,'SS') = 2 then 'Квартал '||to_char(f181,'Q')
     when to_char(f181,'SS') = 3 then 'Год '||to_char(f181,'YYYY')
     else to_char(f181,'DD.MM.YYYY')
        end f181,
case when to_char(f182,'SS') = 2 then 'Квартал '||to_char(f182,'Q')
     when to_char(f182,'SS') = 3 then 'Год '||to_char(f182,'YYYY')
     else to_char(f182,'DD.MM.YYYY')
        end f182,
case when to_char(f183,'SS') = 2 then 'Квартал '||to_char(f183,'Q')
     when to_char(f183,'SS') = 3 then 'Год '||to_char(f183,'YYYY')
     else to_char(f183,'DD.MM.YYYY')
        end f183,
case when to_char(f184,'SS') = 2 then 'Квартал '||to_char(f184,'Q')
     when to_char(f184,'SS') = 3 then 'Год '||to_char(f184,'YYYY')
     else to_char(f184,'DD.MM.YYYY')
        end f184,
case when to_char(f185,'SS') = 2 then 'Квартал '||to_char(f185,'Q')
     when to_char(f185,'SS') = 3 then 'Год '||to_char(f185,'YYYY')
     else to_char(f185,'DD.MM.YYYY')
        end f185,
case when to_char(f186,'SS') = 2 then 'Квартал '||to_char(f186,'Q')
     when to_char(f186,'SS') = 3 then 'Год '||to_char(f186,'YYYY')
     else to_char(f186,'DD.MM.YYYY')
        end f186,
case when to_char(f187,'SS') = 2 then 'Квартал '||to_char(f187,'Q')
     when to_char(f187,'SS') = 3 then 'Год '||to_char(f187,'YYYY')
     else to_char(f187,'DD.MM.YYYY')
        end f187,
case when to_char(f188,'SS') = 2 then 'Квартал '||to_char(f188,'Q')
     when to_char(f188,'SS') = 3 then 'Год '||to_char(f188,'YYYY')
     else to_char(f188,'DD.MM.YYYY')
        end f188,
case when to_char(f189,'SS') = 2 then 'Квартал '||to_char(f189,'Q')
     when to_char(f189,'SS') = 3 then 'Год '||to_char(f189,'YYYY')
     else to_char(f189,'DD.MM.YYYY')
        end f189,
case when to_char(f190,'SS') = 2 then 'Квартал '||to_char(f190,'Q')
     when to_char(f190,'SS') = 3 then 'Год '||to_char(f190,'YYYY')
     else to_char(f190,'DD.MM.YYYY')
        end f190,
case when to_char(f191,'SS') = 2 then 'Квартал '||to_char(f191,'Q')
     when to_char(f191,'SS') = 3 then 'Год '||to_char(f191,'YYYY')
     else to_char(f191,'DD.MM.YYYY')
        end f191,
case when to_char(f192,'SS') = 2 then 'Квартал '||to_char(f192,'Q')
     when to_char(f192,'SS') = 3 then 'Год '||to_char(f192,'YYYY')
     else to_char(f192,'DD.MM.YYYY')
        end f192,
case when to_char(f193,'SS') = 2 then 'Квартал '||to_char(f193,'Q')
     when to_char(f193,'SS') = 3 then 'Год '||to_char(f193,'YYYY')
     else to_char(f193,'DD.MM.YYYY')
        end f193,
case when to_char(f194,'SS') = 2 then 'Квартал '||to_char(f194,'Q')
     when to_char(f194,'SS') = 3 then 'Год '||to_char(f194,'YYYY')
     else to_char(f194,'DD.MM.YYYY')
        end f194,
case when to_char(f195,'SS') = 2 then 'Квартал '||to_char(f195,'Q')
     when to_char(f195,'SS') = 3 then 'Год '||to_char(f195,'YYYY')
     else to_char(f195,'DD.MM.YYYY')
        end f195,
case when to_char(f196,'SS') = 2 then 'Квартал '||to_char(f196,'Q')
     when to_char(f196,'SS') = 3 then 'Год '||to_char(f196,'YYYY')
     else to_char(f196,'DD.MM.YYYY')
        end f196,
case when to_char(f197,'SS') = 2 then 'Квартал '||to_char(f197,'Q')
     when to_char(f197,'SS') = 3 then 'Год '||to_char(f197,'YYYY')
     else to_char(f197,'DD.MM.YYYY')
        end f197,
case when to_char(f198,'SS') = 2 then 'Квартал '||to_char(f198,'Q')
     when to_char(f198,'SS') = 3 then 'Год '||to_char(f198,'YYYY')
     else to_char(f198,'DD.MM.YYYY')
        end f198,
case when to_char(f199,'SS') = 2 then 'Квартал '||to_char(f199,'Q')
     when to_char(f199,'SS') = 3 then 'Год '||to_char(f199,'YYYY')
     else to_char(f199,'DD.MM.YYYY')
        end f199,
case when to_char(f200,'SS') = 2 then 'Квартал '||to_char(f200,'Q')
     when to_char(f200,'SS') = 3 then 'Год '||to_char(f200,'YYYY')
     else to_char(f200,'DD.MM.YYYY')
        end f200,
case when to_char(f201,'SS') = 2 then 'Квартал '||to_char(f201,'Q')
     when to_char(f201,'SS') = 3 then 'Год '||to_char(f201,'YYYY')
     else to_char(f201,'DD.MM.YYYY')
        end f201,
case when to_char(f202,'SS') = 2 then 'Квартал '||to_char(f202,'Q')
     when to_char(f202,'SS') = 3 then 'Год '||to_char(f202,'YYYY')
     else to_char(f202,'DD.MM.YYYY')
        end f202,
case when to_char(f203,'SS') = 2 then 'Квартал '||to_char(f203,'Q')
     when to_char(f203,'SS') = 3 then 'Год '||to_char(f203,'YYYY')
     else to_char(f203,'DD.MM.YYYY')
        end f203,
case when to_char(f204,'SS') = 2 then 'Квартал '||to_char(f204,'Q')
     when to_char(f204,'SS') = 3 then 'Год '||to_char(f204,'YYYY')
     else to_char(f204,'DD.MM.YYYY')
        end f204,
case when to_char(f205,'SS') = 2 then 'Квартал '||to_char(f205,'Q')
     when to_char(f205,'SS') = 3 then 'Год '||to_char(f205,'YYYY')
     else to_char(f205,'DD.MM.YYYY')
        end f205,
case when to_char(f206,'SS') = 2 then 'Квартал '||to_char(f206,'Q')
     when to_char(f206,'SS') = 3 then 'Год '||to_char(f206,'YYYY')
     else to_char(f206,'DD.MM.YYYY')
        end f206,
case when to_char(f207,'SS') = 2 then 'Квартал '||to_char(f207,'Q')
     when to_char(f207,'SS') = 3 then 'Год '||to_char(f207,'YYYY')
     else to_char(f207,'DD.MM.YYYY')
        end f207,
case when to_char(f208,'SS') = 2 then 'Квартал '||to_char(f208,'Q')
     when to_char(f208,'SS') = 3 then 'Год '||to_char(f208,'YYYY')
     else to_char(f208,'DD.MM.YYYY')
        end f208,
case when to_char(f209,'SS') = 2 then 'Квартал '||to_char(f209,'Q')
     when to_char(f209,'SS') = 3 then 'Год '||to_char(f209,'YYYY')
     else to_char(f209,'DD.MM.YYYY')
        end f209,
case when to_char(f210,'SS') = 2 then 'Квартал '||to_char(f210,'Q')
     when to_char(f210,'SS') = 3 then 'Год '||to_char(f210,'YYYY')
     else to_char(f210,'DD.MM.YYYY')
        end f210,
case when to_char(f211,'SS') = 2 then 'Квартал '||to_char(f211,'Q')
     when to_char(f211,'SS') = 3 then 'Год '||to_char(f211,'YYYY')
     else to_char(f211,'DD.MM.YYYY')
        end f211,
case when to_char(f212,'SS') = 2 then 'Квартал '||to_char(f212,'Q')
     when to_char(f212,'SS') = 3 then 'Год '||to_char(f212,'YYYY')
     else to_char(f212,'DD.MM.YYYY')
        end f212,
case when to_char(f213,'SS') = 2 then 'Квартал '||to_char(f213,'Q')
     when to_char(f213,'SS') = 3 then 'Год '||to_char(f213,'YYYY')
     else to_char(f213,'DD.MM.YYYY')
        end f213,
case when to_char(f214,'SS') = 2 then 'Квартал '||to_char(f214,'Q')
     when to_char(f214,'SS') = 3 then 'Год '||to_char(f214,'YYYY')
     else to_char(f214,'DD.MM.YYYY')
        end f214,
case when to_char(f215,'SS') = 2 then 'Квартал '||to_char(f215,'Q')
     when to_char(f215,'SS') = 3 then 'Год '||to_char(f215,'YYYY')
     else to_char(f215,'DD.MM.YYYY')
        end f215,
case when to_char(f216,'SS') = 2 then 'Квартал '||to_char(f216,'Q')
     when to_char(f216,'SS') = 3 then 'Год '||to_char(f216,'YYYY')
     else to_char(f216,'DD.MM.YYYY')
        end f216,
case when to_char(f217,'SS') = 2 then 'Квартал '||to_char(f217,'Q')
     when to_char(f217,'SS') = 3 then 'Год '||to_char(f217,'YYYY')
     else to_char(f217,'DD.MM.YYYY')
        end f217,
case when to_char(f218,'SS') = 2 then 'Квартал '||to_char(f218,'Q')
     when to_char(f218,'SS') = 3 then 'Год '||to_char(f218,'YYYY')
     else to_char(f218,'DD.MM.YYYY')
        end f218,
case when to_char(f219,'SS') = 2 then 'Квартал '||to_char(f219,'Q')
     when to_char(f219,'SS') = 3 then 'Год '||to_char(f219,'YYYY')
     else to_char(f219,'DD.MM.YYYY')
        end f219,
case when to_char(f220,'SS') = 2 then 'Квартал '||to_char(f220,'Q')
     when to_char(f220,'SS') = 3 then 'Год '||to_char(f220,'YYYY')
     else to_char(f220,'DD.MM.YYYY')
        end f220,
case when to_char(f221,'SS') = 2 then 'Квартал '||to_char(f221,'Q')
     when to_char(f221,'SS') = 3 then 'Год '||to_char(f221,'YYYY')
     else to_char(f221,'DD.MM.YYYY')
        end f221,
case when to_char(f222,'SS') = 2 then 'Квартал '||to_char(f222,'Q')
     when to_char(f222,'SS') = 3 then 'Год '||to_char(f222,'YYYY')
     else to_char(f222,'DD.MM.YYYY')
        end f222,
case when to_char(f223,'SS') = 2 then 'Квартал '||to_char(f223,'Q')
     when to_char(f223,'SS') = 3 then 'Год '||to_char(f223,'YYYY')
     else to_char(f223,'DD.MM.YYYY')
        end f223,
case when to_char(f224,'SS') = 2 then 'Квартал '||to_char(f224,'Q')
     when to_char(f224,'SS') = 3 then 'Год '||to_char(f224,'YYYY')
     else to_char(f224,'DD.MM.YYYY')
        end f224,
case when to_char(f225,'SS') = 2 then 'Квартал '||to_char(f225,'Q')
     when to_char(f225,'SS') = 3 then 'Год '||to_char(f225,'YYYY')
     else to_char(f225,'DD.MM.YYYY')
        end f225,
case when to_char(f226,'SS') = 2 then 'Квартал '||to_char(f226,'Q')
     when to_char(f226,'SS') = 3 then 'Год '||to_char(f226,'YYYY')
     else to_char(f226,'DD.MM.YYYY')
        end f226,
case when to_char(f227,'SS') = 2 then 'Квартал '||to_char(f227,'Q')
     when to_char(f227,'SS') = 3 then 'Год '||to_char(f227,'YYYY')
     else to_char(f227,'DD.MM.YYYY')
        end f227,
case when to_char(f228,'SS') = 2 then 'Квартал '||to_char(f228,'Q')
     when to_char(f228,'SS') = 3 then 'Год '||to_char(f228,'YYYY')
     else to_char(f228,'DD.MM.YYYY')
        end f228,
case when to_char(f229,'SS') = 2 then 'Квартал '||to_char(f229,'Q')
     when to_char(f229,'SS') = 3 then 'Год '||to_char(f229,'YYYY')
     else to_char(f229,'DD.MM.YYYY')
        end f229,
case when to_char(f230,'SS') = 2 then 'Квартал '||to_char(f230,'Q')
     when to_char(f230,'SS') = 3 then 'Год '||to_char(f230,'YYYY')
     else to_char(f230,'DD.MM.YYYY')
        end f230,
case when to_char(f231,'SS') = 2 then 'Квартал '||to_char(f231,'Q')
     when to_char(f231,'SS') = 3 then 'Год '||to_char(f231,'YYYY')
     else to_char(f231,'DD.MM.YYYY')
        end f231,
case when to_char(f232,'SS') = 2 then 'Квартал '||to_char(f232,'Q')
     when to_char(f232,'SS') = 3 then 'Год '||to_char(f232,'YYYY')
     else to_char(f232,'DD.MM.YYYY')
        end f232,
case when to_char(f233,'SS') = 2 then 'Квартал '||to_char(f233,'Q')
     when to_char(f233,'SS') = 3 then 'Год '||to_char(f233,'YYYY')
     else to_char(f233,'DD.MM.YYYY')
        end f233,
case when to_char(f234,'SS') = 2 then 'Квартал '||to_char(f234,'Q')
     when to_char(f234,'SS') = 3 then 'Год '||to_char(f234,'YYYY')
     else to_char(f234,'DD.MM.YYYY')
        end f234,
case when to_char(f235,'SS') = 2 then 'Квартал '||to_char(f235,'Q')
     when to_char(f235,'SS') = 3 then 'Год '||to_char(f235,'YYYY')
     else to_char(f235,'DD.MM.YYYY')
        end f235,
case when to_char(f236,'SS') = 2 then 'Квартал '||to_char(f236,'Q')
     when to_char(f236,'SS') = 3 then 'Год '||to_char(f236,'YYYY')
     else to_char(f236,'DD.MM.YYYY')
        end f236,
case when to_char(f237,'SS') = 2 then 'Квартал '||to_char(f237,'Q')
     when to_char(f237,'SS') = 3 then 'Год '||to_char(f237,'YYYY')
     else to_char(f237,'DD.MM.YYYY')
        end f237,
case when to_char(f238,'SS') = 2 then 'Квартал '||to_char(f238,'Q')
     when to_char(f238,'SS') = 3 then 'Год '||to_char(f238,'YYYY')
     else to_char(f238,'DD.MM.YYYY')
        end f238,
case when to_char(f239,'SS') = 2 then 'Квартал '||to_char(f239,'Q')
     when to_char(f239,'SS') = 3 then 'Год '||to_char(f239,'YYYY')
     else to_char(f239,'DD.MM.YYYY')
        end f239,
case when to_char(f240,'SS') = 2 then 'Квартал '||to_char(f240,'Q')
     when to_char(f240,'SS') = 3 then 'Год '||to_char(f240,'YYYY')
     else to_char(f240,'DD.MM.YYYY')
        end f240,
case when to_char(f241,'SS') = 2 then 'Квартал '||to_char(f241,'Q')
     when to_char(f241,'SS') = 3 then 'Год '||to_char(f241,'YYYY')
     else to_char(f241,'DD.MM.YYYY')
        end f241,
case when to_char(f242,'SS') = 2 then 'Квартал '||to_char(f242,'Q')
     when to_char(f242,'SS') = 3 then 'Год '||to_char(f242,'YYYY')
     else to_char(f242,'DD.MM.YYYY')
        end f242,
case when to_char(f243,'SS') = 2 then 'Квартал '||to_char(f243,'Q')
     when to_char(f243,'SS') = 3 then 'Год '||to_char(f243,'YYYY')
     else to_char(f243,'DD.MM.YYYY')
        end f243,
case when to_char(f244,'SS') = 2 then 'Квартал '||to_char(f244,'Q')
     when to_char(f244,'SS') = 3 then 'Год '||to_char(f244,'YYYY')
     else to_char(f244,'DD.MM.YYYY')
        end f244,
case when to_char(f245,'SS') = 2 then 'Квартал '||to_char(f245,'Q')
     when to_char(f245,'SS') = 3 then 'Год '||to_char(f245,'YYYY')
     else to_char(f245,'DD.MM.YYYY')
        end f245,
case when to_char(f246,'SS') = 2 then 'Квартал '||to_char(f246,'Q')
     when to_char(f246,'SS') = 3 then 'Год '||to_char(f246,'YYYY')
     else to_char(f246,'DD.MM.YYYY')
        end f246,
case when to_char(f247,'SS') = 2 then 'Квартал '||to_char(f247,'Q')
     when to_char(f247,'SS') = 3 then 'Год '||to_char(f247,'YYYY')
     else to_char(f247,'DD.MM.YYYY')
        end f247,
case when to_char(f248,'SS') = 2 then 'Квартал '||to_char(f248,'Q')
     when to_char(f248,'SS') = 3 then 'Год '||to_char(f248,'YYYY')
     else to_char(f248,'DD.MM.YYYY')
        end f248,
case when to_char(f249,'SS') = 2 then 'Квартал '||to_char(f249,'Q')
     when to_char(f249,'SS') = 3 then 'Год '||to_char(f249,'YYYY')
     else to_char(f249,'DD.MM.YYYY')
        end f249,
case when to_char(f250,'SS') = 2 then 'Квартал '||to_char(f250,'Q')
     when to_char(f250,'SS') = 3 then 'Год '||to_char(f250,'YYYY')
     else to_char(f250,'DD.MM.YYYY')
        end f250,
case when to_char(f251,'SS') = 2 then 'Квартал '||to_char(f251,'Q')
     when to_char(f251,'SS') = 3 then 'Год '||to_char(f251,'YYYY')
     else to_char(f251,'DD.MM.YYYY')
        end f251,
case when to_char(f252,'SS') = 2 then 'Квартал '||to_char(f252,'Q')
     when to_char(f252,'SS') = 3 then 'Год '||to_char(f252,'YYYY')
     else to_char(f252,'DD.MM.YYYY')
        end f252,
case when to_char(f253,'SS') = 2 then 'Квартал '||to_char(f253,'Q')
     when to_char(f253,'SS') = 3 then 'Год '||to_char(f253,'YYYY')
     else to_char(f253,'DD.MM.YYYY')
        end f253,
case when to_char(f254,'SS') = 2 then 'Квартал '||to_char(f254,'Q')
     when to_char(f254,'SS') = 3 then 'Год '||to_char(f254,'YYYY')
     else to_char(f254,'DD.MM.YYYY')
        end f254,
case when to_char(f255,'SS') = 2 then 'Квартал '||to_char(f255,'Q')
     when to_char(f255,'SS') = 3 then 'Год '||to_char(f255,'YYYY')
     else to_char(f255,'DD.MM.YYYY')
        end f255,
case when to_char(f256,'SS') = 2 then 'Квартал '||to_char(f256,'Q')
     when to_char(f256,'SS') = 3 then 'Год '||to_char(f256,'YYYY')
     else to_char(f256,'DD.MM.YYYY')
        end f256,
case when to_char(f257,'SS') = 2 then 'Квартал '||to_char(f257,'Q')
     when to_char(f257,'SS') = 3 then 'Год '||to_char(f257,'YYYY')
     else to_char(f257,'DD.MM.YYYY')
        end f257,
case when to_char(f258,'SS') = 2 then 'Квартал '||to_char(f258,'Q')
     when to_char(f258,'SS') = 3 then 'Год '||to_char(f258,'YYYY')
     else to_char(f258,'DD.MM.YYYY')
        end f258,
case when to_char(f259,'SS') = 2 then 'Квартал '||to_char(f259,'Q')
     when to_char(f259,'SS') = 3 then 'Год '||to_char(f259,'YYYY')
     else to_char(f259,'DD.MM.YYYY')
        end f259,
case when to_char(f260,'SS') = 2 then 'Квартал '||to_char(f260,'Q')
     when to_char(f260,'SS') = 3 then 'Год '||to_char(f260,'YYYY')
     else to_char(f260,'DD.MM.YYYY')
        end f260,
case when to_char(f261,'SS') = 2 then 'Квартал '||to_char(f261,'Q')
     when to_char(f261,'SS') = 3 then 'Год '||to_char(f261,'YYYY')
     else to_char(f261,'DD.MM.YYYY')
        end f261,
case when to_char(f262,'SS') = 2 then 'Квартал '||to_char(f262,'Q')
     when to_char(f262,'SS') = 3 then 'Год '||to_char(f262,'YYYY')
     else to_char(f262,'DD.MM.YYYY')
        end f262,
case when to_char(f263,'SS') = 2 then 'Квартал '||to_char(f263,'Q')
     when to_char(f263,'SS') = 3 then 'Год '||to_char(f263,'YYYY')
     else to_char(f263,'DD.MM.YYYY')
        end f263,
case when to_char(f264,'SS') = 2 then 'Квартал '||to_char(f264,'Q')
     when to_char(f264,'SS') = 3 then 'Год '||to_char(f264,'YYYY')
     else to_char(f264,'DD.MM.YYYY')
        end f264,
case when to_char(f265,'SS') = 2 then 'Квартал '||to_char(f265,'Q')
     when to_char(f265,'SS') = 3 then 'Год '||to_char(f265,'YYYY')
     else to_char(f265,'DD.MM.YYYY')
        end f265,
case when to_char(f266,'SS') = 2 then 'Квартал '||to_char(f266,'Q')
     when to_char(f266,'SS') = 3 then 'Год '||to_char(f266,'YYYY')
     else to_char(f266,'DD.MM.YYYY')
        end f266,
case when to_char(f267,'SS') = 2 then 'Квартал '||to_char(f267,'Q')
     when to_char(f267,'SS') = 3 then 'Год '||to_char(f267,'YYYY')
     else to_char(f267,'DD.MM.YYYY')
        end f267,
case when to_char(f268,'SS') = 2 then 'Квартал '||to_char(f268,'Q')
     when to_char(f268,'SS') = 3 then 'Год '||to_char(f268,'YYYY')
     else to_char(f268,'DD.MM.YYYY')
        end f268,
case when to_char(f269,'SS') = 2 then 'Квартал '||to_char(f269,'Q')
     when to_char(f269,'SS') = 3 then 'Год '||to_char(f269,'YYYY')
     else to_char(f269,'DD.MM.YYYY')
        end f269,
case when to_char(f270,'SS') = 2 then 'Квартал '||to_char(f270,'Q')
     when to_char(f270,'SS') = 3 then 'Год '||to_char(f270,'YYYY')
     else to_char(f270,'DD.MM.YYYY')
        end f270,
case when to_char(f271,'SS') = 2 then 'Квартал '||to_char(f271,'Q')
     when to_char(f271,'SS') = 3 then 'Год '||to_char(f271,'YYYY')
     else to_char(f271,'DD.MM.YYYY')
        end f271,
case when to_char(f272,'SS') = 2 then 'Квартал '||to_char(f272,'Q')
     when to_char(f272,'SS') = 3 then 'Год '||to_char(f272,'YYYY')
     else to_char(f272,'DD.MM.YYYY')
        end f272,
case when to_char(f273,'SS') = 2 then 'Квартал '||to_char(f273,'Q')
     when to_char(f273,'SS') = 3 then 'Год '||to_char(f273,'YYYY')
     else to_char(f273,'DD.MM.YYYY')
        end f273,
case when to_char(f274,'SS') = 2 then 'Квартал '||to_char(f274,'Q')
     when to_char(f274,'SS') = 3 then 'Год '||to_char(f274,'YYYY')
     else to_char(f274,'DD.MM.YYYY')
        end f274,
case when to_char(f275,'SS') = 2 then 'Квартал '||to_char(f275,'Q')
     when to_char(f275,'SS') = 3 then 'Год '||to_char(f275,'YYYY')
     else to_char(f275,'DD.MM.YYYY')
        end f275,
case when to_char(f276,'SS') = 2 then 'Квартал '||to_char(f276,'Q')
     when to_char(f276,'SS') = 3 then 'Год '||to_char(f276,'YYYY')
     else to_char(f276,'DD.MM.YYYY')
        end f276,
case when to_char(f277,'SS') = 2 then 'Квартал '||to_char(f277,'Q')
     when to_char(f277,'SS') = 3 then 'Год '||to_char(f277,'YYYY')
     else to_char(f277,'DD.MM.YYYY')
        end f277,
case when to_char(f278,'SS') = 2 then 'Квартал '||to_char(f278,'Q')
     when to_char(f278,'SS') = 3 then 'Год '||to_char(f278,'YYYY')
     else to_char(f278,'DD.MM.YYYY')
        end f278,
case when to_char(f279,'SS') = 2 then 'Квартал '||to_char(f279,'Q')
     when to_char(f279,'SS') = 3 then 'Год '||to_char(f279,'YYYY')
     else to_char(f279,'DD.MM.YYYY')
        end f279,
case when to_char(f280,'SS') = 2 then 'Квартал '||to_char(f280,'Q')
     when to_char(f280,'SS') = 3 then 'Год '||to_char(f280,'YYYY')
     else to_char(f280,'DD.MM.YYYY')
        end f280,
case when to_char(f281,'SS') = 2 then 'Квартал '||to_char(f281,'Q')
     when to_char(f281,'SS') = 3 then 'Год '||to_char(f281,'YYYY')
     else to_char(f281,'DD.MM.YYYY')
        end f281,
case when to_char(f282,'SS') = 2 then 'Квартал '||to_char(f282,'Q')
     when to_char(f282,'SS') = 3 then 'Год '||to_char(f282,'YYYY')
     else to_char(f282,'DD.MM.YYYY')
        end f282,
case when to_char(f283,'SS') = 2 then 'Квартал '||to_char(f283,'Q')
     when to_char(f283,'SS') = 3 then 'Год '||to_char(f283,'YYYY')
     else to_char(f283,'DD.MM.YYYY')
        end f283,
case when to_char(f284,'SS') = 2 then 'Квартал '||to_char(f284,'Q')
     when to_char(f284,'SS') = 3 then 'Год '||to_char(f284,'YYYY')
     else to_char(f284,'DD.MM.YYYY')
        end f284,
case when to_char(f285,'SS') = 2 then 'Квартал '||to_char(f285,'Q')
     when to_char(f285,'SS') = 3 then 'Год '||to_char(f285,'YYYY')
     else to_char(f285,'DD.MM.YYYY')
        end f285,
case when to_char(f286,'SS') = 2 then 'Квартал '||to_char(f286,'Q')
     when to_char(f286,'SS') = 3 then 'Год '||to_char(f286,'YYYY')
     else to_char(f286,'DD.MM.YYYY')
        end f286,
case when to_char(f287,'SS') = 2 then 'Квартал '||to_char(f287,'Q')
     when to_char(f287,'SS') = 3 then 'Год '||to_char(f287,'YYYY')
     else to_char(f287,'DD.MM.YYYY')
        end f287,
case when to_char(f288,'SS') = 2 then 'Квартал '||to_char(f288,'Q')
     when to_char(f288,'SS') = 3 then 'Год '||to_char(f288,'YYYY')
     else to_char(f288,'DD.MM.YYYY')
        end f288,
case when to_char(f289,'SS') = 2 then 'Квартал '||to_char(f289,'Q')
     when to_char(f289,'SS') = 3 then 'Год '||to_char(f289,'YYYY')
     else to_char(f289,'DD.MM.YYYY')
        end f289,
case when to_char(f290,'SS') = 2 then 'Квартал '||to_char(f290,'Q')
     when to_char(f290,'SS') = 3 then 'Год '||to_char(f290,'YYYY')
     else to_char(f290,'DD.MM.YYYY')
        end f290,
case when to_char(f291,'SS') = 2 then 'Квартал '||to_char(f291,'Q')
     when to_char(f291,'SS') = 3 then 'Год '||to_char(f291,'YYYY')
     else to_char(f291,'DD.MM.YYYY')
        end f291,
case when to_char(f292,'SS') = 2 then 'Квартал '||to_char(f292,'Q')
     when to_char(f292,'SS') = 3 then 'Год '||to_char(f292,'YYYY')
     else to_char(f292,'DD.MM.YYYY')
        end f292,
case when to_char(f293,'SS') = 2 then 'Квартал '||to_char(f293,'Q')
     when to_char(f293,'SS') = 3 then 'Год '||to_char(f293,'YYYY')
     else to_char(f293,'DD.MM.YYYY')
        end f293,
case when to_char(f294,'SS') = 2 then 'Квартал '||to_char(f294,'Q')
     when to_char(f294,'SS') = 3 then 'Год '||to_char(f294,'YYYY')
     else to_char(f294,'DD.MM.YYYY')
        end f294,
case when to_char(f295,'SS') = 2 then 'Квартал '||to_char(f295,'Q')
     when to_char(f295,'SS') = 3 then 'Год '||to_char(f295,'YYYY')
     else to_char(f295,'DD.MM.YYYY')
        end f295,
case when to_char(f296,'SS') = 2 then 'Квартал '||to_char(f296,'Q')
     when to_char(f296,'SS') = 3 then 'Год '||to_char(f296,'YYYY')
     else to_char(f296,'DD.MM.YYYY')
        end f296,
case when to_char(f297,'SS') = 2 then 'Квартал '||to_char(f297,'Q')
     when to_char(f297,'SS') = 3 then 'Год '||to_char(f297,'YYYY')
     else to_char(f297,'DD.MM.YYYY')
        end f297,
case when to_char(f298,'SS') = 2 then 'Квартал '||to_char(f298,'Q')
     when to_char(f298,'SS') = 3 then 'Год '||to_char(f298,'YYYY')
     else to_char(f298,'DD.MM.YYYY')
        end f298,
case when to_char(f299,'SS') = 2 then 'Квартал '||to_char(f299,'Q')
     when to_char(f299,'SS') = 3 then 'Год '||to_char(f299,'YYYY')
     else to_char(f299,'DD.MM.YYYY')
        end f299,
case when to_char(f300,'SS') = 2 then 'Квартал '||to_char(f300,'Q')
     when to_char(f300,'SS') = 3 then 'Год '||to_char(f300,'YYYY')
     else to_char(f300,'DD.MM.YYYY')
        end f300,
case when to_char(f301,'SS') = 2 then 'Квартал '||to_char(f301,'Q')
     when to_char(f301,'SS') = 3 then 'Год '||to_char(f301,'YYYY')
     else to_char(f301,'DD.MM.YYYY')
        end f301,
case when to_char(f302,'SS') = 2 then 'Квартал '||to_char(f302,'Q')
     when to_char(f302,'SS') = 3 then 'Год '||to_char(f302,'YYYY')
     else to_char(f302,'DD.MM.YYYY')
        end f302,
case when to_char(f303,'SS') = 2 then 'Квартал '||to_char(f303,'Q')
     when to_char(f303,'SS') = 3 then 'Год '||to_char(f303,'YYYY')
     else to_char(f303,'DD.MM.YYYY')
        end f303,
case when to_char(f304,'SS') = 2 then 'Квартал '||to_char(f304,'Q')
     when to_char(f304,'SS') = 3 then 'Год '||to_char(f304,'YYYY')
     else to_char(f304,'DD.MM.YYYY')
        end f304,
case when to_char(f305,'SS') = 2 then 'Квартал '||to_char(f305,'Q')
     when to_char(f305,'SS') = 3 then 'Год '||to_char(f305,'YYYY')
     else to_char(f305,'DD.MM.YYYY')
        end f305,
case when to_char(f306,'SS') = 2 then 'Квартал '||to_char(f306,'Q')
     when to_char(f306,'SS') = 3 then 'Год '||to_char(f306,'YYYY')
     else to_char(f306,'DD.MM.YYYY')
        end f306,
case when to_char(f307,'SS') = 2 then 'Квартал '||to_char(f307,'Q')
     when to_char(f307,'SS') = 3 then 'Год '||to_char(f307,'YYYY')
     else to_char(f307,'DD.MM.YYYY')
        end f307,
case when to_char(f308,'SS') = 2 then 'Квартал '||to_char(f308,'Q')
     when to_char(f308,'SS') = 3 then 'Год '||to_char(f308,'YYYY')
     else to_char(f308,'DD.MM.YYYY')
        end f308,
case when to_char(f309,'SS') = 2 then 'Квартал '||to_char(f309,'Q')
     when to_char(f309,'SS') = 3 then 'Год '||to_char(f309,'YYYY')
     else to_char(f309,'DD.MM.YYYY')
        end f309,
case when to_char(f310,'SS') = 2 then 'Квартал '||to_char(f310,'Q')
     when to_char(f310,'SS') = 3 then 'Год '||to_char(f310,'YYYY')
     else to_char(f310,'DD.MM.YYYY')
        end f310,
case when to_char(f311,'SS') = 2 then 'Квартал '||to_char(f311,'Q')
     when to_char(f311,'SS') = 3 then 'Год '||to_char(f311,'YYYY')
     else to_char(f311,'DD.MM.YYYY')
        end f311,
case when to_char(f312,'SS') = 2 then 'Квартал '||to_char(f312,'Q')
     when to_char(f312,'SS') = 3 then 'Год '||to_char(f312,'YYYY')
     else to_char(f312,'DD.MM.YYYY')
        end f312,
case when to_char(f313,'SS') = 2 then 'Квартал '||to_char(f313,'Q')
     when to_char(f313,'SS') = 3 then 'Год '||to_char(f313,'YYYY')
     else to_char(f313,'DD.MM.YYYY')
        end f313,
case when to_char(f314,'SS') = 2 then 'Квартал '||to_char(f314,'Q')
     when to_char(f314,'SS') = 3 then 'Год '||to_char(f314,'YYYY')
     else to_char(f314,'DD.MM.YYYY')
        end f314,
case when to_char(f315,'SS') = 2 then 'Квартал '||to_char(f315,'Q')
     when to_char(f315,'SS') = 3 then 'Год '||to_char(f315,'YYYY')
     else to_char(f315,'DD.MM.YYYY')
        end f315,
case when to_char(f316,'SS') = 2 then 'Квартал '||to_char(f316,'Q')
     when to_char(f316,'SS') = 3 then 'Год '||to_char(f316,'YYYY')
     else to_char(f316,'DD.MM.YYYY')
        end f316,
case when to_char(f317,'SS') = 2 then 'Квартал '||to_char(f317,'Q')
     when to_char(f317,'SS') = 3 then 'Год '||to_char(f317,'YYYY')
     else to_char(f317,'DD.MM.YYYY')
        end f317,
case when to_char(f318,'SS') = 2 then 'Квартал '||to_char(f318,'Q')
     when to_char(f318,'SS') = 3 then 'Год '||to_char(f318,'YYYY')
     else to_char(f318,'DD.MM.YYYY')
        end f318,
case when to_char(f319,'SS') = 2 then 'Квартал '||to_char(f319,'Q')
     when to_char(f319,'SS') = 3 then 'Год '||to_char(f319,'YYYY')
     else to_char(f319,'DD.MM.YYYY')
        end f319,
case when to_char(f320,'SS') = 2 then 'Квартал '||to_char(f320,'Q')
     when to_char(f320,'SS') = 3 then 'Год '||to_char(f320,'YYYY')
     else to_char(f320,'DD.MM.YYYY')
        end f320,
case when to_char(f321,'SS') = 2 then 'Квартал '||to_char(f321,'Q')
     when to_char(f321,'SS') = 3 then 'Год '||to_char(f321,'YYYY')
     else to_char(f321,'DD.MM.YYYY')
        end f321,
case when to_char(f322,'SS') = 2 then 'Квартал '||to_char(f322,'Q')
     when to_char(f322,'SS') = 3 then 'Год '||to_char(f322,'YYYY')
     else to_char(f322,'DD.MM.YYYY')
        end f322,
case when to_char(f323,'SS') = 2 then 'Квартал '||to_char(f323,'Q')
     when to_char(f323,'SS') = 3 then 'Год '||to_char(f323,'YYYY')
     else to_char(f323,'DD.MM.YYYY')
        end f323,
case when to_char(f324,'SS') = 2 then 'Квартал '||to_char(f324,'Q')
     when to_char(f324,'SS') = 3 then 'Год '||to_char(f324,'YYYY')
     else to_char(f324,'DD.MM.YYYY')
        end f324,
case when to_char(f325,'SS') = 2 then 'Квартал '||to_char(f325,'Q')
     when to_char(f325,'SS') = 3 then 'Год '||to_char(f325,'YYYY')
     else to_char(f325,'DD.MM.YYYY')
        end f325,
case when to_char(f326,'SS') = 2 then 'Квартал '||to_char(f326,'Q')
     when to_char(f326,'SS') = 3 then 'Год '||to_char(f326,'YYYY')
     else to_char(f326,'DD.MM.YYYY')
        end f326,
case when to_char(f327,'SS') = 2 then 'Квартал '||to_char(f327,'Q')
     when to_char(f327,'SS') = 3 then 'Год '||to_char(f327,'YYYY')
     else to_char(f327,'DD.MM.YYYY')
        end f327,
case when to_char(f328,'SS') = 2 then 'Квартал '||to_char(f328,'Q')
     when to_char(f328,'SS') = 3 then 'Год '||to_char(f328,'YYYY')
     else to_char(f328,'DD.MM.YYYY')
        end f328,
case when to_char(f329,'SS') = 2 then 'Квартал '||to_char(f329,'Q')
     when to_char(f329,'SS') = 3 then 'Год '||to_char(f329,'YYYY')
     else to_char(f329,'DD.MM.YYYY')
        end f329,
case when to_char(f330,'SS') = 2 then 'Квартал '||to_char(f330,'Q')
     when to_char(f330,'SS') = 3 then 'Год '||to_char(f330,'YYYY')
     else to_char(f330,'DD.MM.YYYY')
        end f330,
case when to_char(f331,'SS') = 2 then 'Квартал '||to_char(f331,'Q')
     when to_char(f331,'SS') = 3 then 'Год '||to_char(f331,'YYYY')
     else to_char(f331,'DD.MM.YYYY')
        end f331,
case when to_char(f332,'SS') = 2 then 'Квартал '||to_char(f332,'Q')
     when to_char(f332,'SS') = 3 then 'Год '||to_char(f332,'YYYY')
     else to_char(f332,'DD.MM.YYYY')
        end f332,
case when to_char(f333,'SS') = 2 then 'Квартал '||to_char(f333,'Q')
     when to_char(f333,'SS') = 3 then 'Год '||to_char(f333,'YYYY')
     else to_char(f333,'DD.MM.YYYY')
        end f333,
case when to_char(f334,'SS') = 2 then 'Квартал '||to_char(f334,'Q')
     when to_char(f334,'SS') = 3 then 'Год '||to_char(f334,'YYYY')
     else to_char(f334,'DD.MM.YYYY')
        end f334,
case when to_char(f335,'SS') = 2 then 'Квартал '||to_char(f335,'Q')
     when to_char(f335,'SS') = 3 then 'Год '||to_char(f335,'YYYY')
     else to_char(f335,'DD.MM.YYYY')
        end f335,
case when to_char(f336,'SS') = 2 then 'Квартал '||to_char(f336,'Q')
     when to_char(f336,'SS') = 3 then 'Год '||to_char(f336,'YYYY')
     else to_char(f336,'DD.MM.YYYY')
        end f336,
case when to_char(f337,'SS') = 2 then 'Квартал '||to_char(f337,'Q')
     when to_char(f337,'SS') = 3 then 'Год '||to_char(f337,'YYYY')
     else to_char(f337,'DD.MM.YYYY')
        end f337,
case when to_char(f338,'SS') = 2 then 'Квартал '||to_char(f338,'Q')
     when to_char(f338,'SS') = 3 then 'Год '||to_char(f338,'YYYY')
     else to_char(f338,'DD.MM.YYYY')
        end f338,
case when to_char(f339,'SS') = 2 then 'Квартал '||to_char(f339,'Q')
     when to_char(f339,'SS') = 3 then 'Год '||to_char(f339,'YYYY')
     else to_char(f339,'DD.MM.YYYY')
        end f339,
case when to_char(f340,'SS') = 2 then 'Квартал '||to_char(f340,'Q')
     when to_char(f340,'SS') = 3 then 'Год '||to_char(f340,'YYYY')
     else to_char(f340,'DD.MM.YYYY')
        end f340,
case when to_char(f341,'SS') = 2 then 'Квартал '||to_char(f341,'Q')
     when to_char(f341,'SS') = 3 then 'Год '||to_char(f341,'YYYY')
     else to_char(f341,'DD.MM.YYYY')
        end f341,
case when to_char(f342,'SS') = 2 then 'Квартал '||to_char(f342,'Q')
     when to_char(f342,'SS') = 3 then 'Год '||to_char(f342,'YYYY')
     else to_char(f342,'DD.MM.YYYY')
        end f342,
case when to_char(f343,'SS') = 2 then 'Квартал '||to_char(f343,'Q')
     when to_char(f343,'SS') = 3 then 'Год '||to_char(f343,'YYYY')
     else to_char(f343,'DD.MM.YYYY')
        end f343,
case when to_char(f344,'SS') = 2 then 'Квартал '||to_char(f344,'Q')
     when to_char(f344,'SS') = 3 then 'Год '||to_char(f344,'YYYY')
     else to_char(f344,'DD.MM.YYYY')
        end f344,
case when to_char(f345,'SS') = 2 then 'Квартал '||to_char(f345,'Q')
     when to_char(f345,'SS') = 3 then 'Год '||to_char(f345,'YYYY')
     else to_char(f345,'DD.MM.YYYY')
        end f345,
case when to_char(f346,'SS') = 2 then 'Квартал '||to_char(f346,'Q')
     when to_char(f346,'SS') = 3 then 'Год '||to_char(f346,'YYYY')
     else to_char(f346,'DD.MM.YYYY')
        end f346,
case when to_char(f347,'SS') = 2 then 'Квартал '||to_char(f347,'Q')
     when to_char(f347,'SS') = 3 then 'Год '||to_char(f347,'YYYY')
     else to_char(f347,'DD.MM.YYYY')
        end f347,
case when to_char(f348,'SS') = 2 then 'Квартал '||to_char(f348,'Q')
     when to_char(f348,'SS') = 3 then 'Год '||to_char(f348,'YYYY')
     else to_char(f348,'DD.MM.YYYY')
        end f348,
case when to_char(f349,'SS') = 2 then 'Квартал '||to_char(f349,'Q')
     when to_char(f349,'SS') = 3 then 'Год '||to_char(f349,'YYYY')
     else to_char(f349,'DD.MM.YYYY')
        end f349,
case when to_char(f350,'SS') = 2 then 'Квартал '||to_char(f350,'Q')
     when to_char(f350,'SS') = 3 then 'Год '||to_char(f350,'YYYY')
     else to_char(f350,'DD.MM.YYYY')
        end f350,
case when to_char(f351,'SS') = 2 then 'Квартал '||to_char(f351,'Q')
     when to_char(f351,'SS') = 3 then 'Год '||to_char(f351,'YYYY')
     else to_char(f351,'DD.MM.YYYY')
        end f351,
case when to_char(f352,'SS') = 2 then 'Квартал '||to_char(f352,'Q')
     when to_char(f352,'SS') = 3 then 'Год '||to_char(f352,'YYYY')
     else to_char(f352,'DD.MM.YYYY')
        end f352,
case when to_char(f353,'SS') = 2 then 'Квартал '||to_char(f353,'Q')
     when to_char(f353,'SS') = 3 then 'Год '||to_char(f353,'YYYY')
     else to_char(f353,'DD.MM.YYYY')
        end f353,
case when to_char(f354,'SS') = 2 then 'Квартал '||to_char(f354,'Q')
     when to_char(f354,'SS') = 3 then 'Год '||to_char(f354,'YYYY')
     else to_char(f354,'DD.MM.YYYY')
        end f354,
case when to_char(f355,'SS') = 2 then 'Квартал '||to_char(f355,'Q')
     when to_char(f355,'SS') = 3 then 'Год '||to_char(f355,'YYYY')
     else to_char(f355,'DD.MM.YYYY')
        end f355,
case when to_char(f356,'SS') = 2 then 'Квартал '||to_char(f356,'Q')
     when to_char(f356,'SS') = 3 then 'Год '||to_char(f356,'YYYY')
     else to_char(f356,'DD.MM.YYYY')
        end f356,
case when to_char(f357,'SS') = 2 then 'Квартал '||to_char(f357,'Q')
     when to_char(f357,'SS') = 3 then 'Год '||to_char(f357,'YYYY')
     else to_char(f357,'DD.MM.YYYY')
        end f357,
case when to_char(f358,'SS') = 2 then 'Квартал '||to_char(f358,'Q')
     when to_char(f358,'SS') = 3 then 'Год '||to_char(f358,'YYYY')
     else to_char(f358,'DD.MM.YYYY')
        end f358,
case when to_char(f359,'SS') = 2 then 'Квартал '||to_char(f359,'Q')
     when to_char(f359,'SS') = 3 then 'Год '||to_char(f359,'YYYY')
     else to_char(f359,'DD.MM.YYYY')
        end f359,
case when to_char(f360,'SS') = 2 then 'Квартал '||to_char(f360,'Q')
     when to_char(f360,'SS') = 3 then 'Год '||to_char(f360,'YYYY')
     else to_char(f360,'DD.MM.YYYY')
        end f360,
case when to_char(f361,'SS') = 2 then 'Квартал '||to_char(f361,'Q')
     when to_char(f361,'SS') = 3 then 'Год '||to_char(f361,'YYYY')
     else to_char(f361,'DD.MM.YYYY')
        end f361,
case when to_char(f362,'SS') = 2 then 'Квартал '||to_char(f362,'Q')
     when to_char(f362,'SS') = 3 then 'Год '||to_char(f362,'YYYY')
     else to_char(f362,'DD.MM.YYYY')
        end f362,
case when to_char(f363,'SS') = 2 then 'Квартал '||to_char(f363,'Q')
     when to_char(f363,'SS') = 3 then 'Год '||to_char(f363,'YYYY')
     else to_char(f363,'DD.MM.YYYY')
        end f363,
case when to_char(f364,'SS') = 2 then 'Квартал '||to_char(f364,'Q')
     when to_char(f364,'SS') = 3 then 'Год '||to_char(f364,'YYYY')
     else to_char(f364,'DD.MM.YYYY')
        end f364,
case when to_char(f365,'SS') = 2 then 'Квартал '||to_char(f365,'Q')
     when to_char(f365,'SS') = 3 then 'Год '||to_char(f365,'YYYY')
     else to_char(f365,'DD.MM.YYYY')
        end f365,
case when to_char(f366,'SS') = 2 then 'Квартал '||to_char(f366,'Q')
     when to_char(f366,'SS') = 3 then 'Год '||to_char(f366,'YYYY')
     else to_char(f366,'DD.MM.YYYY')
        end f366,
case when to_char(f367,'SS') = 2 then 'Квартал '||to_char(f367,'Q')
     when to_char(f367,'SS') = 3 then 'Год '||to_char(f367,'YYYY')
     else to_char(f367,'DD.MM.YYYY')
        end f367,
case when to_char(f368,'SS') = 2 then 'Квартал '||to_char(f368,'Q')
     when to_char(f368,'SS') = 3 then 'Год '||to_char(f368,'YYYY')
     else to_char(f368,'DD.MM.YYYY')
        end f368,
case when to_char(f369,'SS') = 2 then 'Квартал '||to_char(f369,'Q')
     when to_char(f369,'SS') = 3 then 'Год '||to_char(f369,'YYYY')
     else to_char(f369,'DD.MM.YYYY')
        end f369,
case when to_char(f370,'SS') = 2 then 'Квартал '||to_char(f370,'Q')
     when to_char(f370,'SS') = 3 then 'Год '||to_char(f370,'YYYY')
     else to_char(f370,'DD.MM.YYYY')
        end f370,
case when to_char(f371,'SS') = 2 then 'Квартал '||to_char(f371,'Q')
     when to_char(f371,'SS') = 3 then 'Год '||to_char(f371,'YYYY')
     else to_char(f371,'DD.MM.YYYY')
        end f371,
case when to_char(f372,'SS') = 2 then 'Квартал '||to_char(f372,'Q')
     when to_char(f372,'SS') = 3 then 'Год '||to_char(f372,'YYYY')
     else to_char(f372,'DD.MM.YYYY')
        end f372,
case when to_char(f373,'SS') = 2 then 'Квартал '||to_char(f373,'Q')
     when to_char(f373,'SS') = 3 then 'Год '||to_char(f373,'YYYY')
     else to_char(f373,'DD.MM.YYYY')
        end f373,
case when to_char(f374,'SS') = 2 then 'Квартал '||to_char(f374,'Q')
     when to_char(f374,'SS') = 3 then 'Год '||to_char(f374,'YYYY')
     else to_char(f374,'DD.MM.YYYY')
        end f374,
case when to_char(f375,'SS') = 2 then 'Квартал '||to_char(f375,'Q')
     when to_char(f375,'SS') = 3 then 'Год '||to_char(f375,'YYYY')
     else to_char(f375,'DD.MM.YYYY')
        end f375,
case when to_char(f376,'SS') = 2 then 'Квартал '||to_char(f376,'Q')
     when to_char(f376,'SS') = 3 then 'Год '||to_char(f376,'YYYY')
     else to_char(f376,'DD.MM.YYYY')
        end f376,
case when to_char(f377,'SS') = 2 then 'Квартал '||to_char(f377,'Q')
     when to_char(f377,'SS') = 3 then 'Год '||to_char(f377,'YYYY')
     else to_char(f377,'DD.MM.YYYY')
        end f377,
case when to_char(f378,'SS') = 2 then 'Квартал '||to_char(f378,'Q')
     when to_char(f378,'SS') = 3 then 'Год '||to_char(f378,'YYYY')
     else to_char(f378,'DD.MM.YYYY')
        end f378,
case when to_char(f379,'SS') = 2 then 'Квартал '||to_char(f379,'Q')
     when to_char(f379,'SS') = 3 then 'Год '||to_char(f379,'YYYY')
     else to_char(f379,'DD.MM.YYYY')
        end f379,
case when to_char(f380,'SS') = 2 then 'Квартал '||to_char(f380,'Q')
     when to_char(f380,'SS') = 3 then 'Год '||to_char(f380,'YYYY')
     else to_char(f380,'DD.MM.YYYY')
        end f380,
case when to_char(f381,'SS') = 2 then 'Квартал '||to_char(f381,'Q')
     when to_char(f381,'SS') = 3 then 'Год '||to_char(f381,'YYYY')
     else to_char(f381,'DD.MM.YYYY')
        end f381,
case when to_char(f382,'SS') = 2 then 'Квартал '||to_char(f382,'Q')
     when to_char(f382,'SS') = 3 then 'Год '||to_char(f382,'YYYY')
     else to_char(f382,'DD.MM.YYYY')
        end f382,
case when to_char(f383,'SS') = 2 then 'Квартал '||to_char(f383,'Q')
     when to_char(f383,'SS') = 3 then 'Год '||to_char(f383,'YYYY')
     else to_char(f383,'DD.MM.YYYY')
        end f383,
case when to_char(f384,'SS') = 2 then 'Квартал '||to_char(f384,'Q')
     when to_char(f384,'SS') = 3 then 'Год '||to_char(f384,'YYYY')
     else to_char(f384,'DD.MM.YYYY')
        end f384,
case when to_char(f385,'SS') = 2 then 'Квартал '||to_char(f385,'Q')
     when to_char(f385,'SS') = 3 then 'Год '||to_char(f385,'YYYY')
     else to_char(f385,'DD.MM.YYYY')
        end f385,
case when to_char(f386,'SS') = 2 then 'Квартал '||to_char(f386,'Q')
     when to_char(f386,'SS') = 3 then 'Год '||to_char(f386,'YYYY')
     else to_char(f386,'DD.MM.YYYY')
        end f386,
case when to_char(f387,'SS') = 2 then 'Квартал '||to_char(f387,'Q')
     when to_char(f387,'SS') = 3 then 'Год '||to_char(f387,'YYYY')
     else to_char(f387,'DD.MM.YYYY')
        end f387,
case when to_char(f388,'SS') = 2 then 'Квартал '||to_char(f388,'Q')
     when to_char(f388,'SS') = 3 then 'Год '||to_char(f388,'YYYY')
     else to_char(f388,'DD.MM.YYYY')
        end f388,
case when to_char(f389,'SS') = 2 then 'Квартал '||to_char(f389,'Q')
     when to_char(f389,'SS') = 3 then 'Год '||to_char(f389,'YYYY')
     else to_char(f389,'DD.MM.YYYY')
        end f389,
case when to_char(f390,'SS') = 2 then 'Квартал '||to_char(f390,'Q')
     when to_char(f390,'SS') = 3 then 'Год '||to_char(f390,'YYYY')
     else to_char(f390,'DD.MM.YYYY')
        end f390,
case when to_char(f391,'SS') = 2 then 'Квартал '||to_char(f391,'Q')
     when to_char(f391,'SS') = 3 then 'Год '||to_char(f391,'YYYY')
     else to_char(f391,'DD.MM.YYYY')
        end f391,
case when to_char(f392,'SS') = 2 then 'Квартал '||to_char(f392,'Q')
     when to_char(f392,'SS') = 3 then 'Год '||to_char(f392,'YYYY')
     else to_char(f392,'DD.MM.YYYY')
        end f392,
case when to_char(f393,'SS') = 2 then 'Квартал '||to_char(f393,'Q')
     when to_char(f393,'SS') = 3 then 'Год '||to_char(f393,'YYYY')
     else to_char(f393,'DD.MM.YYYY')
        end f393,
case when to_char(f394,'SS') = 2 then 'Квартал '||to_char(f394,'Q')
     when to_char(f394,'SS') = 3 then 'Год '||to_char(f394,'YYYY')
     else to_char(f394,'DD.MM.YYYY')
        end f394,
case when to_char(f395,'SS') = 2 then 'Квартал '||to_char(f395,'Q')
     when to_char(f395,'SS') = 3 then 'Год '||to_char(f395,'YYYY')
     else to_char(f395,'DD.MM.YYYY')
        end f395,
case when to_char(f396,'SS') = 2 then 'Квартал '||to_char(f396,'Q')
     when to_char(f396,'SS') = 3 then 'Год '||to_char(f396,'YYYY')
     else to_char(f396,'DD.MM.YYYY')
        end f396,
case when to_char(f397,'SS') = 2 then 'Квартал '||to_char(f397,'Q')
     when to_char(f397,'SS') = 3 then 'Год '||to_char(f397,'YYYY')
     else to_char(f397,'DD.MM.YYYY')
        end f397,
case when to_char(f398,'SS') = 2 then 'Квартал '||to_char(f398,'Q')
     when to_char(f398,'SS') = 3 then 'Год '||to_char(f398,'YYYY')
     else to_char(f398,'DD.MM.YYYY')
        end f398,
case when to_char(f399,'SS') = 2 then 'Квартал '||to_char(f399,'Q')
     when to_char(f399,'SS') = 3 then 'Год '||to_char(f399,'YYYY')
     else to_char(f399,'DD.MM.YYYY')
        end f399,
case when to_char(f400,'SS') = 2 then 'Квартал '||to_char(f400,'Q')
     when to_char(f400,'SS') = 3 then 'Год '||to_char(f400,'YYYY')
     else to_char(f400,'DD.MM.YYYY')
        end f400,
case when to_char(f401,'SS') = 2 then 'Квартал '||to_char(f401,'Q')
     when to_char(f401,'SS') = 3 then 'Год '||to_char(f401,'YYYY')
     else to_char(f401,'DD.MM.YYYY')
        end f401,
case when to_char(f402,'SS') = 2 then 'Квартал '||to_char(f402,'Q')
     when to_char(f402,'SS') = 3 then 'Год '||to_char(f402,'YYYY')
     else to_char(f402,'DD.MM.YYYY')
        end f402,
case when to_char(f403,'SS') = 2 then 'Квартал '||to_char(f403,'Q')
     when to_char(f403,'SS') = 3 then 'Год '||to_char(f403,'YYYY')
     else to_char(f403,'DD.MM.YYYY')
        end f403,
case when to_char(f404,'SS') = 2 then 'Квартал '||to_char(f404,'Q')
     when to_char(f404,'SS') = 3 then 'Год '||to_char(f404,'YYYY')
     else to_char(f404,'DD.MM.YYYY')
        end f404,
case when to_char(f405,'SS') = 2 then 'Квартал '||to_char(f405,'Q')
     when to_char(f405,'SS') = 3 then 'Год '||to_char(f405,'YYYY')
     else to_char(f405,'DD.MM.YYYY')
        end f405,
case when to_char(f406,'SS') = 2 then 'Квартал '||to_char(f406,'Q')
     when to_char(f406,'SS') = 3 then 'Год '||to_char(f406,'YYYY')
     else to_char(f406,'DD.MM.YYYY')
        end f406,
case when to_char(f407,'SS') = 2 then 'Квартал '||to_char(f407,'Q')
     when to_char(f407,'SS') = 3 then 'Год '||to_char(f407,'YYYY')
     else to_char(f407,'DD.MM.YYYY')
        end f407,
case when to_char(f408,'SS') = 2 then 'Квартал '||to_char(f408,'Q')
     when to_char(f408,'SS') = 3 then 'Год '||to_char(f408,'YYYY')
     else to_char(f408,'DD.MM.YYYY')
        end f408,
case when to_char(f409,'SS') = 2 then 'Квартал '||to_char(f409,'Q')
     when to_char(f409,'SS') = 3 then 'Год '||to_char(f409,'YYYY')
     else to_char(f409,'DD.MM.YYYY')
        end f409,
case when to_char(f410,'SS') = 2 then 'Квартал '||to_char(f410,'Q')
     when to_char(f410,'SS') = 3 then 'Год '||to_char(f410,'YYYY')
     else to_char(f410,'DD.MM.YYYY')
        end f410,
case when to_char(f411,'SS') = 2 then 'Квартал '||to_char(f411,'Q')
     when to_char(f411,'SS') = 3 then 'Год '||to_char(f411,'YYYY')
     else to_char(f411,'DD.MM.YYYY')
        end f411,
case when to_char(f412,'SS') = 2 then 'Квартал '||to_char(f412,'Q')
     when to_char(f412,'SS') = 3 then 'Год '||to_char(f412,'YYYY')
     else to_char(f412,'DD.MM.YYYY')
        end f412,
case when to_char(f413,'SS') = 2 then 'Квартал '||to_char(f413,'Q')
     when to_char(f413,'SS') = 3 then 'Год '||to_char(f413,'YYYY')
     else to_char(f413,'DD.MM.YYYY')
        end f413,
case when to_char(f414,'SS') = 2 then 'Квартал '||to_char(f414,'Q')
     when to_char(f414,'SS') = 3 then 'Год '||to_char(f414,'YYYY')
     else to_char(f414,'DD.MM.YYYY')
        end f414,
case when to_char(f415,'SS') = 2 then 'Квартал '||to_char(f415,'Q')
     when to_char(f415,'SS') = 3 then 'Год '||to_char(f415,'YYYY')
     else to_char(f415,'DD.MM.YYYY')
        end f415,
case when to_char(f416,'SS') = 2 then 'Квартал '||to_char(f416,'Q')
     when to_char(f416,'SS') = 3 then 'Год '||to_char(f416,'YYYY')
     else to_char(f416,'DD.MM.YYYY')
        end f416,
case when to_char(f417,'SS') = 2 then 'Квартал '||to_char(f417,'Q')
     when to_char(f417,'SS') = 3 then 'Год '||to_char(f417,'YYYY')
     else to_char(f417,'DD.MM.YYYY')
        end f417,
case when to_char(f418,'SS') = 2 then 'Квартал '||to_char(f418,'Q')
     when to_char(f418,'SS') = 3 then 'Год '||to_char(f418,'YYYY')
     else to_char(f418,'DD.MM.YYYY')
        end f418,
case when to_char(f419,'SS') = 2 then 'Квартал '||to_char(f419,'Q')
     when to_char(f419,'SS') = 3 then 'Год '||to_char(f419,'YYYY')
     else to_char(f419,'DD.MM.YYYY')
        end f419,
case when to_char(f420,'SS') = 2 then 'Квартал '||to_char(f420,'Q')
     when to_char(f420,'SS') = 3 then 'Год '||to_char(f420,'YYYY')
     else to_char(f420,'DD.MM.YYYY')
        end f420,
case when to_char(f421,'SS') = 2 then 'Квартал '||to_char(f421,'Q')
     when to_char(f421,'SS') = 3 then 'Год '||to_char(f421,'YYYY')
     else to_char(f421,'DD.MM.YYYY')
        end f421,
case when to_char(f422,'SS') = 2 then 'Квартал '||to_char(f422,'Q')
     when to_char(f422,'SS') = 3 then 'Год '||to_char(f422,'YYYY')
     else to_char(f422,'DD.MM.YYYY')
        end f422,
case when to_char(f423,'SS') = 2 then 'Квартал '||to_char(f423,'Q')
     when to_char(f423,'SS') = 3 then 'Год '||to_char(f423,'YYYY')
     else to_char(f423,'DD.MM.YYYY')
        end f423,
case when to_char(f424,'SS') = 2 then 'Квартал '||to_char(f424,'Q')
     when to_char(f424,'SS') = 3 then 'Год '||to_char(f424,'YYYY')
     else to_char(f424,'DD.MM.YYYY')
        end f424,
case when to_char(f425,'SS') = 2 then 'Квартал '||to_char(f425,'Q')
     when to_char(f425,'SS') = 3 then 'Год '||to_char(f425,'YYYY')
     else to_char(f425,'DD.MM.YYYY')
        end f425,
case when to_char(f426,'SS') = 2 then 'Квартал '||to_char(f426,'Q')
     when to_char(f426,'SS') = 3 then 'Год '||to_char(f426,'YYYY')
     else to_char(f426,'DD.MM.YYYY')
        end f426,
case when to_char(f427,'SS') = 2 then 'Квартал '||to_char(f427,'Q')
     when to_char(f427,'SS') = 3 then 'Год '||to_char(f427,'YYYY')
     else to_char(f427,'DD.MM.YYYY')
        end f427,
case when to_char(f428,'SS') = 2 then 'Квартал '||to_char(f428,'Q')
     when to_char(f428,'SS') = 3 then 'Год '||to_char(f428,'YYYY')
     else to_char(f428,'DD.MM.YYYY')
        end f428,
case when to_char(f429,'SS') = 2 then 'Квартал '||to_char(f429,'Q')
     when to_char(f429,'SS') = 3 then 'Год '||to_char(f429,'YYYY')
     else to_char(f429,'DD.MM.YYYY')
        end f429,
case when to_char(f430,'SS') = 2 then 'Квартал '||to_char(f430,'Q')
     when to_char(f430,'SS') = 3 then 'Год '||to_char(f430,'YYYY')
     else to_char(f430,'DD.MM.YYYY')
        end f430,
case when to_char(f431,'SS') = 2 then 'Квартал '||to_char(f431,'Q')
     when to_char(f431,'SS') = 3 then 'Год '||to_char(f431,'YYYY')
     else to_char(f431,'DD.MM.YYYY')
        end f431,
case when to_char(f432,'SS') = 2 then 'Квартал '||to_char(f432,'Q')
     when to_char(f432,'SS') = 3 then 'Год '||to_char(f432,'YYYY')
     else to_char(f432,'DD.MM.YYYY')
        end f432,
case when to_char(f433,'SS') = 2 then 'Квартал '||to_char(f433,'Q')
     when to_char(f433,'SS') = 3 then 'Год '||to_char(f433,'YYYY')
     else to_char(f433,'DD.MM.YYYY')
        end f433,
case when to_char(f434,'SS') = 2 then 'Квартал '||to_char(f434,'Q')
     when to_char(f434,'SS') = 3 then 'Год '||to_char(f434,'YYYY')
     else to_char(f434,'DD.MM.YYYY')
        end f434,
case when to_char(f435,'SS') = 2 then 'Квартал '||to_char(f435,'Q')
     when to_char(f435,'SS') = 3 then 'Год '||to_char(f435,'YYYY')
     else to_char(f435,'DD.MM.YYYY')
        end f435,
case when to_char(f436,'SS') = 2 then 'Квартал '||to_char(f436,'Q')
     when to_char(f436,'SS') = 3 then 'Год '||to_char(f436,'YYYY')
     else to_char(f436,'DD.MM.YYYY')
        end f436,
case when to_char(f437,'SS') = 2 then 'Квартал '||to_char(f437,'Q')
     when to_char(f437,'SS') = 3 then 'Год '||to_char(f437,'YYYY')
     else to_char(f437,'DD.MM.YYYY')
        end f437,
case when to_char(f438,'SS') = 2 then 'Квартал '||to_char(f438,'Q')
     when to_char(f438,'SS') = 3 then 'Год '||to_char(f438,'YYYY')
     else to_char(f438,'DD.MM.YYYY')
        end f438,
case when to_char(f439,'SS') = 2 then 'Квартал '||to_char(f439,'Q')
     when to_char(f439,'SS') = 3 then 'Год '||to_char(f439,'YYYY')
     else to_char(f439,'DD.MM.YYYY')
        end f439,
case when to_char(f440,'SS') = 2 then 'Квартал '||to_char(f440,'Q')
     when to_char(f440,'SS') = 3 then 'Год '||to_char(f440,'YYYY')
     else to_char(f440,'DD.MM.YYYY')
        end f440,
case when to_char(f441,'SS') = 2 then 'Квартал '||to_char(f441,'Q')
     when to_char(f441,'SS') = 3 then 'Год '||to_char(f441,'YYYY')
     else to_char(f441,'DD.MM.YYYY')
        end f441,
case when to_char(f442,'SS') = 2 then 'Квартал '||to_char(f442,'Q')
     when to_char(f442,'SS') = 3 then 'Год '||to_char(f442,'YYYY')
     else to_char(f442,'DD.MM.YYYY')
        end f442,
case when to_char(f443,'SS') = 2 then 'Квартал '||to_char(f443,'Q')
     when to_char(f443,'SS') = 3 then 'Год '||to_char(f443,'YYYY')
     else to_char(f443,'DD.MM.YYYY')
        end f443,
case when to_char(f444,'SS') = 2 then 'Квартал '||to_char(f444,'Q')
     when to_char(f444,'SS') = 3 then 'Год '||to_char(f444,'YYYY')
     else to_char(f444,'DD.MM.YYYY')
        end f444,
case when to_char(f445,'SS') = 2 then 'Квартал '||to_char(f445,'Q')
     when to_char(f445,'SS') = 3 then 'Год '||to_char(f445,'YYYY')
     else to_char(f445,'DD.MM.YYYY')
        end f445,
case when to_char(f446,'SS') = 2 then 'Квартал '||to_char(f446,'Q')
     when to_char(f446,'SS') = 3 then 'Год '||to_char(f446,'YYYY')
     else to_char(f446,'DD.MM.YYYY')
        end f446,
case when to_char(f447,'SS') = 2 then 'Квартал '||to_char(f447,'Q')
     when to_char(f447,'SS') = 3 then 'Год '||to_char(f447,'YYYY')
     else to_char(f447,'DD.MM.YYYY')
        end f447,
case when to_char(f448,'SS') = 2 then 'Квартал '||to_char(f448,'Q')
     when to_char(f448,'SS') = 3 then 'Год '||to_char(f448,'YYYY')
     else to_char(f448,'DD.MM.YYYY')
        end f448,
case when to_char(f449,'SS') = 2 then 'Квартал '||to_char(f449,'Q')
     when to_char(f449,'SS') = 3 then 'Год '||to_char(f449,'YYYY')
     else to_char(f449,'DD.MM.YYYY')
        end f449,
case when to_char(f450,'SS') = 2 then 'Квартал '||to_char(f450,'Q')
     when to_char(f450,'SS') = 3 then 'Год '||to_char(f450,'YYYY')
     else to_char(f450,'DD.MM.YYYY')
        end f450,
case when to_char(f451,'SS') = 2 then 'Квартал '||to_char(f451,'Q')
     when to_char(f451,'SS') = 3 then 'Год '||to_char(f451,'YYYY')
     else to_char(f451,'DD.MM.YYYY')
        end f451,
case when to_char(f452,'SS') = 2 then 'Квартал '||to_char(f452,'Q')
     when to_char(f452,'SS') = 3 then 'Год '||to_char(f452,'YYYY')
     else to_char(f452,'DD.MM.YYYY')
        end f452,
case when to_char(f453,'SS') = 2 then 'Квартал '||to_char(f453,'Q')
     when to_char(f453,'SS') = 3 then 'Год '||to_char(f453,'YYYY')
     else to_char(f453,'DD.MM.YYYY')
        end f453,
case when to_char(f454,'SS') = 2 then 'Квартал '||to_char(f454,'Q')
     when to_char(f454,'SS') = 3 then 'Год '||to_char(f454,'YYYY')
     else to_char(f454,'DD.MM.YYYY')
        end f454,
case when to_char(f455,'SS') = 2 then 'Квартал '||to_char(f455,'Q')
     when to_char(f455,'SS') = 3 then 'Год '||to_char(f455,'YYYY')
     else to_char(f455,'DD.MM.YYYY')
        end f455,
case when to_char(f456,'SS') = 2 then 'Квартал '||to_char(f456,'Q')
     when to_char(f456,'SS') = 3 then 'Год '||to_char(f456,'YYYY')
     else to_char(f456,'DD.MM.YYYY')
        end f456,
case when to_char(f457,'SS') = 2 then 'Квартал '||to_char(f457,'Q')
     when to_char(f457,'SS') = 3 then 'Год '||to_char(f457,'YYYY')
     else to_char(f457,'DD.MM.YYYY')
        end f457,
case when to_char(f458,'SS') = 2 then 'Квартал '||to_char(f458,'Q')
     when to_char(f458,'SS') = 3 then 'Год '||to_char(f458,'YYYY')
     else to_char(f458,'DD.MM.YYYY')
        end f458,
case when to_char(f459,'SS') = 2 then 'Квартал '||to_char(f459,'Q')
     when to_char(f459,'SS') = 3 then 'Год '||to_char(f459,'YYYY')
     else to_char(f459,'DD.MM.YYYY')
        end f459,
case when to_char(f460,'SS') = 2 then 'Квартал '||to_char(f460,'Q')
     when to_char(f460,'SS') = 3 then 'Год '||to_char(f460,'YYYY')
     else to_char(f460,'DD.MM.YYYY')
        end f460,
case when to_char(f461,'SS') = 2 then 'Квартал '||to_char(f461,'Q')
     when to_char(f461,'SS') = 3 then 'Год '||to_char(f461,'YYYY')
     else to_char(f461,'DD.MM.YYYY')
        end f461,
case when to_char(f462,'SS') = 2 then 'Квартал '||to_char(f462,'Q')
     when to_char(f462,'SS') = 3 then 'Год '||to_char(f462,'YYYY')
     else to_char(f462,'DD.MM.YYYY')
        end f462,
case when to_char(f463,'SS') = 2 then 'Квартал '||to_char(f463,'Q')
     when to_char(f463,'SS') = 3 then 'Год '||to_char(f463,'YYYY')
     else to_char(f463,'DD.MM.YYYY')
        end f463,
case when to_char(f464,'SS') = 2 then 'Квартал '||to_char(f464,'Q')
     when to_char(f464,'SS') = 3 then 'Год '||to_char(f464,'YYYY')
     else to_char(f464,'DD.MM.YYYY')
        end f464,
case when to_char(f465,'SS') = 2 then 'Квартал '||to_char(f465,'Q')
     when to_char(f465,'SS') = 3 then 'Год '||to_char(f465,'YYYY')
     else to_char(f465,'DD.MM.YYYY')
        end f465,
case when to_char(f466,'SS') = 2 then 'Квартал '||to_char(f466,'Q')
     when to_char(f466,'SS') = 3 then 'Год '||to_char(f466,'YYYY')
     else to_char(f466,'DD.MM.YYYY')
        end f466,
case when to_char(f467,'SS') = 2 then 'Квартал '||to_char(f467,'Q')
     when to_char(f467,'SS') = 3 then 'Год '||to_char(f467,'YYYY')
     else to_char(f467,'DD.MM.YYYY')
        end f467,
case when to_char(f468,'SS') = 2 then 'Квартал '||to_char(f468,'Q')
     when to_char(f468,'SS') = 3 then 'Год '||to_char(f468,'YYYY')
     else to_char(f468,'DD.MM.YYYY')
        end f468,
case when to_char(f469,'SS') = 2 then 'Квартал '||to_char(f469,'Q')
     when to_char(f469,'SS') = 3 then 'Год '||to_char(f469,'YYYY')
     else to_char(f469,'DD.MM.YYYY')
        end f469,
case when to_char(f470,'SS') = 2 then 'Квартал '||to_char(f470,'Q')
     when to_char(f470,'SS') = 3 then 'Год '||to_char(f470,'YYYY')
     else to_char(f470,'DD.MM.YYYY')
        end f470,
case when to_char(f471,'SS') = 2 then 'Квартал '||to_char(f471,'Q')
     when to_char(f471,'SS') = 3 then 'Год '||to_char(f471,'YYYY')
     else to_char(f471,'DD.MM.YYYY')
        end f471,
case when to_char(f472,'SS') = 2 then 'Квартал '||to_char(f472,'Q')
     when to_char(f472,'SS') = 3 then 'Год '||to_char(f472,'YYYY')
     else to_char(f472,'DD.MM.YYYY')
        end f472,
case when to_char(f473,'SS') = 2 then 'Квартал '||to_char(f473,'Q')
     when to_char(f473,'SS') = 3 then 'Год '||to_char(f473,'YYYY')
     else to_char(f473,'DD.MM.YYYY')
        end f473,
case when to_char(f474,'SS') = 2 then 'Квартал '||to_char(f474,'Q')
     when to_char(f474,'SS') = 3 then 'Год '||to_char(f474,'YYYY')
     else to_char(f474,'DD.MM.YYYY')
        end f474,
case when to_char(f475,'SS') = 2 then 'Квартал '||to_char(f475,'Q')
     when to_char(f475,'SS') = 3 then 'Год '||to_char(f475,'YYYY')
     else to_char(f475,'DD.MM.YYYY')
        end f475,
case when to_char(f476,'SS') = 2 then 'Квартал '||to_char(f476,'Q')
     when to_char(f476,'SS') = 3 then 'Год '||to_char(f476,'YYYY')
     else to_char(f476,'DD.MM.YYYY')
        end f476,
case when to_char(f477,'SS') = 2 then 'Квартал '||to_char(f477,'Q')
     when to_char(f477,'SS') = 3 then 'Год '||to_char(f477,'YYYY')
     else to_char(f477,'DD.MM.YYYY')
        end f477,
case when to_char(f478,'SS') = 2 then 'Квартал '||to_char(f478,'Q')
     when to_char(f478,'SS') = 3 then 'Год '||to_char(f478,'YYYY')
     else to_char(f478,'DD.MM.YYYY')
        end f478,
case when to_char(f479,'SS') = 2 then 'Квартал '||to_char(f479,'Q')
     when to_char(f479,'SS') = 3 then 'Год '||to_char(f479,'YYYY')
     else to_char(f479,'DD.MM.YYYY')
        end f479,
case when to_char(f480,'SS') = 2 then 'Квартал '||to_char(f480,'Q')
     when to_char(f480,'SS') = 3 then 'Год '||to_char(f480,'YYYY')
     else to_char(f480,'DD.MM.YYYY')
        end f480,
case when to_char(f481,'SS') = 2 then 'Квартал '||to_char(f481,'Q')
     when to_char(f481,'SS') = 3 then 'Год '||to_char(f481,'YYYY')
     else to_char(f481,'DD.MM.YYYY')
        end f481,
case when to_char(f482,'SS') = 2 then 'Квартал '||to_char(f482,'Q')
     when to_char(f482,'SS') = 3 then 'Год '||to_char(f482,'YYYY')
     else to_char(f482,'DD.MM.YYYY')
        end f482,
case when to_char(f483,'SS') = 2 then 'Квартал '||to_char(f483,'Q')
     when to_char(f483,'SS') = 3 then 'Год '||to_char(f483,'YYYY')
     else to_char(f483,'DD.MM.YYYY')
        end f483,
case when to_char(f484,'SS') = 2 then 'Квартал '||to_char(f484,'Q')
     when to_char(f484,'SS') = 3 then 'Год '||to_char(f484,'YYYY')
     else to_char(f484,'DD.MM.YYYY')
        end f484,
case when to_char(f485,'SS') = 2 then 'Квартал '||to_char(f485,'Q')
     when to_char(f485,'SS') = 3 then 'Год '||to_char(f485,'YYYY')
     else to_char(f485,'DD.MM.YYYY')
        end f485,
case when to_char(f486,'SS') = 2 then 'Квартал '||to_char(f486,'Q')
     when to_char(f486,'SS') = 3 then 'Год '||to_char(f486,'YYYY')
     else to_char(f486,'DD.MM.YYYY')
        end f486,
case when to_char(f487,'SS') = 2 then 'Квартал '||to_char(f487,'Q')
     when to_char(f487,'SS') = 3 then 'Год '||to_char(f487,'YYYY')
     else to_char(f487,'DD.MM.YYYY')
        end f487,
case when to_char(f488,'SS') = 2 then 'Квартал '||to_char(f488,'Q')
     when to_char(f488,'SS') = 3 then 'Год '||to_char(f488,'YYYY')
     else to_char(f488,'DD.MM.YYYY')
        end f488,
case when to_char(f489,'SS') = 2 then 'Квартал '||to_char(f489,'Q')
     when to_char(f489,'SS') = 3 then 'Год '||to_char(f489,'YYYY')
     else to_char(f489,'DD.MM.YYYY')
        end f489,
case when to_char(f490,'SS') = 2 then 'Квартал '||to_char(f490,'Q')
     when to_char(f490,'SS') = 3 then 'Год '||to_char(f490,'YYYY')
     else to_char(f490,'DD.MM.YYYY')
        end f490,
case when to_char(f491,'SS') = 2 then 'Квартал '||to_char(f491,'Q')
     when to_char(f491,'SS') = 3 then 'Год '||to_char(f491,'YYYY')
     else to_char(f491,'DD.MM.YYYY')
        end f491,
case when to_char(f492,'SS') = 2 then 'Квартал '||to_char(f492,'Q')
     when to_char(f492,'SS') = 3 then 'Год '||to_char(f492,'YYYY')
     else to_char(f492,'DD.MM.YYYY')
        end f492,
case when to_char(f493,'SS') = 2 then 'Квартал '||to_char(f493,'Q')
     when to_char(f493,'SS') = 3 then 'Год '||to_char(f493,'YYYY')
     else to_char(f493,'DD.MM.YYYY')
        end f493,
case when to_char(f494,'SS') = 2 then 'Квартал '||to_char(f494,'Q')
     when to_char(f494,'SS') = 3 then 'Год '||to_char(f494,'YYYY')
     else to_char(f494,'DD.MM.YYYY')
        end f494,
case when to_char(f495,'SS') = 2 then 'Квартал '||to_char(f495,'Q')
     when to_char(f495,'SS') = 3 then 'Год '||to_char(f495,'YYYY')
     else to_char(f495,'DD.MM.YYYY')
        end f495,
case when to_char(f496,'SS') = 2 then 'Квартал '||to_char(f496,'Q')
     when to_char(f496,'SS') = 3 then 'Год '||to_char(f496,'YYYY')
     else to_char(f496,'DD.MM.YYYY')
        end f496,
case when to_char(f497,'SS') = 2 then 'Квартал '||to_char(f497,'Q')
     when to_char(f497,'SS') = 3 then 'Год '||to_char(f497,'YYYY')
     else to_char(f497,'DD.MM.YYYY')
        end f497,
case when to_char(f498,'SS') = 2 then 'Квартал '||to_char(f498,'Q')
     when to_char(f498,'SS') = 3 then 'Год '||to_char(f498,'YYYY')
     else to_char(f498,'DD.MM.YYYY')
        end f498,
case when to_char(f499,'SS') = 2 then 'Квартал '||to_char(f499,'Q')
     when to_char(f499,'SS') = 3 then 'Год '||to_char(f499,'YYYY')
     else to_char(f499,'DD.MM.YYYY')
        end f499,
case when to_char(f500,'SS') = 2 then 'Квартал '||to_char(f500,'Q')
     when to_char(f500,'SS') = 3 then 'Год '||to_char(f500,'YYYY')
     else to_char(f500,'DD.MM.YYYY')
        end f500,
case when to_char(f501,'SS') = 2 then 'Квартал '||to_char(f501,'Q')
     when to_char(f501,'SS') = 3 then 'Год '||to_char(f501,'YYYY')
     else to_char(f501,'DD.MM.YYYY')
        end f501,
case when to_char(f502,'SS') = 2 then 'Квартал '||to_char(f502,'Q')
     when to_char(f502,'SS') = 3 then 'Год '||to_char(f502,'YYYY')
     else to_char(f502,'DD.MM.YYYY')
        end f502,
case when to_char(f503,'SS') = 2 then 'Квартал '||to_char(f503,'Q')
     when to_char(f503,'SS') = 3 then 'Год '||to_char(f503,'YYYY')
     else to_char(f503,'DD.MM.YYYY')
        end f503,
case when to_char(f504,'SS') = 2 then 'Квартал '||to_char(f504,'Q')
     when to_char(f504,'SS') = 3 then 'Год '||to_char(f504,'YYYY')
     else to_char(f504,'DD.MM.YYYY')
        end f504,
case when to_char(f505,'SS') = 2 then 'Квартал '||to_char(f505,'Q')
     when to_char(f505,'SS') = 3 then 'Год '||to_char(f505,'YYYY')
     else to_char(f505,'DD.MM.YYYY')
        end f505,
case when to_char(f506,'SS') = 2 then 'Квартал '||to_char(f506,'Q')
     when to_char(f506,'SS') = 3 then 'Год '||to_char(f506,'YYYY')
     else to_char(f506,'DD.MM.YYYY')
        end f506,
case when to_char(f507,'SS') = 2 then 'Квартал '||to_char(f507,'Q')
     when to_char(f507,'SS') = 3 then 'Год '||to_char(f507,'YYYY')
     else to_char(f507,'DD.MM.YYYY')
        end f507,
case when to_char(f508,'SS') = 2 then 'Квартал '||to_char(f508,'Q')
     when to_char(f508,'SS') = 3 then 'Год '||to_char(f508,'YYYY')
     else to_char(f508,'DD.MM.YYYY')
        end f508,
case when to_char(f509,'SS') = 2 then 'Квартал '||to_char(f509,'Q')
     when to_char(f509,'SS') = 3 then 'Год '||to_char(f509,'YYYY')
     else to_char(f509,'DD.MM.YYYY')
        end f509,
case when to_char(f510,'SS') = 2 then 'Квартал '||to_char(f510,'Q')
     when to_char(f510,'SS') = 3 then 'Год '||to_char(f510,'YYYY')
     else to_char(f510,'DD.MM.YYYY')
        end f510,
case when to_char(f511,'SS') = 2 then 'Квартал '||to_char(f511,'Q')
     when to_char(f511,'SS') = 3 then 'Год '||to_char(f511,'YYYY')
     else to_char(f511,'DD.MM.YYYY')
        end f511,
case when to_char(f512,'SS') = 2 then 'Квартал '||to_char(f512,'Q')
     when to_char(f512,'SS') = 3 then 'Год '||to_char(f512,'YYYY')
     else to_char(f512,'DD.MM.YYYY')
        end f512,
case when to_char(f513,'SS') = 2 then 'Квартал '||to_char(f513,'Q')
     when to_char(f513,'SS') = 3 then 'Год '||to_char(f513,'YYYY')
     else to_char(f513,'DD.MM.YYYY')
        end f513,
case when to_char(f514,'SS') = 2 then 'Квартал '||to_char(f514,'Q')
     when to_char(f514,'SS') = 3 then 'Год '||to_char(f514,'YYYY')
     else to_char(f514,'DD.MM.YYYY')
        end f514,
case when to_char(f515,'SS') = 2 then 'Квартал '||to_char(f515,'Q')
     when to_char(f515,'SS') = 3 then 'Год '||to_char(f515,'YYYY')
     else to_char(f515,'DD.MM.YYYY')
        end f515,
case when to_char(f516,'SS') = 2 then 'Квартал '||to_char(f516,'Q')
     when to_char(f516,'SS') = 3 then 'Год '||to_char(f516,'YYYY')
     else to_char(f516,'DD.MM.YYYY')
        end f516,
case when to_char(f517,'SS') = 2 then 'Квартал '||to_char(f517,'Q')
     when to_char(f517,'SS') = 3 then 'Год '||to_char(f517,'YYYY')
     else to_char(f517,'DD.MM.YYYY')
        end f517,
case when to_char(f518,'SS') = 2 then 'Квартал '||to_char(f518,'Q')
     when to_char(f518,'SS') = 3 then 'Год '||to_char(f518,'YYYY')
     else to_char(f518,'DD.MM.YYYY')
        end f518,
case when to_char(f519,'SS') = 2 then 'Квартал '||to_char(f519,'Q')
     when to_char(f519,'SS') = 3 then 'Год '||to_char(f519,'YYYY')
     else to_char(f519,'DD.MM.YYYY')
        end f519,
case when to_char(f520,'SS') = 2 then 'Квартал '||to_char(f520,'Q')
     when to_char(f520,'SS') = 3 then 'Год '||to_char(f520,'YYYY')
     else to_char(f520,'DD.MM.YYYY')
        end f520,
case when to_char(f521,'SS') = 2 then 'Квартал '||to_char(f521,'Q')
     when to_char(f521,'SS') = 3 then 'Год '||to_char(f521,'YYYY')
     else to_char(f521,'DD.MM.YYYY')
        end f521,
case when to_char(f522,'SS') = 2 then 'Квартал '||to_char(f522,'Q')
     when to_char(f522,'SS') = 3 then 'Год '||to_char(f522,'YYYY')
     else to_char(f522,'DD.MM.YYYY')
        end f522,
case when to_char(f523,'SS') = 2 then 'Квартал '||to_char(f523,'Q')
     when to_char(f523,'SS') = 3 then 'Год '||to_char(f523,'YYYY')
     else to_char(f523,'DD.MM.YYYY')
        end f523,
case when to_char(f524,'SS') = 2 then 'Квартал '||to_char(f524,'Q')
     when to_char(f524,'SS') = 3 then 'Год '||to_char(f524,'YYYY')
     else to_char(f524,'DD.MM.YYYY')
        end f524,
case when to_char(f525,'SS') = 2 then 'Квартал '||to_char(f525,'Q')
     when to_char(f525,'SS') = 3 then 'Год '||to_char(f525,'YYYY')
     else to_char(f525,'DD.MM.YYYY')
        end f525,
case when to_char(f526,'SS') = 2 then 'Квартал '||to_char(f526,'Q')
     when to_char(f526,'SS') = 3 then 'Год '||to_char(f526,'YYYY')
     else to_char(f526,'DD.MM.YYYY')
        end f526,
case when to_char(f527,'SS') = 2 then 'Квартал '||to_char(f527,'Q')
     when to_char(f527,'SS') = 3 then 'Год '||to_char(f527,'YYYY')
     else to_char(f527,'DD.MM.YYYY')
        end f527,
case when to_char(f528,'SS') = 2 then 'Квартал '||to_char(f528,'Q')
     when to_char(f528,'SS') = 3 then 'Год '||to_char(f528,'YYYY')
     else to_char(f528,'DD.MM.YYYY')
        end f528,
case when to_char(f529,'SS') = 2 then 'Квартал '||to_char(f529,'Q')
     when to_char(f529,'SS') = 3 then 'Год '||to_char(f529,'YYYY')
     else to_char(f529,'DD.MM.YYYY')
        end f529,
case when to_char(f530,'SS') = 2 then 'Квартал '||to_char(f530,'Q')
     when to_char(f530,'SS') = 3 then 'Год '||to_char(f530,'YYYY')
     else to_char(f530,'DD.MM.YYYY')
        end f530,
case when to_char(f531,'SS') = 2 then 'Квартал '||to_char(f531,'Q')
     when to_char(f531,'SS') = 3 then 'Год '||to_char(f531,'YYYY')
     else to_char(f531,'DD.MM.YYYY')
        end f531,
case when to_char(f532,'SS') = 2 then 'Квартал '||to_char(f532,'Q')
     when to_char(f532,'SS') = 3 then 'Год '||to_char(f532,'YYYY')
     else to_char(f532,'DD.MM.YYYY')
        end f532,
case when to_char(f533,'SS') = 2 then 'Квартал '||to_char(f533,'Q')
     when to_char(f533,'SS') = 3 then 'Год '||to_char(f533,'YYYY')
     else to_char(f533,'DD.MM.YYYY')
        end f533,
case when to_char(f534,'SS') = 2 then 'Квартал '||to_char(f534,'Q')
     when to_char(f534,'SS') = 3 then 'Год '||to_char(f534,'YYYY')
     else to_char(f534,'DD.MM.YYYY')
        end f534,
case when to_char(f535,'SS') = 2 then 'Квартал '||to_char(f535,'Q')
     when to_char(f535,'SS') = 3 then 'Год '||to_char(f535,'YYYY')
     else to_char(f535,'DD.MM.YYYY')
        end f535,
case when to_char(f536,'SS') = 2 then 'Квартал '||to_char(f536,'Q')
     when to_char(f536,'SS') = 3 then 'Год '||to_char(f536,'YYYY')
     else to_char(f536,'DD.MM.YYYY')
        end f536,
case when to_char(f537,'SS') = 2 then 'Квартал '||to_char(f537,'Q')
     when to_char(f537,'SS') = 3 then 'Год '||to_char(f537,'YYYY')
     else to_char(f537,'DD.MM.YYYY')
        end f537,
case when to_char(f538,'SS') = 2 then 'Квартал '||to_char(f538,'Q')
     when to_char(f538,'SS') = 3 then 'Год '||to_char(f538,'YYYY')
     else to_char(f538,'DD.MM.YYYY')
        end f538,
case when to_char(f539,'SS') = 2 then 'Квартал '||to_char(f539,'Q')
     when to_char(f539,'SS') = 3 then 'Год '||to_char(f539,'YYYY')
     else to_char(f539,'DD.MM.YYYY')
        end f539,
case when to_char(f540,'SS') = 2 then 'Квартал '||to_char(f540,'Q')
     when to_char(f540,'SS') = 3 then 'Год '||to_char(f540,'YYYY')
     else to_char(f540,'DD.MM.YYYY')
        end f540,
case when to_char(f541,'SS') = 2 then 'Квартал '||to_char(f541,'Q')
     when to_char(f541,'SS') = 3 then 'Год '||to_char(f541,'YYYY')
     else to_char(f541,'DD.MM.YYYY')
        end f541,
case when to_char(f542,'SS') = 2 then 'Квартал '||to_char(f542,'Q')
     when to_char(f542,'SS') = 3 then 'Год '||to_char(f542,'YYYY')
     else to_char(f542,'DD.MM.YYYY')
        end f542,
case when to_char(f543,'SS') = 2 then 'Квартал '||to_char(f543,'Q')
     when to_char(f543,'SS') = 3 then 'Год '||to_char(f543,'YYYY')
     else to_char(f543,'DD.MM.YYYY')
        end f543,
case when to_char(f544,'SS') = 2 then 'Квартал '||to_char(f544,'Q')
     when to_char(f544,'SS') = 3 then 'Год '||to_char(f544,'YYYY')
     else to_char(f544,'DD.MM.YYYY')
        end f544,
case when to_char(f545,'SS') = 2 then 'Квартал '||to_char(f545,'Q')
     when to_char(f545,'SS') = 3 then 'Год '||to_char(f545,'YYYY')
     else to_char(f545,'DD.MM.YYYY')
        end f545,
case when to_char(f546,'SS') = 2 then 'Квартал '||to_char(f546,'Q')
     when to_char(f546,'SS') = 3 then 'Год '||to_char(f546,'YYYY')
     else to_char(f546,'DD.MM.YYYY')
        end f546,
case when to_char(f547,'SS') = 2 then 'Квартал '||to_char(f547,'Q')
     when to_char(f547,'SS') = 3 then 'Год '||to_char(f547,'YYYY')
     else to_char(f547,'DD.MM.YYYY')
        end f547,
case when to_char(f548,'SS') = 2 then 'Квартал '||to_char(f548,'Q')
     when to_char(f548,'SS') = 3 then 'Год '||to_char(f548,'YYYY')
     else to_char(f548,'DD.MM.YYYY')
        end f548,
case when to_char(f549,'SS') = 2 then 'Квартал '||to_char(f549,'Q')
     when to_char(f549,'SS') = 3 then 'Год '||to_char(f549,'YYYY')
     else to_char(f549,'DD.MM.YYYY')
        end f549,
case when to_char(f550,'SS') = 2 then 'Квартал '||to_char(f550,'Q')
     when to_char(f550,'SS') = 3 then 'Год '||to_char(f550,'YYYY')
     else to_char(f550,'DD.MM.YYYY')
        end f550,
case when to_char(f551,'SS') = 2 then 'Квартал '||to_char(f551,'Q')
     when to_char(f551,'SS') = 3 then 'Год '||to_char(f551,'YYYY')
     else to_char(f551,'DD.MM.YYYY')
        end f551,
case when to_char(f552,'SS') = 2 then 'Квартал '||to_char(f552,'Q')
     when to_char(f552,'SS') = 3 then 'Год '||to_char(f552,'YYYY')
     else to_char(f552,'DD.MM.YYYY')
        end f552,
case when to_char(f553,'SS') = 2 then 'Квартал '||to_char(f553,'Q')
     when to_char(f553,'SS') = 3 then 'Год '||to_char(f553,'YYYY')
     else to_char(f553,'DD.MM.YYYY')
        end f553,
case when to_char(f554,'SS') = 2 then 'Квартал '||to_char(f554,'Q')
     when to_char(f554,'SS') = 3 then 'Год '||to_char(f554,'YYYY')
     else to_char(f554,'DD.MM.YYYY')
        end f554,
case when to_char(f555,'SS') = 2 then 'Квартал '||to_char(f555,'Q')
     when to_char(f555,'SS') = 3 then 'Год '||to_char(f555,'YYYY')
     else to_char(f555,'DD.MM.YYYY')
        end f555,
case when to_char(f556,'SS') = 2 then 'Квартал '||to_char(f556,'Q')
     when to_char(f556,'SS') = 3 then 'Год '||to_char(f556,'YYYY')
     else to_char(f556,'DD.MM.YYYY')
        end f556,
case when to_char(f557,'SS') = 2 then 'Квартал '||to_char(f557,'Q')
     when to_char(f557,'SS') = 3 then 'Год '||to_char(f557,'YYYY')
     else to_char(f557,'DD.MM.YYYY')
        end f557,
case when to_char(f558,'SS') = 2 then 'Квартал '||to_char(f558,'Q')
     when to_char(f558,'SS') = 3 then 'Год '||to_char(f558,'YYYY')
     else to_char(f558,'DD.MM.YYYY')
        end f558,
case when to_char(f559,'SS') = 2 then 'Квартал '||to_char(f559,'Q')
     when to_char(f559,'SS') = 3 then 'Год '||to_char(f559,'YYYY')
     else to_char(f559,'DD.MM.YYYY')
        end f559,
case when to_char(f560,'SS') = 2 then 'Квартал '||to_char(f560,'Q')
     when to_char(f560,'SS') = 3 then 'Год '||to_char(f560,'YYYY')
     else to_char(f560,'DD.MM.YYYY')
        end f560,
case when to_char(f561,'SS') = 2 then 'Квартал '||to_char(f561,'Q')
     when to_char(f561,'SS') = 3 then 'Год '||to_char(f561,'YYYY')
     else to_char(f561,'DD.MM.YYYY')
        end f561,
case when to_char(f562,'SS') = 2 then 'Квартал '||to_char(f562,'Q')
     when to_char(f562,'SS') = 3 then 'Год '||to_char(f562,'YYYY')
     else to_char(f562,'DD.MM.YYYY')
        end f562,
case when to_char(f563,'SS') = 2 then 'Квартал '||to_char(f563,'Q')
     when to_char(f563,'SS') = 3 then 'Год '||to_char(f563,'YYYY')
     else to_char(f563,'DD.MM.YYYY')
        end f563,
case when to_char(f564,'SS') = 2 then 'Квартал '||to_char(f564,'Q')
     when to_char(f564,'SS') = 3 then 'Год '||to_char(f564,'YYYY')
     else to_char(f564,'DD.MM.YYYY')
        end f564,
case when to_char(f565,'SS') = 2 then 'Квартал '||to_char(f565,'Q')
     when to_char(f565,'SS') = 3 then 'Год '||to_char(f565,'YYYY')
     else to_char(f565,'DD.MM.YYYY')
        end f565,
case when to_char(f566,'SS') = 2 then 'Квартал '||to_char(f566,'Q')
     when to_char(f566,'SS') = 3 then 'Год '||to_char(f566,'YYYY')
     else to_char(f566,'DD.MM.YYYY')
        end f566,
case when to_char(f567,'SS') = 2 then 'Квартал '||to_char(f567,'Q')
     when to_char(f567,'SS') = 3 then 'Год '||to_char(f567,'YYYY')
     else to_char(f567,'DD.MM.YYYY')
        end f567,
case when to_char(f568,'SS') = 2 then 'Квартал '||to_char(f568,'Q')
     when to_char(f568,'SS') = 3 then 'Год '||to_char(f568,'YYYY')
     else to_char(f568,'DD.MM.YYYY')
        end f568,
case when to_char(f569,'SS') = 2 then 'Квартал '||to_char(f569,'Q')
     when to_char(f569,'SS') = 3 then 'Год '||to_char(f569,'YYYY')
     else to_char(f569,'DD.MM.YYYY')
        end f569,
case when to_char(f570,'SS') = 2 then 'Квартал '||to_char(f570,'Q')
     when to_char(f570,'SS') = 3 then 'Год '||to_char(f570,'YYYY')
     else to_char(f570,'DD.MM.YYYY')
        end f570,
case when to_char(f571,'SS') = 2 then 'Квартал '||to_char(f571,'Q')
     when to_char(f571,'SS') = 3 then 'Год '||to_char(f571,'YYYY')
     else to_char(f571,'DD.MM.YYYY')
        end f571,
case when to_char(f572,'SS') = 2 then 'Квартал '||to_char(f572,'Q')
     when to_char(f572,'SS') = 3 then 'Год '||to_char(f572,'YYYY')
     else to_char(f572,'DD.MM.YYYY')
        end f572,
case when to_char(f573,'SS') = 2 then 'Квартал '||to_char(f573,'Q')
     when to_char(f573,'SS') = 3 then 'Год '||to_char(f573,'YYYY')
     else to_char(f573,'DD.MM.YYYY')
        end f573,
case when to_char(f574,'SS') = 2 then 'Квартал '||to_char(f574,'Q')
     when to_char(f574,'SS') = 3 then 'Год '||to_char(f574,'YYYY')
     else to_char(f574,'DD.MM.YYYY')
        end f574,
case when to_char(f575,'SS') = 2 then 'Квартал '||to_char(f575,'Q')
     when to_char(f575,'SS') = 3 then 'Год '||to_char(f575,'YYYY')
     else to_char(f575,'DD.MM.YYYY')
        end f575,
case when to_char(f576,'SS') = 2 then 'Квартал '||to_char(f576,'Q')
     when to_char(f576,'SS') = 3 then 'Год '||to_char(f576,'YYYY')
     else to_char(f576,'DD.MM.YYYY')
        end f576,
case when to_char(f577,'SS') = 2 then 'Квартал '||to_char(f577,'Q')
     when to_char(f577,'SS') = 3 then 'Год '||to_char(f577,'YYYY')
     else to_char(f577,'DD.MM.YYYY')
        end f577,
case when to_char(f578,'SS') = 2 then 'Квартал '||to_char(f578,'Q')
     when to_char(f578,'SS') = 3 then 'Год '||to_char(f578,'YYYY')
     else to_char(f578,'DD.MM.YYYY')
        end f578,
case when to_char(f579,'SS') = 2 then 'Квартал '||to_char(f579,'Q')
     when to_char(f579,'SS') = 3 then 'Год '||to_char(f579,'YYYY')
     else to_char(f579,'DD.MM.YYYY')
        end f579,
case when to_char(f580,'SS') = 2 then 'Квартал '||to_char(f580,'Q')
     when to_char(f580,'SS') = 3 then 'Год '||to_char(f580,'YYYY')
     else to_char(f580,'DD.MM.YYYY')
        end f580,
case when to_char(f581,'SS') = 2 then 'Квартал '||to_char(f581,'Q')
     when to_char(f581,'SS') = 3 then 'Год '||to_char(f581,'YYYY')
     else to_char(f581,'DD.MM.YYYY')
        end f581,
case when to_char(f582,'SS') = 2 then 'Квартал '||to_char(f582,'Q')
     when to_char(f582,'SS') = 3 then 'Год '||to_char(f582,'YYYY')
     else to_char(f582,'DD.MM.YYYY')
        end f582,
case when to_char(f583,'SS') = 2 then 'Квартал '||to_char(f583,'Q')
     when to_char(f583,'SS') = 3 then 'Год '||to_char(f583,'YYYY')
     else to_char(f583,'DD.MM.YYYY')
        end f583,
case when to_char(f584,'SS') = 2 then 'Квартал '||to_char(f584,'Q')
     when to_char(f584,'SS') = 3 then 'Год '||to_char(f584,'YYYY')
     else to_char(f584,'DD.MM.YYYY')
        end f584,
case when to_char(f585,'SS') = 2 then 'Квартал '||to_char(f585,'Q')
     when to_char(f585,'SS') = 3 then 'Год '||to_char(f585,'YYYY')
     else to_char(f585,'DD.MM.YYYY')
        end f585,
case when to_char(f586,'SS') = 2 then 'Квартал '||to_char(f586,'Q')
     when to_char(f586,'SS') = 3 then 'Год '||to_char(f586,'YYYY')
     else to_char(f586,'DD.MM.YYYY')
        end f586,
case when to_char(f587,'SS') = 2 then 'Квартал '||to_char(f587,'Q')
     when to_char(f587,'SS') = 3 then 'Год '||to_char(f587,'YYYY')
     else to_char(f587,'DD.MM.YYYY')
        end f587,
case when to_char(f588,'SS') = 2 then 'Квартал '||to_char(f588,'Q')
     when to_char(f588,'SS') = 3 then 'Год '||to_char(f588,'YYYY')
     else to_char(f588,'DD.MM.YYYY')
        end f588,
case when to_char(f589,'SS') = 2 then 'Квартал '||to_char(f589,'Q')
     when to_char(f589,'SS') = 3 then 'Год '||to_char(f589,'YYYY')
     else to_char(f589,'DD.MM.YYYY')
        end f589,
case when to_char(f590,'SS') = 2 then 'Квартал '||to_char(f590,'Q')
     when to_char(f590,'SS') = 3 then 'Год '||to_char(f590,'YYYY')
     else to_char(f590,'DD.MM.YYYY')
        end f590,
case when to_char(f591,'SS') = 2 then 'Квартал '||to_char(f591,'Q')
     when to_char(f591,'SS') = 3 then 'Год '||to_char(f591,'YYYY')
     else to_char(f591,'DD.MM.YYYY')
        end f591,
case when to_char(f592,'SS') = 2 then 'Квартал '||to_char(f592,'Q')
     when to_char(f592,'SS') = 3 then 'Год '||to_char(f592,'YYYY')
     else to_char(f592,'DD.MM.YYYY')
        end f592,
case when to_char(f593,'SS') = 2 then 'Квартал '||to_char(f593,'Q')
     when to_char(f593,'SS') = 3 then 'Год '||to_char(f593,'YYYY')
     else to_char(f593,'DD.MM.YYYY')
        end f593,
case when to_char(f594,'SS') = 2 then 'Квартал '||to_char(f594,'Q')
     when to_char(f594,'SS') = 3 then 'Год '||to_char(f594,'YYYY')
     else to_char(f594,'DD.MM.YYYY')
        end f594,
case when to_char(f595,'SS') = 2 then 'Квартал '||to_char(f595,'Q')
     when to_char(f595,'SS') = 3 then 'Год '||to_char(f595,'YYYY')
     else to_char(f595,'DD.MM.YYYY')
        end f595,
case when to_char(f596,'SS') = 2 then 'Квартал '||to_char(f596,'Q')
     when to_char(f596,'SS') = 3 then 'Год '||to_char(f596,'YYYY')
     else to_char(f596,'DD.MM.YYYY')
        end f596,
case when to_char(f597,'SS') = 2 then 'Квартал '||to_char(f597,'Q')
     when to_char(f597,'SS') = 3 then 'Год '||to_char(f597,'YYYY')
     else to_char(f597,'DD.MM.YYYY')
        end f597,
case when to_char(f598,'SS') = 2 then 'Квартал '||to_char(f598,'Q')
     when to_char(f598,'SS') = 3 then 'Год '||to_char(f598,'YYYY')
     else to_char(f598,'DD.MM.YYYY')
        end f598,
case when to_char(f599,'SS') = 2 then 'Квартал '||to_char(f599,'Q')
     when to_char(f599,'SS') = 3 then 'Год '||to_char(f599,'YYYY')
     else to_char(f599,'DD.MM.YYYY')
        end f599,
case when to_char(f600,'SS') = 2 then 'Квартал '||to_char(f600,'Q')
     when to_char(f600,'SS') = 3 then 'Год '||to_char(f600,'YYYY')
     else to_char(f600,'DD.MM.YYYY')
        end f600,
case when to_char(f601,'SS') = 2 then 'Квартал '||to_char(f601,'Q')
     when to_char(f601,'SS') = 3 then 'Год '||to_char(f601,'YYYY')
     else to_char(f601,'DD.MM.YYYY')
        end f601,
case when to_char(f602,'SS') = 2 then 'Квартал '||to_char(f602,'Q')
     when to_char(f602,'SS') = 3 then 'Год '||to_char(f602,'YYYY')
     else to_char(f602,'DD.MM.YYYY')
        end f602,
case when to_char(f603,'SS') = 2 then 'Квартал '||to_char(f603,'Q')
     when to_char(f603,'SS') = 3 then 'Год '||to_char(f603,'YYYY')
     else to_char(f603,'DD.MM.YYYY')
        end f603,
case when to_char(f604,'SS') = 2 then 'Квартал '||to_char(f604,'Q')
     when to_char(f604,'SS') = 3 then 'Год '||to_char(f604,'YYYY')
     else to_char(f604,'DD.MM.YYYY')
        end f604,
case when to_char(f605,'SS') = 2 then 'Квартал '||to_char(f605,'Q')
     when to_char(f605,'SS') = 3 then 'Год '||to_char(f605,'YYYY')
     else to_char(f605,'DD.MM.YYYY')
        end f605,
case when to_char(f606,'SS') = 2 then 'Квартал '||to_char(f606,'Q')
     when to_char(f606,'SS') = 3 then 'Год '||to_char(f606,'YYYY')
     else to_char(f606,'DD.MM.YYYY')
        end f606,
case when to_char(f607,'SS') = 2 then 'Квартал '||to_char(f607,'Q')
     when to_char(f607,'SS') = 3 then 'Год '||to_char(f607,'YYYY')
     else to_char(f607,'DD.MM.YYYY')
        end f607,
case when to_char(f608,'SS') = 2 then 'Квартал '||to_char(f608,'Q')
     when to_char(f608,'SS') = 3 then 'Год '||to_char(f608,'YYYY')
     else to_char(f608,'DD.MM.YYYY')
        end f608,
case when to_char(f609,'SS') = 2 then 'Квартал '||to_char(f609,'Q')
     when to_char(f609,'SS') = 3 then 'Год '||to_char(f609,'YYYY')
     else to_char(f609,'DD.MM.YYYY')
        end f609,
case when to_char(f610,'SS') = 2 then 'Квартал '||to_char(f610,'Q')
     when to_char(f610,'SS') = 3 then 'Год '||to_char(f610,'YYYY')
     else to_char(f610,'DD.MM.YYYY')
        end f610,
case when to_char(f611,'SS') = 2 then 'Квартал '||to_char(f611,'Q')
     when to_char(f611,'SS') = 3 then 'Год '||to_char(f611,'YYYY')
     else to_char(f611,'DD.MM.YYYY')
        end f611,
case when to_char(f612,'SS') = 2 then 'Квартал '||to_char(f612,'Q')
     when to_char(f612,'SS') = 3 then 'Год '||to_char(f612,'YYYY')
     else to_char(f612,'DD.MM.YYYY')
        end f612,
case when to_char(f613,'SS') = 2 then 'Квартал '||to_char(f613,'Q')
     when to_char(f613,'SS') = 3 then 'Год '||to_char(f613,'YYYY')
     else to_char(f613,'DD.MM.YYYY')
        end f613,
case when to_char(f614,'SS') = 2 then 'Квартал '||to_char(f614,'Q')
     when to_char(f614,'SS') = 3 then 'Год '||to_char(f614,'YYYY')
     else to_char(f614,'DD.MM.YYYY')
        end f614,
case when to_char(f615,'SS') = 2 then 'Квартал '||to_char(f615,'Q')
     when to_char(f615,'SS') = 3 then 'Год '||to_char(f615,'YYYY')
     else to_char(f615,'DD.MM.YYYY')
        end f615,
case when to_char(f616,'SS') = 2 then 'Квартал '||to_char(f616,'Q')
     when to_char(f616,'SS') = 3 then 'Год '||to_char(f616,'YYYY')
     else to_char(f616,'DD.MM.YYYY')
        end f616,
case when to_char(f617,'SS') = 2 then 'Квартал '||to_char(f617,'Q')
     when to_char(f617,'SS') = 3 then 'Год '||to_char(f617,'YYYY')
     else to_char(f617,'DD.MM.YYYY')
        end f617,
case when to_char(f618,'SS') = 2 then 'Квартал '||to_char(f618,'Q')
     when to_char(f618,'SS') = 3 then 'Год '||to_char(f618,'YYYY')
     else to_char(f618,'DD.MM.YYYY')
        end f618,
case when to_char(f619,'SS') = 2 then 'Квартал '||to_char(f619,'Q')
     when to_char(f619,'SS') = 3 then 'Год '||to_char(f619,'YYYY')
     else to_char(f619,'DD.MM.YYYY')
        end f619,
case when to_char(f620,'SS') = 2 then 'Квартал '||to_char(f620,'Q')
     when to_char(f620,'SS') = 3 then 'Год '||to_char(f620,'YYYY')
     else to_char(f620,'DD.MM.YYYY')
        end f620,
case when to_char(f621,'SS') = 2 then 'Квартал '||to_char(f621,'Q')
     when to_char(f621,'SS') = 3 then 'Год '||to_char(f621,'YYYY')
     else to_char(f621,'DD.MM.YYYY')
        end f621,
case when to_char(f622,'SS') = 2 then 'Квартал '||to_char(f622,'Q')
     when to_char(f622,'SS') = 3 then 'Год '||to_char(f622,'YYYY')
     else to_char(f622,'DD.MM.YYYY')
        end f622,
case when to_char(f623,'SS') = 2 then 'Квартал '||to_char(f623,'Q')
     when to_char(f623,'SS') = 3 then 'Год '||to_char(f623,'YYYY')
     else to_char(f623,'DD.MM.YYYY')
        end f623,
case when to_char(f624,'SS') = 2 then 'Квартал '||to_char(f624,'Q')
     when to_char(f624,'SS') = 3 then 'Год '||to_char(f624,'YYYY')
     else to_char(f624,'DD.MM.YYYY')
        end f624,
case when to_char(f625,'SS') = 2 then 'Квартал '||to_char(f625,'Q')
     when to_char(f625,'SS') = 3 then 'Год '||to_char(f625,'YYYY')
     else to_char(f625,'DD.MM.YYYY')
        end f625,
case when to_char(f626,'SS') = 2 then 'Квартал '||to_char(f626,'Q')
     when to_char(f626,'SS') = 3 then 'Год '||to_char(f626,'YYYY')
     else to_char(f626,'DD.MM.YYYY')
        end f626,
case when to_char(f627,'SS') = 2 then 'Квартал '||to_char(f627,'Q')
     when to_char(f627,'SS') = 3 then 'Год '||to_char(f627,'YYYY')
     else to_char(f627,'DD.MM.YYYY')
        end f627,
case when to_char(f628,'SS') = 2 then 'Квартал '||to_char(f628,'Q')
     when to_char(f628,'SS') = 3 then 'Год '||to_char(f628,'YYYY')
     else to_char(f628,'DD.MM.YYYY')
        end f628,
case when to_char(f629,'SS') = 2 then 'Квартал '||to_char(f629,'Q')
     when to_char(f629,'SS') = 3 then 'Год '||to_char(f629,'YYYY')
     else to_char(f629,'DD.MM.YYYY')
        end f629,
case when to_char(f630,'SS') = 2 then 'Квартал '||to_char(f630,'Q')
     when to_char(f630,'SS') = 3 then 'Год '||to_char(f630,'YYYY')
     else to_char(f630,'DD.MM.YYYY')
        end f630,
case when to_char(f631,'SS') = 2 then 'Квартал '||to_char(f631,'Q')
     when to_char(f631,'SS') = 3 then 'Год '||to_char(f631,'YYYY')
     else to_char(f631,'DD.MM.YYYY')
        end f631,
case when to_char(f632,'SS') = 2 then 'Квартал '||to_char(f632,'Q')
     when to_char(f632,'SS') = 3 then 'Год '||to_char(f632,'YYYY')
     else to_char(f632,'DD.MM.YYYY')
        end f632,
case when to_char(f633,'SS') = 2 then 'Квартал '||to_char(f633,'Q')
     when to_char(f633,'SS') = 3 then 'Год '||to_char(f633,'YYYY')
     else to_char(f633,'DD.MM.YYYY')
        end f633,
case when to_char(f634,'SS') = 2 then 'Квартал '||to_char(f634,'Q')
     when to_char(f634,'SS') = 3 then 'Год '||to_char(f634,'YYYY')
     else to_char(f634,'DD.MM.YYYY')
        end f634,
case when to_char(f635,'SS') = 2 then 'Квартал '||to_char(f635,'Q')
     when to_char(f635,'SS') = 3 then 'Год '||to_char(f635,'YYYY')
     else to_char(f635,'DD.MM.YYYY')
        end f635,
case when to_char(f636,'SS') = 2 then 'Квартал '||to_char(f636,'Q')
     when to_char(f636,'SS') = 3 then 'Год '||to_char(f636,'YYYY')
     else to_char(f636,'DD.MM.YYYY')
        end f636,
case when to_char(f637,'SS') = 2 then 'Квартал '||to_char(f637,'Q')
     when to_char(f637,'SS') = 3 then 'Год '||to_char(f637,'YYYY')
     else to_char(f637,'DD.MM.YYYY')
        end f637,
case when to_char(f638,'SS') = 2 then 'Квартал '||to_char(f638,'Q')
     when to_char(f638,'SS') = 3 then 'Год '||to_char(f638,'YYYY')
     else to_char(f638,'DD.MM.YYYY')
        end f638,
case when to_char(f639,'SS') = 2 then 'Квартал '||to_char(f639,'Q')
     when to_char(f639,'SS') = 3 then 'Год '||to_char(f639,'YYYY')
     else to_char(f639,'DD.MM.YYYY')
        end f639,
case when to_char(f640,'SS') = 2 then 'Квартал '||to_char(f640,'Q')
     when to_char(f640,'SS') = 3 then 'Год '||to_char(f640,'YYYY')
     else to_char(f640,'DD.MM.YYYY')
        end f640,
case when to_char(f641,'SS') = 2 then 'Квартал '||to_char(f641,'Q')
     when to_char(f641,'SS') = 3 then 'Год '||to_char(f641,'YYYY')
     else to_char(f641,'DD.MM.YYYY')
        end f641,
case when to_char(f642,'SS') = 2 then 'Квартал '||to_char(f642,'Q')
     when to_char(f642,'SS') = 3 then 'Год '||to_char(f642,'YYYY')
     else to_char(f642,'DD.MM.YYYY')
        end f642,
case when to_char(f643,'SS') = 2 then 'Квартал '||to_char(f643,'Q')
     when to_char(f643,'SS') = 3 then 'Год '||to_char(f643,'YYYY')
     else to_char(f643,'DD.MM.YYYY')
        end f643,
case when to_char(f644,'SS') = 2 then 'Квартал '||to_char(f644,'Q')
     when to_char(f644,'SS') = 3 then 'Год '||to_char(f644,'YYYY')
     else to_char(f644,'DD.MM.YYYY')
        end f644,
case when to_char(f645,'SS') = 2 then 'Квартал '||to_char(f645,'Q')
     when to_char(f645,'SS') = 3 then 'Год '||to_char(f645,'YYYY')
     else to_char(f645,'DD.MM.YYYY')
        end f645,
case when to_char(f646,'SS') = 2 then 'Квартал '||to_char(f646,'Q')
     when to_char(f646,'SS') = 3 then 'Год '||to_char(f646,'YYYY')
     else to_char(f646,'DD.MM.YYYY')
        end f646,
case when to_char(f647,'SS') = 2 then 'Квартал '||to_char(f647,'Q')
     when to_char(f647,'SS') = 3 then 'Год '||to_char(f647,'YYYY')
     else to_char(f647,'DD.MM.YYYY')
        end f647,
case when to_char(f648,'SS') = 2 then 'Квартал '||to_char(f648,'Q')
     when to_char(f648,'SS') = 3 then 'Год '||to_char(f648,'YYYY')
     else to_char(f648,'DD.MM.YYYY')
        end f648,
case when to_char(f649,'SS') = 2 then 'Квартал '||to_char(f649,'Q')
     when to_char(f649,'SS') = 3 then 'Год '||to_char(f649,'YYYY')
     else to_char(f649,'DD.MM.YYYY')
        end f649,
case when to_char(f650,'SS') = 2 then 'Квартал '||to_char(f650,'Q')
     when to_char(f650,'SS') = 3 then 'Год '||to_char(f650,'YYYY')
     else to_char(f650,'DD.MM.YYYY')
        end f650,
case when to_char(f651,'SS') = 2 then 'Квартал '||to_char(f651,'Q')
     when to_char(f651,'SS') = 3 then 'Год '||to_char(f651,'YYYY')
     else to_char(f651,'DD.MM.YYYY')
        end f651,
case when to_char(f652,'SS') = 2 then 'Квартал '||to_char(f652,'Q')
     when to_char(f652,'SS') = 3 then 'Год '||to_char(f652,'YYYY')
     else to_char(f652,'DD.MM.YYYY')
        end f652,
case when to_char(f653,'SS') = 2 then 'Квартал '||to_char(f653,'Q')
     when to_char(f653,'SS') = 3 then 'Год '||to_char(f653,'YYYY')
     else to_char(f653,'DD.MM.YYYY')
        end f653,
case when to_char(f654,'SS') = 2 then 'Квартал '||to_char(f654,'Q')
     when to_char(f654,'SS') = 3 then 'Год '||to_char(f654,'YYYY')
     else to_char(f654,'DD.MM.YYYY')
        end f654,
case when to_char(f655,'SS') = 2 then 'Квартал '||to_char(f655,'Q')
     when to_char(f655,'SS') = 3 then 'Год '||to_char(f655,'YYYY')
     else to_char(f655,'DD.MM.YYYY')
        end f655,
case when to_char(f656,'SS') = 2 then 'Квартал '||to_char(f656,'Q')
     when to_char(f656,'SS') = 3 then 'Год '||to_char(f656,'YYYY')
     else to_char(f656,'DD.MM.YYYY')
        end f656,
case when to_char(f657,'SS') = 2 then 'Квартал '||to_char(f657,'Q')
     when to_char(f657,'SS') = 3 then 'Год '||to_char(f657,'YYYY')
     else to_char(f657,'DD.MM.YYYY')
        end f657,
case when to_char(f658,'SS') = 2 then 'Квартал '||to_char(f658,'Q')
     when to_char(f658,'SS') = 3 then 'Год '||to_char(f658,'YYYY')
     else to_char(f658,'DD.MM.YYYY')
        end f658,
case when to_char(f659,'SS') = 2 then 'Квартал '||to_char(f659,'Q')
     when to_char(f659,'SS') = 3 then 'Год '||to_char(f659,'YYYY')
     else to_char(f659,'DD.MM.YYYY')
        end f659,
case when to_char(f660,'SS') = 2 then 'Квартал '||to_char(f660,'Q')
     when to_char(f660,'SS') = 3 then 'Год '||to_char(f660,'YYYY')
     else to_char(f660,'DD.MM.YYYY')
        end f660,
case when to_char(f661,'SS') = 2 then 'Квартал '||to_char(f661,'Q')
     when to_char(f661,'SS') = 3 then 'Год '||to_char(f661,'YYYY')
     else to_char(f661,'DD.MM.YYYY')
        end f661,
case when to_char(f662,'SS') = 2 then 'Квартал '||to_char(f662,'Q')
     when to_char(f662,'SS') = 3 then 'Год '||to_char(f662,'YYYY')
     else to_char(f662,'DD.MM.YYYY')
        end f662,
case when to_char(f663,'SS') = 2 then 'Квартал '||to_char(f663,'Q')
     when to_char(f663,'SS') = 3 then 'Год '||to_char(f663,'YYYY')
     else to_char(f663,'DD.MM.YYYY')
        end f663,
case when to_char(f664,'SS') = 2 then 'Квартал '||to_char(f664,'Q')
     when to_char(f664,'SS') = 3 then 'Год '||to_char(f664,'YYYY')
     else to_char(f664,'DD.MM.YYYY')
        end f664,
case when to_char(f665,'SS') = 2 then 'Квартал '||to_char(f665,'Q')
     when to_char(f665,'SS') = 3 then 'Год '||to_char(f665,'YYYY')
     else to_char(f665,'DD.MM.YYYY')
        end f665,
case when to_char(f666,'SS') = 2 then 'Квартал '||to_char(f666,'Q')
     when to_char(f666,'SS') = 3 then 'Год '||to_char(f666,'YYYY')
     else to_char(f666,'DD.MM.YYYY')
        end f666,
case when to_char(f667,'SS') = 2 then 'Квартал '||to_char(f667,'Q')
     when to_char(f667,'SS') = 3 then 'Год '||to_char(f667,'YYYY')
     else to_char(f667,'DD.MM.YYYY')
        end f667,
case when to_char(f668,'SS') = 2 then 'Квартал '||to_char(f668,'Q')
     when to_char(f668,'SS') = 3 then 'Год '||to_char(f668,'YYYY')
     else to_char(f668,'DD.MM.YYYY')
        end f668,
case when to_char(f669,'SS') = 2 then 'Квартал '||to_char(f669,'Q')
     when to_char(f669,'SS') = 3 then 'Год '||to_char(f669,'YYYY')
     else to_char(f669,'DD.MM.YYYY')
        end f669,
case when to_char(f670,'SS') = 2 then 'Квартал '||to_char(f670,'Q')
     when to_char(f670,'SS') = 3 then 'Год '||to_char(f670,'YYYY')
     else to_char(f670,'DD.MM.YYYY')
        end f670,
case when to_char(f671,'SS') = 2 then 'Квартал '||to_char(f671,'Q')
     when to_char(f671,'SS') = 3 then 'Год '||to_char(f671,'YYYY')
     else to_char(f671,'DD.MM.YYYY')
        end f671,
case when to_char(f672,'SS') = 2 then 'Квартал '||to_char(f672,'Q')
     when to_char(f672,'SS') = 3 then 'Год '||to_char(f672,'YYYY')
     else to_char(f672,'DD.MM.YYYY')
        end f672,
case when to_char(f673,'SS') = 2 then 'Квартал '||to_char(f673,'Q')
     when to_char(f673,'SS') = 3 then 'Год '||to_char(f673,'YYYY')
     else to_char(f673,'DD.MM.YYYY')
        end f673,
case when to_char(f674,'SS') = 2 then 'Квартал '||to_char(f674,'Q')
     when to_char(f674,'SS') = 3 then 'Год '||to_char(f674,'YYYY')
     else to_char(f674,'DD.MM.YYYY')
        end f674,
case when to_char(f675,'SS') = 2 then 'Квартал '||to_char(f675,'Q')
     when to_char(f675,'SS') = 3 then 'Год '||to_char(f675,'YYYY')
     else to_char(f675,'DD.MM.YYYY')
        end f675,
case when to_char(f676,'SS') = 2 then 'Квартал '||to_char(f676,'Q')
     when to_char(f676,'SS') = 3 then 'Год '||to_char(f676,'YYYY')
     else to_char(f676,'DD.MM.YYYY')
        end f676,
case when to_char(f677,'SS') = 2 then 'Квартал '||to_char(f677,'Q')
     when to_char(f677,'SS') = 3 then 'Год '||to_char(f677,'YYYY')
     else to_char(f677,'DD.MM.YYYY')
        end f677,
case when to_char(f678,'SS') = 2 then 'Квартал '||to_char(f678,'Q')
     when to_char(f678,'SS') = 3 then 'Год '||to_char(f678,'YYYY')
     else to_char(f678,'DD.MM.YYYY')
        end f678,
case when to_char(f679,'SS') = 2 then 'Квартал '||to_char(f679,'Q')
     when to_char(f679,'SS') = 3 then 'Год '||to_char(f679,'YYYY')
     else to_char(f679,'DD.MM.YYYY')
        end f679,
case when to_char(f680,'SS') = 2 then 'Квартал '||to_char(f680,'Q')
     when to_char(f680,'SS') = 3 then 'Год '||to_char(f680,'YYYY')
     else to_char(f680,'DD.MM.YYYY')
        end f680,
case when to_char(f681,'SS') = 2 then 'Квартал '||to_char(f681,'Q')
     when to_char(f681,'SS') = 3 then 'Год '||to_char(f681,'YYYY')
     else to_char(f681,'DD.MM.YYYY')
        end f681,
case when to_char(f682,'SS') = 2 then 'Квартал '||to_char(f682,'Q')
     when to_char(f682,'SS') = 3 then 'Год '||to_char(f682,'YYYY')
     else to_char(f682,'DD.MM.YYYY')
        end f682,
case when to_char(f683,'SS') = 2 then 'Квартал '||to_char(f683,'Q')
     when to_char(f683,'SS') = 3 then 'Год '||to_char(f683,'YYYY')
     else to_char(f683,'DD.MM.YYYY')
        end f683,
case when to_char(f684,'SS') = 2 then 'Квартал '||to_char(f684,'Q')
     when to_char(f684,'SS') = 3 then 'Год '||to_char(f684,'YYYY')
     else to_char(f684,'DD.MM.YYYY')
        end f684,
case when to_char(f685,'SS') = 2 then 'Квартал '||to_char(f685,'Q')
     when to_char(f685,'SS') = 3 then 'Год '||to_char(f685,'YYYY')
     else to_char(f685,'DD.MM.YYYY')
        end f685,
case when to_char(f686,'SS') = 2 then 'Квартал '||to_char(f686,'Q')
     when to_char(f686,'SS') = 3 then 'Год '||to_char(f686,'YYYY')
     else to_char(f686,'DD.MM.YYYY')
        end f686,
case when to_char(f687,'SS') = 2 then 'Квартал '||to_char(f687,'Q')
     when to_char(f687,'SS') = 3 then 'Год '||to_char(f687,'YYYY')
     else to_char(f687,'DD.MM.YYYY')
        end f687,
case when to_char(f688,'SS') = 2 then 'Квартал '||to_char(f688,'Q')
     when to_char(f688,'SS') = 3 then 'Год '||to_char(f688,'YYYY')
     else to_char(f688,'DD.MM.YYYY')
        end f688,
case when to_char(f689,'SS') = 2 then 'Квартал '||to_char(f689,'Q')
     when to_char(f689,'SS') = 3 then 'Год '||to_char(f689,'YYYY')
     else to_char(f689,'DD.MM.YYYY')
        end f689,
case when to_char(f690,'SS') = 2 then 'Квартал '||to_char(f690,'Q')
     when to_char(f690,'SS') = 3 then 'Год '||to_char(f690,'YYYY')
     else to_char(f690,'DD.MM.YYYY')
        end f690,
case when to_char(f691,'SS') = 2 then 'Квартал '||to_char(f691,'Q')
     when to_char(f691,'SS') = 3 then 'Год '||to_char(f691,'YYYY')
     else to_char(f691,'DD.MM.YYYY')
        end f691,
case when to_char(f692,'SS') = 2 then 'Квартал '||to_char(f692,'Q')
     when to_char(f692,'SS') = 3 then 'Год '||to_char(f692,'YYYY')
     else to_char(f692,'DD.MM.YYYY')
        end f692,
case when to_char(f693,'SS') = 2 then 'Квартал '||to_char(f693,'Q')
     when to_char(f693,'SS') = 3 then 'Год '||to_char(f693,'YYYY')
     else to_char(f693,'DD.MM.YYYY')
        end f693,
case when to_char(f694,'SS') = 2 then 'Квартал '||to_char(f694,'Q')
     when to_char(f694,'SS') = 3 then 'Год '||to_char(f694,'YYYY')
     else to_char(f694,'DD.MM.YYYY')
        end f694,
case when to_char(f695,'SS') = 2 then 'Квартал '||to_char(f695,'Q')
     when to_char(f695,'SS') = 3 then 'Год '||to_char(f695,'YYYY')
     else to_char(f695,'DD.MM.YYYY')
        end f695,
case when to_char(f696,'SS') = 2 then 'Квартал '||to_char(f696,'Q')
     when to_char(f696,'SS') = 3 then 'Год '||to_char(f696,'YYYY')
     else to_char(f696,'DD.MM.YYYY')
        end f696,
case when to_char(f697,'SS') = 2 then 'Квартал '||to_char(f697,'Q')
     when to_char(f697,'SS') = 3 then 'Год '||to_char(f697,'YYYY')
     else to_char(f697,'DD.MM.YYYY')
        end f697,
case when to_char(f698,'SS') = 2 then 'Квартал '||to_char(f698,'Q')
     when to_char(f698,'SS') = 3 then 'Год '||to_char(f698,'YYYY')
     else to_char(f698,'DD.MM.YYYY')
        end f698,
case when to_char(f699,'SS') = 2 then 'Квартал '||to_char(f699,'Q')
     when to_char(f699,'SS') = 3 then 'Год '||to_char(f699,'YYYY')
     else to_char(f699,'DD.MM.YYYY')
        end f699,
case when to_char(f700,'SS') = 2 then 'Квартал '||to_char(f700,'Q')
     when to_char(f700,'SS') = 3 then 'Год '||to_char(f700,'YYYY')
