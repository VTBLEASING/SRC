CREATE OR REPLACE FORCE VIEW DM.V$DM_XIRR_FLOW AS
SELECT
        ...
        og.CUR1,
--        og.branch_key,
        sum(case when tp = 'LEASING' then pay_amt_cur end ) LEASING_PLAN_PAY_AMT,
        sum(case when tp = 'LEASING_FACT' AND pay_dt <= og.snapshot_dt then pay_amt_cur end ) LEASING_FACT_PAY_AMT,
        sum(case when tp = 'LEASING' AND pay_dt <= og.snapshot_dt then pay_amt_cur end ) DT_LEASING_PLAN_PAY_AMT,
