CREATE OR REPLACE FORCE VIEW DM.V_EVENTS AS
WITH
Q11 AS
(
SELECT
...
,ACT_DT   AS   ACT_DT
,TASK_BEGIN_DT   AS   TASK_BEGIN_DT
,TASK_END_DT   AS   TASK_END_DT
,ADVANCE_PERCENT  AS   ADVANCE_PERCENT
--,ADVPAYMENT AS ADVPAYMENT
,AGENT_FEE_AMT  AS   AGENT_FEE_AMT
,AGENT_FEE_RATE   AS   AGENT_FEE_RATE
,AGREEMENT_KEY AS AGREEMENT_KEY
,TO_CHAR(AGREEMENT_RESULT)   AS   AGREEMENT_RESULT
,APPEAL_SOURCE  AS   APPEAL_SOURCE
,DBMS_LOB.SUBSTR(APPROVAL_CONDITIONS,2000,1) AS  APPROVAL_CONDITIONS_1
,DBMS_LOB.SUBSTR(APPROVAL_CONDITIONS,2000,1) AS  APPROVAL_CONDITIONS_2
,DBMS_LOB.SUBSTR(TASK_HISTORY,2000,1) AS   TASK_HISTORY_1
,DBMS_LOB.SUBSTR(TASK_HISTORY,2000,2001) AS   TASK_HISTORY_2
,DBMS_LOB.SUBSTR(RECOMMENDAT_DESC,2000,1) AS  RECOMMENDAT_DESC_1
,DBMS_LOB.SUBSTR(RECOMMENDAT_DESC,2000,2001) AS  RECOMMENDAT_DESC_2
...
,CASE WHEN SUBSIDY_FLG=0 THEN PREPAY_AMT  WHEN SUBSIDY_FLG=1 THEN ADVPAYMENT END AS PREPAY_AMT --CHR-447
--,PREPAY_RATE  AS   PREPAY_RATE
,CASE WHEN SUBSIDY_FLG=0 THEN PREPAY_AMT/FULLCOST_AMT*CONVERT_RATE  WHEN SUBSIDY_FLG=1 THEN ADVPAYMENT/FULLCOST_AMT*CONVERT_RATE END AS PREPAY_RATE --CHR-447
...
WHEN EVENT_NUM=10000001 THEN 'Запуск задачи на верификацию'
  WHEN EVENT_NUM=10000002 THEN 'Запуск задачи на Экпертизу УОБ'
    WHEN EVENT_NUM=10000003 THEN 'Закрытие экспертизы УОБ'
...
,
Q1 AS
(/*cmr_contract_id_cd) + сквозной номер + событие */
SELECT A.EVENT_DATE AS EVENT_DATE, A.STAGE AS STAGE, B.EVENT AS EVENT,A.EVENTS_KEY AS EVENTS_KEY FROM
(SELECT
Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY, MIN(Q11.EVENT_DATE) over (partition by CRM_CONTRACT_ID_CD, leasing_subject_num, event) as EVENT_DATE
--Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY, MIN(Q11.EVENT_DATE) over (partition by Q11.STAGE,Q11.EVENTS_KEY) AS EVENT_DATE
--MIN(TO_CHAR(Q11.EVENT_DATE,'mm/dd/yyyy hh24:mi:ss')) AS EVENT_DATE, Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY
FROM Q11
where Q11.STAGE IN ('Запуск задачи на верификацию','Запуск задачи на Экпертизу УОБ','Запуск задачи на Одобрение'
...
SELECT A.EVENT_DATE AS EVENT_DATE, A.STAGE AS STAGE, B.EVENT AS EVENT,A.EVENTS_KEY AS EVENTS_KEY FROM
(SELECT
Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY, MAX(Q11.EVENT_DATE) over (partition by CRM_CONTRACT_ID_CD, leasing_subject_num, event) as EVENT_DATE
--Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY, MAX(Q11.EVENT_DATE) over (partition by Q11.STAGE,Q11.EVENTS_KEY) AS EVENT_DATE
--MAX(TO_CHAR(Q11.EVENT_DATE,'mm/dd/yyyy hh24:mi:ss')) AS EVENT_DATE, Q11.STAGE AS STAGE,Q11.EVENTS_KEY AS EVENTS_KEY
FROM Q11
where Q11.STAGE IN ('Закрытие задачи Одобрения','Закрытие экспертизы УОБ','Закрытие экспертизы УАиКР')
--GROUP BY Q11.STAGE, Q11.EVENTS_KEY
)A INNER JOIN  (SELECT Q11.STAGE AS STAGE, Q11.EVENT_DATE AS EVENT_DATE
...
,Q11.TASK_BEGIN_DT AS TASK_BEGIN_DT
,Q11.TASK_END_DT AS TASK_END_DT
,Q11.advance_percent AS ADVANCE_PERCENT
,Q11.agent_fee_amt AS AGENT_FREE_AMT
,Q11.agent_fee_rate AS AGENT_FREE_RATE
,Q11.AGREEMENT_KEY AS AGREEMENT_KEY
,Q11.agreement_result AS AGREEMENT_RESULT
,Q11.appeal_source AS APPEAL_SOURCE
,Q11.APPROVAL_CONDITIONS_1 AS APPROVAL_CONDITIONS_1
,Q11.APPROVAL_CONDITIONS_2 AS APPROVAL_CONDITIONS_2
,Q11.TASK_HISTORY_1 AS TASK_HISTORY_1
,Q11.TASK_HISTORY_2 AS TASK_HISTORY_2
,Q11.RECOMMENDAT_DESC_1 AS RECOMMENDAT_DESC_1
,Q11.RECOMMENDAT_DESC_2 AS RECOMMENDAT_DESC_2
...
,Q11.agent_fee_amt AS AGENT_FREE_AMT
,Q11.agent_fee_rate AS AGENT_FREE_RATE
,Q11.AGREEMENT_KEY AS AGREEMNT_KEY
,Q11.agreement_result AS AGREEMENT_RESULT
,Q11.appeal_source AS APPEAL_SOURCE
,Q11.APPROVAL_CONDITIONS_1 AS APPROVAL_CONDITIONS_1
,Q11.APPROVAL_CONDITIONS_2 AS APPROVAL_CONDITIONS_2
...
SELECT "EVENTS_KEY","ACT_DT","TASK_BEGIN_DT","TASK_END_DT","ADVANCE_PERCENT","AGENT_FREE_AMT","AGENT_FREE_RATE","AGREEMENT_KEY","AGREEMENT_RESULT","APPEAL_SOURCE","APPROVAL_CONDITIONS_1","APPROVAL_CONDITIONS_2","TASK_HISTORY_1","TASK_HISTORY_2","RECOMMENDAT_DESC_1","RECOMMENDAT_DESC_2","APPROVAL_RESULT","APPROVAL_ROUTE","BREND_MODEL_NAM","BREND_NAM","BUSINESS_UNIT_NAM","CAR_TYPE_NAM","COMISSION_AMT","COMISSION_RATE","CONVERT_RATE","COUNTRY_RU_NAM","CRM_CONTRACT_ID_CD","CRM_SUBJECT_CD","CURRENCY_LEAS_SUBJECT","EVENT","EVENT_DATE","EVENT_NUM","FINANCE_AMT","FULLCOST_AMT","INN_LEASEHOLDER","INN_SUPPLIER","KASKO_CRM_CLIENT_NAM","LEASE_TERM","LEASING_SUBJECT_NAM","LEASING_SUBJECT_NUM","LIQUIDITY_GROUP","MANAGER_NAM","MODEL_NAM","NAM_LEASEHOLDER","OFFER_RATE","OKVED_CODE","PAYMENT_TYPE_NAM","PLAN_AMT","PREPAY_AMT","PREPAY_RATE","PRE_CONTRACT_ID_CD","PRE_CONTRACT_NUM","PRODUCT_NAM","REDEMPTION_AMT","REDUCE_STEP","REHIRING_FLG","STAGE","SUBPRODUCT_NAM","SUBSIDY_AMT","SUBSIDY_FLG","SUBSIDY_PROGRAM","SUBSIDY_SHARE","SUPPLIER_NAM","TASK_SUBJECT","TESTDRIVE_FLG","USED_FLG","VEHICLE_CATEGORY_NAM","FILE_ID","SESS_NO","VALID_FROM_DTTM","VALID_TO_DTTM","PROCESS_KEY","W_RATE","W_ADVANCE","W_AGENT_FREE_AMT","W_COMMISSION_AMT","W_LEASE_TERM","W_REDUCE_STEP" FROM Q3
UNION ALL
SELECT "EVENTS_KEY","ACT_DT","TASK_BEGIN_DT","TASK_END_DT","ADVANCE_PERCENT","AGENT_FREE_AMT","AGENT_FREE_RATE","AGREEMNT_KEY","AGREEMENT_RESULT","APPEAL_SOURCE","APPROVAL_CONDITIONS_1","APPROVAL_CONDITIONS_2","TASK_HISTORY_1","TASK_HISTORY_2","RECOMMENDAT_DESC_1","RECOMMENDAT_DESC_2","APPROVAL_RESULT","APPROVAL_ROUTE","BREND_MODEL_NAM","BREND_NAM","BUSINESS_UNIT_NAM","CAR_TYPE_NAM","COMISSION_AMT","COMISSION_RATE","CONVERT_RATE","COUNTRY_RU_NAM","CRM_CONTRACT_ID_CD","CRM_SUBJECT_CD","CURRENCY_LEAS_SUBJECT","EVENT","EVENT_DATE","EVENT_NUM","FINANCE_AMT","FULLCOST_AMT","INN_LEASEOLDER","INN_SUPPLIER","KASKO_CRM_CLIENT_NAM","LEASE_TERM","LEASING_SUBJECT_NAM","LEASING_SUBJECT_NUM","LIQUIDITY_GROUP","MANAGER_NAM","MODEL_NAM","NAM_LEASEHOLDER","OFFER_RATE","OKVED_CODE","PAYMENT_TYPE_NAM","PLAN_AMT","PREPAY_AMT","PREPAY_RATE","PRE_CONTRACT_ID_CD","PRE_CONTRACT_NUM","PRODUCT_NAM","REDEMPTION_AMT","REDUCE_STEP","REHIRING_FLG","STAGE","SUBPRODUCT_NAM","SUBSIDY_AMT","SUBSIDY_FLG","SUBSIDY_PROGRAM","SUBSIDY_SHARE","SUPPLIER_NAM","TASK_SUBJECT","TESTDRIVE_FLG","USED_FLG","VEHICLE_CATEGORY_NAM","FILE_ID","SESS_NO","VALID_FROM_DTTM","VALID_TO_DTTM","PROCESS_KEY","W_RATE","W_ADVANCE","W_AGENT_FREE_AMT","W_COMMISSION_AMT","W_LEASE_TERM","W_REDUCE_STEP" FROM Q4
;
