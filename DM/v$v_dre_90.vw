create or replace force view dm.v$v_dre_90 as
select DISTINCT
  A.CLIENT_INN,A.CLIENT_KEY AS CLIENT_ID
  ...
  ,A.DATE_90 AS DATE_DEFAULT--change
    ,B.DATE_90_CNCL AS DATE_DEFAULT_CNCL--change
    ,'90+' AS DEFAULT_EVENT--change
    ,A.CONTRACT_APP_KEY AS CONTRACT_APP_KEY
  ,'' AS SOURCE_ITEM
    FROM
        (select 'N_'||row_number() over(ORDER BY DATE_90) as ANUM--change
                ...
                ,DATE_90, REPORT_DT AS REPORT_DT
                ,CLIENT_KEY AS CLIENT_KEY
                 from dwh.default_base_table
                 where DATE_90 is null and
         VALID_TO_DTTM=TO_DATE('01.01.2400','DD.MM.YY')--change
                 ) A
        left join
                 (select 'N_'||row_number() over(ORDER BY DATE_90_CNCL) as ANUM--change
                         ,contract_num_shrt AS CONTRACT_NUM_SHRT
             ...
             ,REPORT_DT AS REPORT_DTB--change
                         ...
                         from dwh.default_base_table
                          where --DATE_90_CNCL is null and
              VALID_TO_DTTM=TO_DATE('01.01.2400','DD.MM.YY')--change
                 ) B
         on
        ...
        LEFT JOIN
          ...
          ON
          A.CONTRACT_APP_KEY=E.CONTRACT_APP_KEY
;
