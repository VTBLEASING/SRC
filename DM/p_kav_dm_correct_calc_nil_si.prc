CREATE OR REPLACE PROCEDURE DM.p_KAV_DM_CORRECT_CALC_NIL_SI (
      p_contract_key in number,
      p_REPORT_DT in date,
      p_REPORT_CORRECT_DT in date
)
IS
 ...
 /*                                   union all
                     ...
                     UNDERPAY_L as (
                                  select L_fact.L_key CONTRACT_KEY, 
                                         (CASE 
                                            WHEN nvl(L_plan.summ_p,0) = 0 then 0
                                            ELSE                                            
                                            CASE  
                                  ...
                                  dnil as (select a.dnil_amt + b.CORRECT_AMT as new_dnil_amt from dm.dm_repayment_schedule a, dwh.nil_corrects b 
                                                  where a.contract_key = p_contract_key and a.contract_key = b.contract_key and a.snapshot_dt = p_REPORT_CORRECT_DT 
                                                  and a.pay_dt = p_REPORT_CORRECT_DT
                                              ...
                                              select l_key, branch_key, pay_dt, summ, sum(summ + sum_prev) 
                                                    ...
                                                    rows between unbounded preceding and current row) as sum_prev, leasing_pay, supply_pay, pay_amt, under_l_pay from 
                                              ...
                                              p_REPORT_CORRECT_DT as pay_dt, -new_dnil_amt as summ, 0 as sum_prev, 0 as leasing_pay, -new_dnil_amt as supply_pay,
                                              -new_dnil_amt as pay_amt, 0 as under_l_pay from dnil
                                              union all
                                              select l_key, branch_key, pay_dt, summ, 0 as sum_prev, leasing_pay, supply_pay, pay_amt, under_l_pay from 
                                              (Select f.L_KEY, 
                                                     f.branch_key,
                                                     f.PAY_DT, 
                                                     sum (f.summ) summ,
                                              ...
                                              group by f.l_key, f.branch_key, f.pay_dt, UNDER_L.pay_amt) where pay_dt >= p_REPORT_CORRECT_DT)  -- склейка
...
dbms_output.put_line(rec.pay_dt);
dbms_output.put_line(prev_dt);
dbms_output.put_line(p_XIRR);
