CREATE OR REPLACE PROCEDURE DM."P_DM_REG_CGP" (p_REPORT_DT in date, p_reg_group_key in number default 1)
IS


BEGIN
    delete from DM_REG_CGP where snapshot_dt = p_REPORT_DT and branch_key in (select branch_key from dwh.reg_group where reg_group_key = p_reg_group_key);
   dm.u_log(p_proc => 'P_DM_REG_CGP',
    ...
    insert into DM_REG_CGP (
          ...
          select 
                  ...
                  cgp.OVERDUE_VAT_FREE_AMT  AS SRC_AMT,
                  case 
                    when cr.currency_letter_cd = 'RUB' 
                      then cgp.OVERDUE_VAT_FREE_AMT 
                    else cgp.OVERDUE_VAT_FREE_AMT  * er.exchange_rate 
                  end as RUR_AMT,
                  case 
                    when cr.currency_letter_cd in ('USD', 'EUR', 'RUB') 
                      then cgp.OVERDUE_VAT_FREE_AMT 
                  ...
                  cgp.OVERDUE_VAT_FREE_AMT  AS SRC_LIQ_AMT,
                  case 
                    when cr.currency_letter_cd in ('USD', 'EUR', 'RUB') 
                      then cgp.OVERDUE_VAT_FREE_AMT 
                    else cgp.OVERDUE_VAT_FREE_AMT  * er.exchange_rate 
                  end as CIS_LIQ_AMT,
                  cgp.OVERDUE_VAT_FREE_AMT  as SRC_MR_AMT,
                  case 
                    when cr.currency_letter_cd = 'RUB' 
                      then cgp.OVERDUE_VAT_FREE_AMT 
                    else cgp.OVERDUE_VAT_FREE_AMT  * er.exchange_rate 
                   ...
                   case 
                    when cr.currency_letter_cd = 'RUB' 
                      then cgp.OVERDUE_VAT_FREE_AMT  * cgp.XIRR_RATE 
                      else cgp.OVERDUE_VAT_FREE_AMT  * cgp.XIRR_RATE * er.exchange_rate ---// Рашковский - добавил пересчет в рубли для MR
