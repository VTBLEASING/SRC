CREATE OR REPLACE PROCEDURE DM.p_DM_OPER_START_DT (
      p_REPORT_DT date,
...
BEGIN
  dm.u_log(p_proc => 'DM.p_DM_OPER_START_DT',
           p_step => 'INPUT PARAMS',
...
WITH contr
        AS (
                ...
                -- aapolyakov [27.10.2015]: вяжемся с ручным справочником групп организаций для расчета даты для данной группы организаций
                INNER JOIN dwh.cgp_group cgp_group                                   
                       ON    c.branch_key = cgp_group.branch_key
