CREATE OR REPLACE PROCEDURE DM.p_DM_LOANS_CLIENT (
    p_REPORT_DT date
)
-- Витрина данных для отчетов Loans , агрегированная до клиента
is
    v_PROSR_sumr number;  
    v_PROSR_cur number;
    ...
    select  PROSR, PROSR_R
           ...
           ,SUM2, SUM2_R
            ...
            ,PROVISIONS_RATE
            from DM.DM_LOANS_CLIENT
                where snapshot_dt = p_REPORT_DT
                for update;
                
                
BEGIN
    dm.u_log(p_proc => 'DM.p_DM_LOANS_CLIENT',
           p_step => 'INPUT PARAMS',
...
delete from DM.DM_LOANS_CLIENT
where snapshot_dt = p_REPORT_DT;

    insert into DM.DM_LOANS_CLIENT
    (      SNAPSHOT_DT,
           CONTRACT_ID_CD,
           CONTRACT_NUM,
           CLIENT_KEY,
       ...
       WITH CGP as (Select A.SNAPSHOT_DT, A.CONTRACT_NUM, A.CONTRACT_KEY, A.CLIENT_NAM, A.CLIENT_KEY, A.BUSINESS_CATEGORY_KEY, A.START_DT, A.END_DT, A.OVERDUE_DT, A.XIRR_RATE, A.CURRENCY_KEY, A.TERM_AMT,
                    ...
                    else a.overdue_vat_free_amt --Zanozin round(A.OVERDUE_AMT/(1+case when VAT_CON.VAT_RATE is not null then VAT_CON.VAT_RATE else VAT.VAT_RATE end),2)
                    end OVERDUE_AMT, A.CLIENT_ID, A.ACTIVITY_TYPE_KEY
                    from DM.DM_CGP A
                       /*  left join DWH.VAT VAT on A.BRANCH_KEY = VAT.BRANCH_KEY 
...
-- [apolyakov 13012017]: связка с видом деятельности по ключу из КГП     
  ...
  dm.u_log(p_proc => 'DM.p_DM_LOANS',
           p_step => 'update DM.DM_LOANS_CLIENT',
