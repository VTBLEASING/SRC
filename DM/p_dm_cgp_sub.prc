CREATE OR REPLACE PROCEDURE DM.p_DM_CGP_SUB (
    p_REPORT_DT date,
     ...
     (   
      select contract_key as contract_id, sum(term_amt) as term_amt, lastday from 
        ...
        from (
              select 
                  contract_key,
                  nvl (lead(calc_pay_dt) over               -- nvl на случай, если следующего платежа нет, проставляется отчетная дата.
                              (partition by contract_key order by calc_pay_dt), p_REPORT_DT) -
                              pay_dt K_DAY,
                  dnil_amt * (
                              nvl (lead(calc_pay_dt) over               -- nvl на случай, если следующего платежа нет, проставляется отчетная дата.
                              (partition by contract_key order by calc_pay_dt), p_REPORT_DT) -
               ...
               from
                    (select snapshot_dt,
                           ...
                           pay_dt,
                           pay_dt calc_pay_dt,
                           dnil_amt * (100/(100 + vat.vat_rate*100)) dnil_amt
  ...
  /*       left join 
      ...
      case 
         when fact_close_dt <= p_REPORT_DT
                ...
                and (VP.contract_key is not null or br.branch_nam != 'ОАО ВТБ Лизинг')
     ...
     select cgp.*,
      case 
         when fact_close_dt <= p_REPORT_DT
                and nvl(term_amt, 0) = 0
                and nvl(round (overdue_amt), 0) = 0
                and (VP.contract_key is not null or br.branch_nam != 'ОАО ВТБ Лизинг' or lp.exclude_contract_key is not null)
                and cgp.contract_key not in ( 
                                      select contract_key 
                                      from dm_cgp_contracts_include cci
                                      where cci.begin_dt <= p_REPORT_DT 
