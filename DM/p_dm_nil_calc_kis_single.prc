CREATE OR REPLACE PROCEDURE DM.p_DM_NIL_CALC_KIS_SINGLE (
...
insert into DM_NIL(contract_id, contract_date, fact_amt, plan_amt, leasing_amt, supply_amt, nil_pers, nil, nil_diff, odttm, ka, nil_ka, kb, nil_ka_kb, pay_amt, branch_key, snapshot_cd, underpay_leas)
...
select 
      ...
      CLOSED_ROW_FILE_ID
from (
with ovd as (select  /*+ materialize */
                     l_key as contract_key, 
                     ...
                     and l_key = p_contract_key
                     and upper(tp) in ('LEASING_FACT', 'LEASING') group by l_key),
SUB_NIL_AMT as (
              ...
              from (
              select /*+ use_hash(p, ovd) cardinality (ovd, 10000)*/
              c.contract_key, p.pay_dt as pay_dt, cur.currency_cd, cur.currency_key, round(p.pay_amt/118*100,2) as pay_amt, 
              ...
              and upper(p.tp) = 'LEASING') group by pay_dt, contract_key),
/*with SUB_NIL_AMT as (
