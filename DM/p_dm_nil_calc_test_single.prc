CREATE OR REPLACE PROCEDURE DM.p_DM_NIL_CALC_TEST_SINGLE (
                                              ...
                                              left join 
                                                      (
                                                        Select 
                                                              ...
                                                              p.plan_pay_dt 
                                                        from 
                                                              DWH.FACT_REAL_PAYMENTS p,
                                                              dm.dm_xirr cg
                                                        where p.plan_pay_dt>p_REPORT_DT and p.pay_dt<=p_REPORT_DT
                                                              and cg.CONTRACT_id = p.CONTRACT_KEY
                                                              and cg.snapshot_cd = 'Тест'
                                                              and cg.odttm = p_REPORT_DT
                                                              and p.CBC_DESC in (
                                                                                select CBC_DESC 
                                                                                from DWH.CLS_CBC_TYPE_CALC 
                                                                                where TYPE_CALC = 'Leasing' 
                                                                                and to_date('31.10.2014','dd.mm.yyyy') BETWEEN BEGIN_DT and END_DT)
                                                        ) UNDER_L
                                              on UNDER_L.CONTRACT_KEY = f.L_KEY and UNDER_L.plan_pay_dt = f.pay_dt
                                              where x.odttm = p_REPORT_DT
                                              and x.snapshot_cd = 'Тест'
                ...
                then
                 ...
                 -- XIRR := xirr_calc_new(rec.contract_id)/100;
                  ...
                  ka_prev := ka;
                          ...
                          --elsif ka_prev = 0 and diff_prev != 0 then ka := round(diff_prev2 * ka_prev2 / -1, 2); -- падало на нуле
                  ...
                  insert into dm_nil (contract_id,contract_date, fact_amt, plan_amt, leasing_amt, supply_amt,nil_pers,nil,nil_diff,ODTTM,ka, nil_ka, PAY_AMT, branch_key, snapshot_cd, UNDERPAY_LEAS)
                  values(rec.L_KEY,rec.pay_dt,(case when rec.summ < 0 then rec.summ else 0 end), (case when rec.summ > 0 then rec.summ else 0 end), rec.leasing_pay, rec.supply_pay, pers,nill,diff,p_REPORT_DT,round(ka, 2), nil_ka, rec.summ, rec.branch_key, 'Тест', rec.UNDER_L_PAY);
                  ...
                  insert into dm_nil (contract_id,contract_date, fact_amt, plan_amt, leasing_amt, supply_amt, nil_pers,nil,nil_diff,ODTTM, ka, nil_ka, PAY_AMT, branch_key, snapshot_cd, UNDERPAY_LEAS)
                  values(rec.L_KEY,rec.pay_dt,(case when rec.summ < 0 then rec.summ else 0 end), (case when rec.summ > 0 then rec.summ else 0 end), rec.leasing_pay, rec.supply_pay, pers,nill,diff,p_REPORT_DT,round(ka, 2), nil_ka, rec.PAY_AMT, rec.branch_key, 'Тест', rec.UNDER_L_PAY);
                  commit;
              end if;
            end loop;
           -- end if;
...
a.PAY_AMT, 
CASE
  WHEN (round (a.NIL_KA_KB * (100 / (100 + v.vat_rate*100)), 2) - ROUND (a.UNDERPAY_LEAS * (100 / (100 + v.vat_rate*100)), 2)) < 0
    THEN 0
